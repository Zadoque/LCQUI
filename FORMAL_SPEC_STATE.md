...
M0 = VALIDATED; M1 = VALIDATED; M2 = VALIDATED; M3 = VALIDATED; M4 = VALIDATED; M5 = VALIDATED; M6 = VALIDATED; M7 = VALIDATED; M8 = VALIDATED; M9 = VALIDATED; M10 = VALIDATED; M11 = VALIDATED (documental e executável; HQ-M11-001 resolvida = A; escopo exato: Turma, matrícula ativa, vínculo canônico/espelho e convite de ingresso — Posts, Comentários, Roteiros e notificações acadêmicas **não** estão cobertos por M11); M12.1 = VALIDATED (documental e executável; Posts, Comentários, edição/moderação e históricos; CUE → IR v3 aditivo → Alloy → receipt → Rust → LaTeX → PDF; fonte normativa na Seção 7.6). Rodada corretiva fechou contraexemplos de participação (ex-aluno com claim atualizada não edita/lê) e substituiu o M7 vacuoso por composição com receipt (primeira execução/retry/reuso), além de tornar `TurmaArquivadaNegaEscrita` transversal: **40 checks UNSAT + 17 witnesses SAT**; M12.2 = VALIDATED (documental e executável; Roteiros, compartilhamento, Storage/download e associação a Post; fonte normativa na Seção 7.7; Q09 incorporada; CUE → IR v3 aditivo → Alloy → receipt → Rust → LaTeX → PDF; **47 checks UNSAT + 25 witnesses SAT** após rodada corretiva que fechou compartilhamento de roteiro não publicável (M12.2-CORR-01) e escopo Q13 do Chefe (M12.2-CORR-02); refina aditivamente a fronteira de M12.1 sem alterá-la); M12 = VALIDATED (fechamento de composição M12.1/M12.2 e regressão M0–M11; prova conjunta em `specification/alloy/operations/composition_m12.als` — `EstadoIntegrado` único, ponte `ponteAcessoRoteiro`, 37 checks UNSAT + 18 witnesses SAT = 55 resultados; receipt `build/formal-validation-m12.json`; `validation_m12.rs`; guard `m12_composition.mjs`/`m12_contract.test.mjs`; capítulo `Formal-Spec-M12.tex`; PDF de 442 páginas; itens 2a/2b fechados: `compartilhar` exige `roteiroPublicavel` explícito e a rota acadêmica exige papel acadêmico `alunos` + vínculo atual, pois M9/M11 não eliminam vínculos atomicamente; HQs M12 = 0); M13 = VALIDATED (Notificação unificada; contrato documental na Seção 7.8, `\label{sec:regras-notificacoes-m13}`; executável por CUE `#M13Contrato` (16 fixtures válidas + 18 inválidas) → IR v3 aditivo `formal_m13_notificacoes` → `notificacoes_m13.als` com estado composto (**34 checks UNSAT + 25 witnesses SAT = 59 resultados**; escopos 4/5) → receipt `build/formal-validation-m13.json` → `validation_m13.rs` (35 testes Rust no total) → guard `m13_composition.mjs`/`m13_composition.test.mjs`/`m13_contract.test.mjs` (44 testes Node no total) → geração determinística → capítulo `Formal-Spec-M13.tex` → PDF de 457 páginas; caixa única por UID, leitura/marcação/`Limpar tudo` paginado com corte estável sem `DELETE`, expiração, alvo com revalidação corrente M9/M11/M12 e escopo Q13, privacidade e dedup M7/M8; HQs M13 = 0; worklogs `M13_DOCUMENTATION.md` e `M13_EXECUTABLE_VALIDATION.md`). **Fechamento global M0–M13 = PASS** (`documentation/worklogs/formal-spec/GLOBAL_FORMAL_CLOSURE.md`, revisão de evidência `9c01d58a`). O fechamento global após M13 é um gate, sem criar M14 automaticamente. HEAD de entrada da rodada executável M11: `01fe84fde32aacee8e8c165f76faba40a5b2b786`; HEAD de entrada da rodada documental M12.1: `9f5c6713114ee9ef2d77eb98f31e0788053caa7b`. HEAD de entrada da formalização executável M10: `1be21d82fb8fa6f9528425578ddec86565cdb797` (árvore limpa, `origin` sincronizada). A cadeia CUE → IR → Alloy → receipt → Rust → geração → LaTeX → PDF foi concluída: `#M10Contrato` com 11 fixtures válidas e 11 inválidas, IR v3 aditivo (`formal_m10_patrimonio`), `patrimony_m10.als` com 29 checks UNSAT e 17 witnesses SAT, receipt verificável, validator Rust, guard de drift, canonicalização `N(s)`, fragmentos determinísticos e PDF de 389 páginas (390 após a reconciliação documental da busca de reagentes). M0–M9 sem regressão (receipts antigos mudaram somente em `spec_ir_sha256`); a implementação real permanece fora da evidência. A consolidação executável foi commitada em `03aa2771b7237afb59d22daae6e29198503f8034`; a reconciliação documental pós-M10 da busca de reagentes (M0–M10 inalterados, PDF 390 páginas) está em `documentation/worklogs/formal-spec/REAGENT_SEARCH_RECONCILIATION.md`. A rodada documental M11 (Turma, matrícula ativa, vínculo Aluno--Turma e convite de ingresso) foi conduzida sem alterar CUE/IR/Alloy/receipts/validadores Rust/gerador/frontend/functions/Rules: contrato nas Seções 3--11, exemplos de fluxo e PDF de 397 páginas, registrado em `documentation/worklogs/formal-spec/M11_DOCUMENTATION.md`. A HQ-M11-001 (redução de capacidade abaixo da ocupação) foi decidida pela **Opção A** (proibida; servidor rejeita fail-closed) e a fatia não possui HQ bloqueante. Com três auditorias de consistência sem novas falhas, **M11 = DOCUMENTATION_VALIDATED** para a fatia documental explicitamente definida. A formalização executável de M11 foi subsequentemente concluída: CUE `#M11Contrato` (12 fixtures válidas + 12 inválidas), IR v3 aditivo `formal_m11_turmas`, `turmas_m11.als` (33 checks UNSAT + 17 witnesses SAT), receipt verificável `build/formal-validation-m11.json`, `validation_m11.rs` (29 testes Rust no total), guard `m11_contract.test.mjs`, geração determinística e PDF de 405 páginas; **M11 = VALIDATED**. Próxima ação exata: **fechamento global pós-M13 (gate separado, sem M14 automático); M13 = VALIDATED (documental na Seção 7.8 e executável em `notificacoes_m13.als`)**. M12 = VALIDATED (composição M12.1/M12.2; worklog `M12_CLOSURE.md`). O fechamento global após M13 é um gate, sem M14 automático. Os parágrafos abaixo preservam o contexto histórico da fase.

Camada formal ADITIVA CUE + Alloy + Rust → LaTeX. M5, M6, M7, M8, M9 e M10 possuem contratos CUE, IR v3, modelos Alloy, receipts verificáveis, validators Rust, fragmentos gerados e capítulos integrados ao PDF. M0–M11 = VALIDATED; M12.1 = VALIDATED (fonte na Seção 7.6); M12.2 = VALIDATED (fonte na Seção 7.7; executável); M12 = VALIDATED (composição M12.1/M12.2 provada em `composition_m12.als`; worklog `M12_CLOSURE.md`); M13 = VALIDATED (fonte na Seção 7.8; worklogs `M13_DOCUMENTATION.md` e `M13_EXECUTABLE_VALIDATION.md`). **Fechamento global M0–M13 = PASS** (`GLOBAL_FORMAL_CLOSURE.md`). Próxima ação: nenhuma; sem M14. Cancelamento explícito de convite pendente e transferência de ownership de Turma permanecem fora das fatias validadas e não entram em M12 por inércia (não são requisitos V1 efetivos).
...
| M12.2 | Roteiros, compartilhamento, Storage/download e associação a Post | VALIDATED |
| M12 | Fechamento de composição M12.1/M12.2 e regressão M0–M11 | VALIDATED |
| M13 | Notificação unificada (leitura multi-role, lida/Limpar tudo, expiração, alvo, privacidade, dedup V1) | VALIDATED |

## 9. Milestone atual
M4 VALIDATED (Retirada/devolução completas, com erratum pré-M5). M3 VALIDATED (com erratum pré-M5); M5 = VALIDATED; M6 = VALIDATED; M7 = VALIDATED; M8 = VALIDATED; M9 = VALIDATED; M10 = VALIDATED; M11 = VALIDATED; M12.1 = VALIDATED; M12.2 = VALIDATED; M12 = VALIDATED (composição M12.1/M12.2); M13 = VALIDATED (Seção 7.8; 34 checks UNSAT + 25 witnesses SAT; PDF 456 páginas). A implementação Firebase continua fora da evidência formal.
...
M10 permanece encerrado. A entrada em M11 — Turmas / demais domínios — foi avaliada e concluída em rodadas separadas (documental e executável): **M11 = VALIDATED**. M12.1 foi concluído em rodadas documental e executável (**M12.1 = VALIDATED**; worklogs `documentation/worklogs/formal-spec/M12_1_DOCUMENTATION.md` e `M12_1_EXECUTABLE_VALIDATION.md`). M12.2 foi concluído em rodadas documental e executável (**M12.2 = VALIDATED**; Seção 7.7; worklogs `documentation/worklogs/formal-spec/M12_2_DOCUMENTATION.md` e `M12_2_EXECUTABLE_VALIDATION.md`): CUE `#M12_2Contrato` (14 shapes; 18 fixtures válidas + 18 inválidas), IR v3 aditivo `formal_m12_2_roteiros`, `roteiros_m12_2.als` (47 checks UNSAT + 25 witnesses SAT = 72 resultados), receipt verificável `build/formal-validation-m12-2.json`, `validation_m12_2.rs` (33 testes Rust no total), guard `m12_2_contract.test.mjs` (36 testes Node no total), geração determinística e PDF de 435 páginas. A rodada realinhou a Seção 7.7 (URL já emitida vs novas emissões; limite transacional Firestore/Storage) e provou a geração no anexo como refinamento aditivo de M12.1, que permanece byte a byte idêntico. A rodada corretiva fechou dois contraexemplos (M12.2-CORR-01: só roteiro publicável é compartilhável; M12.2-CORR-02: Chefe só emite URL sob escopo Q13 registrado, com Chefe exclusivo sem vínculo de aluno), sem alterar IR, receipts M0–M12.1 nem M12.1. O fechamento M12 foi concluído nesta rodada em `M12_CLOSURE.md`: composição provada em `composition_m12.als` (55 resultados), receipt `build/formal-validation-m12.json`, `validation_m12.rs`, guard de drift, capítulo `Formal-Spec-M12.tex` e PDF de 442 páginas; itens 2a (publicabilidade explícita) e 2b (papel acadêmico × vínculo) fechados; regressão M0–M12.1 intacta. Próxima ação: M13 (Notificação unificada); o fechamento global após M13 é um gate, sem M14 automático. Registro executável de M10 em `documentation/worklogs/formal-spec/M10_EXECUTABLE_VALIDATION.md`; de M11 em `M11_EXECUTABLE_VALIDATION.md`; de M12.2 em `M12_2_EXECUTABLE_VALIDATION.md`.
