...
## 9. Milestone atual
M4 VALIDATED (Retirada/devolução completas, com erratum pré-M5). M3 VALIDATED (com erratum pré-M5); M5 = DOCUMENTATION_VALIDATED; M6 = DOCUMENTATION_VALIDATED. Registro completo em `M4_VALIDATION.md`, `PRE_M5_RECONCILIATION.md`, `M5_DOCUMENTATION.md` e `M6_DOCUMENTATION.md`.
...
INICIAR M7 DOCUMENTAL — Idempotência. M0–M4 = VALIDATED; M5 = DOCUMENTATION_VALIDATED; M6 = DOCUMENTATION_VALIDATED; M7 = NOT_STARTED. Não iniciar M7 nesta rodada.

M6 documental: contrato quantitativo consolidado em `documentation/worklogs/formal-spec/M6_DOCUMENTATION.md`; findings M6-F01/F02 resolvidos por HQ-M6-001 (sem correção administrativa metrológica; três rotas) e HQ-M6-002 (constraint de evaporação); UI-18 e estudos futuros na Seção 12. Nenhum arquivo executável ou formal alterado; PDF 330 páginas. A formalização executável pendente de M5/M6 permanece dívida futura. A implementação real (`functions/src/reagentes.ts`) segue divergente e não foi alterada.
