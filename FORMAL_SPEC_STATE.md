...
**M8 NÃO PODE INICIAR.** HQ-PRE-M8-001 e HQ-PRE-M8-002 RESOLVED: a modelagem agora separa `EstadoFisico`, `SituacaoLocalizacao` e autorização operacional. `EXTRAVIADO` preserva `fisico`, revoga autorização corrente e o reencontro grava `LOCALIZADO` com quarentena; a decisão anterior permanece no histórico. Checks Alloy de coerência são UNSAT e witnesses do ciclo físico/autorização são SAT nos scopes canônicos e ampliados. Regressão M0–M4 PASS. M5–M7 continuam DOCUMENTATION_VALIDATED enquanto o backfill executável não for concluído; M8 = NOT_STARTED. Próxima ação imediata: concluir o backfill executável M5.
...
M5 = VALIDATED; M6 = VALIDATED; M7 = VALIDATED; M8 = NOT_STARTED. A cadeia
executável CUE → IR → Alloy → receipts → Rust → LaTeX → PDF de M5–M7 foi
concluída e os gates finais passaram. A próxima ação exata é **INICIAR M8 DOCUMENTAL**.
Os parágrafos históricos abaixo preservam os estados anteriores à quitação.
