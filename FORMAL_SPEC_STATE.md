...
INICIAR M6 DOCUMENTAL — Q06 / tara / metrologia quantitativa. M0–M4 = VALIDATED (M3/M4 com erratum pré-M5); M5 = DOCUMENTATION_VALIDATED; M6 = NOT_STARTED. Não reabrir M2/M3/M4 e não formalizar M5 antes do ciclo documental: a formalização executável de M5 é dívida futura e o M6 documental é a próxima ação.

M5 documental: documentação normativa consolidada e corrigida (findings M5-F05..F08), PDF de 316 páginas, nenhum arquivo executável ou formal alterado. HQs M5 abertas = 0. A formalização executável pendente de M5 (CUE/Alloy/Rust/IR/receipt) permanece como **dívida futura**, não como próxima ação imediata: a estratégia adotada é fechar documentalmente os milestones até M12 antes de sincronizar implementação ou executar formalização executável. A implementação real (`functions/src/reagentes.ts`) ainda recalcula vencimento no reencontro e grava `medida_utilizada: 0`/`data_devolucao_efetuada` no extravio; permanece `DIVERGENCIA_IMPLEMENTACAO_FUTURA` e não foi alterada. O catálogo JSON segue especificado, não implementado.
