...
## 9. Milestone atual
M4 VALIDATED (Retirada/devolução completas, com erratum pré-M5). M3 VALIDATED (com erratum pré-M5); M5 NOT_STARTED. Registro completo em `documentation/worklogs/formal-spec/M4_VALIDATION.md` e na reconciliação `PRE_M5_RECONCILIATION.md`.
...
INICIAR M5 — Extravio/reencontro/quarentena. M3 = VALIDATED (com erratum pré-M5); M4 = VALIDATED (com erratum pré-M5); a reconciliação pré-M5 está registrada em `documentation/worklogs/formal-spec/PRE_M5_RECONCILIATION.md` e commitada no encadeamento `3213c48a`..`f23adc81`, sendo o HEAD final o commit documental seguinte (`f23adc81` + 1). Não reabrir M2 nem a validação histórica de M3/M4.
