...
FORMALIZAR M5 — Extravio/reencontro/quarentena em CUE/Alloy/Rust/IR/receipt, em rodada própria, a partir da documentação consolidada em `documentation/worklogs/formal-spec/M5_DOCUMENTATION.md`. M3/M4 = VALIDATED (com erratum pré-M5); M5 = DOCUMENTATION_VALIDATED. Não reabrir M2/M3/M4 nem iniciar M6.

M5 documental: documentação normativa consolidada, PDF de 316 páginas, nenhum arquivo executável ou formal alterado. HQs M5 abertas = 0. A implementação real (`functions/src/reagentes.ts`) ainda recalcula vencimento no reencontro e grava `medida_utilizada: 0`/`data_devolucao_efetuada` no extravio; permanece `DIVERGENCIA_IMPLEMENTACAO_FUTURA` e não foi alterada. O catálogo JSON segue especificado, não implementado.
