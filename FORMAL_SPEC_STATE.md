...
M0 = VALIDATED; M1 = VALIDATED; M2 = VALIDATED; M3 = VALIDATED; M4 = VALIDATED; M5 = VALIDATED; M6 = VALIDATED; M7 = VALIDATED; M8 = DOCUMENTATION_VALIDATED. A cadeia executável CUE → IR → Alloy → receipts → Rust → LaTeX → PDF de M5–M7 foi concluída com todos os gates PASS. M8 documental consolidou estoque atual, escassez e notificações; nenhuma HQ bloqueante. Próxima ação exata: **FORMALIZAÇÃO EXECUTÁVEL DE M8** (CUE → IR → Alloy → receipt → Rust → LaTeX → PDF), em rodada separada. Não iniciar M9. Os parágrafos abaixo preservam o contexto histórico da fase.

Camada formal ADITIVA CUE + Alloy + Rust → LaTeX. M5, M6 e M7 agora possuem contratos CUE, IR v3, modelos Alloy, receipts verificáveis, validators Rust, fragmentos gerados e capítulos integrados ao PDF. M8 = DOCUMENTATION_VALIDATED; a formalização executável de M8 é a próxima ação.
...
| M8 | Estoque/escassez/notificações | DOCUMENTATION_VALIDATED |
...
## 9. Milestone atual
M4 VALIDATED (Retirada/devolução completas, com erratum pré-M5). M3 VALIDATED (com erratum pré-M5); M5 = VALIDATED; M6 = VALIDATED; M7 = VALIDATED; M8 = DOCUMENTATION_VALIDATED. Registro completo em `M4_VALIDATION.md`, `PRE_M5_RECONCILIATION.md`, `M5_DOCUMENTATION.md`, `M6_DOCUMENTATION.md`, `M7_DOCUMENTATION.md`, `M8_DOCUMENTATION.md` e nos worklogs executáveis M5–M7.

- M8 = DOCUMENTATION_VALIDATED (entrada `4e142287`): consolidação documental de estoque atual, escassez e notificações. Estoque atual é `Frasco_Reagente` (sem view persistente; resumos `*_Diario` são FLOW histórico); agregação protegida `count()/sum()`; saldos `saldo_aferido_g`/`saldo_aferido_ml` server-owned e desconhecido ≠ zero. Predicado único `frascoAptoParaUso` (localizado, disponível, FECHADO/ABERTO, sem quarentena, validade compatível, sem pendência M5/M6) e `qtdAptos` em frascos. Escassez `qtdAptos < qtd_limiar_escassez` (estritamente menor) por configuração `Almoxarifado/{almox}/Estoques_Configurados/{idResumo_idEspec}`; `ativo`/`notificacao_ativa` distintos; backfill legado é pré-condição operacional. Cache `Sistema_Cache_Dashboard` lazy, TTL semântico < 30 s, geração/invalidação transacional, rate limit 5/min/UID antes do cache (inclusive cache hit); job de escassez não usa cache. Notificação `ESCASSEZ_ESTOQUE` idempotente por docId `escassez_{almox}_{config}_{dataCivil}` em `America/Sao_Paulo`, destinatários vinculados. Matriz de invalidação, UI-19 e casos de regressão adicionados. Findings M8-F01..F06 corrigidos documentalmente; M8-F07..F10 registrados como dívida de implementação; nenhuma HQ. Formalização executável permanece dívida futura. Próxima ação: formalização executável de M8 em rodada separada.
...
**M8 = DOCUMENTATION_VALIDATED.** M0–M7 = VALIDATED; o backfill executável M5–M7 foi concluído e a próxima ação é a **formalização executável de M8**, em rodada separada. A modelagem pré-M8 separa `EstadoFisico`, `SituacaoLocalizacao` e autorização operacional: `EXTRAVIADO` preserva `fisico`, revoga a autorização corrente e o reencontro grava `LOCALIZADO` com quarentena. Checks Alloy de coerência são UNSAT e witnesses do ciclo físico/autorização são SAT nos scopes canônicos e ampliados. Regressão M0–M4 PASS.

FORMALIZAÇÃO EXECUTÁVEL DE M8 (rodada separada): `CUE → IR → Alloy → receipt → Rust → LaTeX → PDF`. M0–M7 = VALIDATED; M8 = DOCUMENTATION_VALIDATED; M9+ = NOT_STARTED. Não iniciar M9 nesta linha. Registro documental em `documentation/worklogs/formal-spec/M8_DOCUMENTATION.md`.

M7 documental: contrato global de idempotência em `documentation/worklogs/formal-spec/M7_DOCUMENTATION.md`; identidade `(uid, tipo_operacao, payload_hash)`, canonicalização única, `idOperacao` obrigatório, comandos atômicos vs workflows externos, dedup de eventos, jobs e materializações; findings M7-F01..F09 corrigidos e M7-F10 registrado como divergência de implementação. A formalização executável CUE/Alloy/Rust de M5–M7 foi concluída no backfill pré-M8 (M5/M6/M7 = VALIDATED). M8 = DOCUMENTATION_VALIDATED. Registro: `documentation/worklogs/formal-spec/PRE_M8_FORMALIZATION_BACKFILL.md` e `documentation/worklogs/formal-spec/M8_DOCUMENTATION.md`. A implementação real (`functions/src/reagentes.ts`) segue divergente e não foi alterada.
