...
Camada formal ADITIVA CUE + Alloy + Rust → LaTeX. Rodada atual: M2.3 — proveniência / IR / geração. Escopo autorizado: `specification/cue/docs/`, `tools/spec-doc/`, `tools/formal/check.mjs`, `build/` e `documentation/generated/`. O CUE de domínio M2.1d e o Alloy M2.2 não foram alterados; `main.tex`/PDF não integram os fragmentos M2. M0/M1 continuam historicamente validados. A cadeia `CUE → IR → evidência Alloy → Rust → generated` agora conhece a projeção M2 completa (29 colunas) e a evidência M2.2, com proveniência estruturada; a composição M0 e M2.2 permanece para M2.4.
...
Planejar M2.4 (composição M0 e M2.2 e/ou integração LaTeX/PDF, conforme o roadmap) em tarefa separada, sem iniciá-lo automaticamente. Não solicitar novamente decisões HQ-M2-004..007.

M2.3 está VALIDATED: o IR v3 transporta a projeção `frasco_reagente_m2` (29 colunas) ao lado da fatia M0; a evidência M2.2 é validada pelo Rust; os fragmentos `entities/frasco_reagente_m2.tex` e `invariants/frasco_reagente_m2.tex` são gerados, mas NÃO integrados ao `main.tex`. A composição M0 e M2.2 permanece dívida de M2.4. Saldo corrente, custódia com pendência, quarentena, resolução metrológica, tara numérica, validade e FLOW permanecem fora do escopo provado. Não inferir prova desses contratos pelos gates históricos.
