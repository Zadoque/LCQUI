...
| M6 | Q06/tara | DOCUMENTATION_IN_PROGRESS |
...
## 9. Milestone atual
M4 VALIDATED (Retirada/devolução completas, com erratum pré-M5). M3 VALIDATED (com erratum pré-M5); M5 = DOCUMENTATION_VALIDATED; M6 = DOCUMENTATION_IN_PROGRESS. Registro completo em `M4_VALIDATION.md`, `PRE_M5_RECONCILIATION.md`, `M5_DOCUMENTATION.md` e `M6_DOCUMENTATION.md`.

- M6 = DOCUMENTATION_IN_PROGRESS (entrada `3d6ecc60`): contrato quantitativo consolidado. Q06 sobre `peso_saida` (normal $\max(1g,0{,}005)$; higroscópico $\max(2g,0{,}02)$; anomalia se retorno $>$ saída $+$ tolerância), nunca contra a tara; limiar de 5\,g eliminado. Vocabulário metrológico com autoridade única (`peso_retorno` imutável, `peso_retorno_efetivo` validado, `peso_atual` corrente, `medida_utilizada`/`consumo_validado` separando pendência de resultado). Consumo $=\max(0,\Delta_{bruto}-\text{evaporação})$; densidade histórica. Tara nas três situações; esgotamento por confirmação explícita; retorno abaixo de tara real versus referência teórica; matriz de ganho; `existePendenciaMetrologicaTx` definido no par `DEVOLVIDO_COM_ANOMALIA` + `consumo_validado=false`; quatro rotas tipadas; resolução não libera quarentena; `massa_perda_estimada_g` como estimativa de sinistro. Findings: M6-F03/F04/F05 corrigidos; **M6-F01 (conjunto concreto da 4ª rota administrativa) e M6-F02 (validação de `peso_perda_evaporacao` acima da perda bruta) são lacunas de domínio** → propostas HQ-M6-001/HQ-M6-002; PDF 327 páginas; nenhum arquivo executável/formal alterado. A formalização executável de M6 permanece dívida futura.
...
AUDITAR/RECONCILIAR OS FINDINGS DE M6 (HQ-M6-001 e HQ-M6-002) antes de iniciar o M7 documental. M0–M4 = VALIDATED; M5 = DOCUMENTATION_VALIDATED; M6 = DOCUMENTATION_IN_PROGRESS; M7 = NOT_STARTED. Não iniciar M7 nesta rodada.

M6 documental: contrato quantitativo consolidado em `documentation/worklogs/formal-spec/M6_DOCUMENTATION.md`. Findings M6-F03/F04/F05 corrigidos; M6-F01 (conjunto concreto da quarta rota administrativa metrológica) e M6-F02 (validação de `peso_perda_evaporacao` acima da perda bruta) são lacunas de domínio que exigem decisão humana. Nenhum arquivo executável ou formal alterado; PDF 327 páginas. A formalização executável pendente de M5/M6 permanece dívida futura. A implementação real (`functions/src/reagentes.ts`) segue divergente e não foi alterada.
