...
## 9. Milestone atual
M2 IN_PROGRESS: M2.0 VALIDATED; M2.1 VALIDATED; M2.1a VALIDATED; M2.1b DOCUMENTATION_VALIDATED; M2.1c VALIDATED; M2.1d VALIDATED; M2.2 VALIDATED; M2.3 VALIDATED; M2.4 NOT_STARTED.

Estado formal explícito após M2.3:
...
- M2.2 passou a carregar a projeção mínima de quarentena (`emQuarentena` e `descarteTecnicoAutorizado`) necessária para formalizar HQ-M2-008/B; M0 (`withdrawal.als`) permanece inalterado e a composição M0 e M2.2 NÃO é declarada provada (dívida de M2.4).
- M2.3 = VALIDATED (proveniência/IR/geração): IR v3 com projeção `frasco_reagente_m2` (29 colunas, derivada de `frascoCompletoCampos`) ao lado da fatia M0; proveniência estruturada (`baseline_historico_m0_m1` = `db29ea2f`, `baseline_documental_m2` = `9df335bc`) validada por Rust; `validation_m2` valida a evidência Alloy M2.2 (modelos, hashes, IDs, scopes, status); gerador emite `entities/frasco_reagente_m2.tex` e `invariants/frasco_reagente_m2.tex` e vincula `formal_validation_m2_sha256` no MANIFEST. Fragmentos M0/M1 byte a byte idênticos; geração determinística; stale/tampering rejeitados. Nenhuma HQ nova. Fragmentos M2 NÃO integrados ao `main.tex`.
...
- Reconciliação pré-M2.4 (HQ-M2-008/B, HQ-M2-009/A): `.tex` das Seções 4/5/7/8/9/10.4/10.5/10.8/11 reconciliados (quarentena → `PENDENTE_DE_DESCARTE` → descarte; resolução metrológica tipada; lock patrimonial; desvínculo individual). HQs M2 OPEN = 0. M2.2 reaberto e revalidado; M2.3 revalidado. CUE M2.1d não reaberto (29 colunas). Backend real continua dívida de M4/M5/M6/M9/M10.
- Dívida registrada: a composição de um modelo único M0 e M2.2 não é feita neste milestone; permanece para M2.4, se o roadmap assim reservar.
...
INICIAR M2.4 (composição M0 e M2.2 e/ou integração LaTeX/PDF, conforme o roadmap). Não solicitar novamente decisões HQ-M2-004..009.

HQs M2 OPEN = 0. A reconciliação pré-M2.4 aplicou HQ-M2-008/B (quarentena → `PENDENTE_DE_DESCARTE` → descarte, sem nova coluna) e HQ-M2-009/A (resolução metrológica tipada). M2.2 foi reconciliado e M2.3 revalidado. O CUE M2.1d permanece com 29 colunas. O IR v3 transporta a projeção `frasco_reagente_m2`; os fragmentos M2 são gerados, mas NÃO integrados ao `main.tex`. A composição M0 e M2.2 permanece dívida de M2.4. Backend real continua dívida de M4/M5/M6/M9/M10. Não inferir prova desses contratos pelos gates históricos.
