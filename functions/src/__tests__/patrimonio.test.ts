...
import {
  ...
  onResumoBemPatrimonialNomeAtualizado,
  onLocalAtualizado,
  limparLocksOrfaos
} from "../patrimonio";
...
describe("Patrimonio Module (40+ Tests)", () => {
  ...
  describe("criarRequisicaoEdicaoBem", () => {
    ...
    it("1. deve falhar se o lock determinístico já existir", async () => {
      ...
      await expect(criarRequisicaoEdicaoBem(reqBase)).rejects.toThrowError("Já existe uma requisição de edição pendente para este bem.");
    });

    it("2. deve criar lock e requisicao com dados mínimos", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      const mockSet = jest.fn();
      (admin.firestore().runTransaction as jest.Mock).mockImplementationOnce(async (cb) => {
        return cb({ get: mockGet, set: mockSet });
      });
      
      mockGet.mockResolvedValueOnce({ exists: false });
      
      const res: any = await criarRequisicaoEdicaoBem(reqBase);
    ...
    it("3. deve salvar os novos dados se forem enviados", async () => {
      ...
      await criarRequisicaoEdicaoBem(reqBase);
  ...
  describe("responderRequisicaoEdicaoBem", () => {
    ...
    it("4. deve falhar se requisicao nao existir", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      await expect(responderRequisicaoEdicaoBem(reqBase)).rejects.toThrowError("Requisição não encontrada.");
    });
    ...
    it("5. deve falhar se requisicao ja tiver sido respondida", async () => {
      ...
      await expect(responderRequisicaoEdicaoBem(reqBase)).rejects.toThrowError("Requisição já foi respondida.");
    });
    ...
    it("6. deve deletar o lock deterministico", async () => {
      ...
      await responderRequisicaoEdicaoBem(reqBase);
    ...
    it("8. aprovar requisição falha se o bem nao for achado", async () => {
      ...
      mockGet.mockResolvedValueOnce({ exists: true, data: () => ({ status: "pendente", id_bem_patrimonial: "bem_1" }) }); 
      mockGet.mockResolvedValueOnce({ exists: false }); 
      await expect(responderRequisicaoEdicaoBem(reqBase)).rejects.toThrowError("Bem patrimonial não encontrado.");
    });
  ...
  describe("criarRequisicaoAdicaoBem", () => {
    ...
    it("11. deve falhar se o lock determinístico da plaqueta já existir", async () => {
      ...
      (admin.firestore().collection("").where as jest.Mock).mockReturnValue({ limit: mockLimit });
      mockGet.mockResolvedValueOnce({ exists: true });
      await expect(criarRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("Já existe requisição pendente para este número de patrimônio.");
    });
    ...
    it("12. deve lançar falha se já houver bem com mesmo numero", async () => {
      ...
      await expect(criarRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("Já existe um bem cadastrado com este número");
    });
    ...
    it("13. deve criar lock e requisicao", async () => {
      ...
      const res: any = await criarRequisicaoAdicaoBem(reqBase);
      expect(res.idRequisicao).toBe("doc_id_mock");
    ...
    it("14. falha se idResumo e nomeResumoProposto estiverem nulos simultaneamente", async () => {
      ...
      await criarRequisicaoAdicaoBem(reqBase);
  ...
  describe("responderRequisicaoAdicaoBem", () => {
    ...
    it("15. deve falhar se requisicao nao existir", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      await expect(responderRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("Requisição não encontrada.");
    });

    it("16. deve falhar se requisicao ja respondida", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: true, data: () => ({ status: "rejeitada" }) });
      await expect(responderRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("Requisição já respondida.");
    });
    ...
    it("17. rejeitar deleta o lock e atualiza req", async () => {
      ...
      await responderRequisicaoAdicaoBem(reqBase);
      expect(mockDelete).toHaveBeenCalledTimes(1);
    ...
    it("18. aprovar falha se local não existir", async () => {
      ...
      await expect(responderRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("O local indicado não existe");
    });
    ...
    it("19. aprovar falha se resumo nao existe (se id_resumo fornecido)", async () => {
      ...
      await expect(responderRequisicaoAdicaoBem(reqBase)).rejects.toThrowError("Resumo de bem não encontrado");
    });
    ...
    it("20. aprovar com sucesso cria o resumo, cria o bem, deleta o lock e atualiza a requisicao", async () => {
      ...
      const res: any = await responderRequisicaoAdicaoBem(reqBase);
