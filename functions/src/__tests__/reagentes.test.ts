...
import {
  calcularValidadeEfetivaNaAbertura,
...
describe("Reagentes Module (40+ Tests)", () => {
  ...
  describe("calcularValidadeEfetivaNaAbertura", () => {
    it("1. deve retornar null se não houver validade fechado nem apos aberto", () => {
      const frasco = {};
      expect(calcularValidadeEfetivaNaAbertura(frasco, new Date())).toBeNull();
    });
    ...
    it("2. deve usar a validade_fechado se validade_apos_aberto não existir", () => {
      ...
      expect(calcularValidadeEfetivaNaAbertura(frasco, new Date())).toEqual(data);
    });
    ...
    it("3. deve usar a dataAbertura + validade_apos_aberto se validade_fechado não existir", () => {
      ...
      expect(calcularValidadeEfetivaNaAbertura(frasco, agora)).toEqual(esperado);
    });
    ...
    it("5. deve usar a validade_fechado se a validade_apos_aberto for maior", () => {
      ...
      expect(calcularValidadeEfetivaNaAbertura(frasco, agora)).toEqual(frasco.validade_fechado);
    });
  });
  ...
  describe("cadastrarFrascoFechado", () => {
    ...
    beforeEach(() => {
      const mockGet = admin.firestore().collection("dummy").doc("").get as jest.Mock;
      mockGet.mockReset();
      reqBase = {
    ...
    it("6. deve lançar not-found se especificação não existir", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      
      await expect(cadastrarFrascoFechado(reqBase))
        .rejects.toThrow(new HttpsError("not-found", "Especificação de reagente não encontrada."));
    });
    ...
    it("7. deve lançar failed-precondition se liquido não tiver densidade", async () => {
      ...
      await expect(cadastrarFrascoFechado(reqBase))
        .rejects.toThrow(new HttpsError("failed-precondition", "Líquido exige densidade positiva."));
    });
    ...
    it("8. deve calcular peso vazio corretamente para SÓLIDO (peso - nominal)", async () => {
      ...
      const res: any = await cadastrarFrascoFechado(reqBase);
    ...
    it("10. deve lançar invalid-argument se pesoVazio <= 0", async () => {
      ...
      await expect(cadastrarFrascoFechado(reqBase))
        .rejects.toThrow(new HttpsError("invalid-argument", "Peso total incompatível com o volume nominal para esta densidade."));
    });
    ...
    it("12. deve lançar failed-precondition se vencido e sem decisão", async () => {
      ...
      await expect(cadastrarFrascoFechado(reqBase))
        .rejects.toThrow(new HttpsError("failed-precondition", "O frasco está vencido no cadastro e exige uma decisão do gestor."));
    });
    ...
    it("13. deve lançar invalid-argument se Quarentena não tiver detalheStatus", async () => {
      ...
      await expect(cadastrarFrascoFechado(reqBase))
        .rejects.toThrow(new HttpsError("invalid-argument", "A entrada em quarentena exige detalhe_status."));
    });
  });
  ...
  describe("registrarRetirada", () => {
    ...
    it("14. deve lançar not-found se frasco não existir", async () => {
      ...
      await expect(registrarRetirada(reqBase)).rejects.toThrow(new HttpsError("not-found", "Frasco não encontrado."));
    });
    ...
    it("15. deve lançar failed-precondition se frasco indisponível", async () => {
      ...
      await expect(registrarRetirada(reqBase)).rejects.toThrowError("Frasco não está disponível para retirada.");
    });
    ...
    it("17. deve registrar a retirada com sucesso", async () => {
      ...
      const res: any = await registrarRetirada(reqBase);
  ...
  describe("registrarDevolucao", () => {
    ...
    it("21. deve lançar not-found se emprestimo não existir", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      await expect(registrarDevolucao(reqBase)).rejects.toThrowError("Empréstimo não encontrado.");
    });
    ...
    it("22. deve lançar falha se emprestimo ja devolvido", async () => {
      ...
      await expect(registrarDevolucao(reqBase)).rejects.toThrowError("Empréstimo já foi devolvido.");
    });
    ...
    it("23. deve aprovar devolução e consumo, se novo peso for igual ao da retirada", async () => {
      ...
      const res: any = await registrarDevolucao(reqBase);
    ...
    it("25. deve rejeitar devolução MAIOR que peso_retirada + tolerância", async () => {
      ...
      await expect(registrarDevolucao(reqBase)).rejects.toThrowError("excede 102% da massa de saída");
    });
  });
  ...
  describe("registrarAberturaFrasco", () => {
    ...
    it("26. deve falhar se frasco não for achado", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      await expect(registrarAberturaFrasco(reqBase)).rejects.toThrowError("Frasco não encontrado.");
    });
    ...
    it("27. deve falhar se o frasco ja estiver ABERTO", async () => {
      ...
      await expect(registrarAberturaFrasco(reqBase)).rejects.toThrowError("Somente um frasco FECHADO pode ser aberto");
    });
    ...
    it("28. deve abrir frasco FECHADO sem recalcular validade se validade_desconhecida = true", async () => {
      ...
      const res: any = await registrarAberturaFrasco(reqBase);
    ...
    it("30. deve bloquear abertura se frasco nao for DISPONIVEL", async () => {
      ...
      await expect(registrarAberturaFrasco(reqBase)).rejects.toThrowError("Somente frascos DISPONIVEL podem ser abertos.");
    });
  });
  ...
  describe("cadastrarFrascoAberto", () => {
    ...
    it("31. falha se especificação nao existe", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: false });
      await expect(cadastrarFrascoAberto(reqBase)).rejects.toThrowError("Especificação");
    });
    
    it("32. falha se liquido e sem densidade", async () => {
      const mockGet = admin.firestore().collection("").doc("").get as jest.Mock;
      mockGet.mockResolvedValueOnce({ exists: true, data: () => ({ estado_fisico: "LIQUIDO" }) });
      await expect(cadastrarFrascoAberto(reqBase)).rejects.toThrowError("Líquido exige densidade");
    });
    ...
    it("33. modalidade CONHECE_TARA solido ok", async () => {
      ...
      const res: any = await cadastrarFrascoAberto(reqBase);
    ...
    it("35. modalidade ESTIMA_VOLUME em SOLIDO lança erro", async () => {
      ...
      await expect(cadastrarFrascoAberto(reqBase)).rejects.toThrowError("Apenas LÍQUIDOS podem usar ESTIMA_VOLUME.");
    });
    ...
    it("36. modalidade ESTIMA_MASSA em LIQUIDO lança erro", async () => {
      ...
      await expect(cadastrarFrascoAberto(reqBase)).rejects.toThrowError("Apenas SÓLIDOS podem usar ESTIMA_MASSA.");
    });
    ...
    it("39. falha se validadeAberto estiver vencida e nao houver decisao", async () => {
      ...
      await expect(cadastrarFrascoAberto(reqBase)).rejects.toThrowError("exige uma decisão");
    });
