...
import {
  resolverPapeisDoToken,
  validarPermissao,
  validarGestorDoAlmoxarifado,
  atualizarCustomClaims
} from "../auth";
...
describe("Auth Module (40+ Tests)", () => {
  ...
  describe("resolverPapeisDoToken", () => {
    it("1. deve retornar [] se auth for undefined", () => {
      expect(resolverPapeisDoToken(undefined)).toEqual([]);
    });

    it("2. deve retornar [] se auth.token for vazio", () => {
      expect(resolverPapeisDoToken({ token: {} })).toEqual([]);
    });

    it("3. deve retornar [] se roles for null", () => {
      expect(resolverPapeisDoToken({ token: { roles: null } })).toEqual([]);
    });

    it("4. deve retornar [] se roles for undefined", () => {
      expect(resolverPapeisDoToken({ token: { roles: undefined } })).toEqual([]);
    });

    it("5. deve retornar [] se roles for uma string simples", () => {
      expect(resolverPapeisDoToken({ token: { roles: "Chefe_Geral" } })).toEqual([]);
    });

    it("6. deve retornar [] se roles for um número", () => {
      expect(resolverPapeisDoToken({ token: { roles: 123 } })).toEqual([]);
    });

    it("7. deve retornar [] se roles for um objeto", () => {
      expect(resolverPapeisDoToken({ token: { roles: { Chefe_Geral: true } } })).toEqual([]);
    });

    it("8. deve retornar os papeis corretos quando roles for array com 1 item", () => {
      expect(resolverPapeisDoToken({ token: { roles: ["Professor"] } })).toEqual(["Professor"]);
    });

    it("9. deve retornar os papeis corretos quando roles for array com múltiplos itens", () => {
      expect(resolverPapeisDoToken({ token: { roles: ["Chefe_Geral", "Professor"] } })).toEqual(["Chefe_Geral", "Professor"]);
    });

    it("10. deve lidar com array vazio", () => {
      expect(resolverPapeisDoToken({ token: { roles: [] } })).toEqual([]);
    });

    it("11. deve manter itens inválidos no array se existirem (typescript array as string[])", () => {
      // @ts-ignore
      expect(resolverPapeisDoToken({ token: { roles: [123, "Chefe_Geral"] } })).toEqual([123, "Chefe_Geral"]);
    });
  });

  describe("validarPermissao", () => {
    it("12. deve lançar unauthenticated se auth for undefined", () => {
      const req = { data: {}, rawRequest: {} as any };
      expect(() => validarPermissao(req, ["Professor"])).toThrow(new HttpsError("unauthenticated", "Usuário não autenticado."));
    });

    it("13. deve lançar unauthenticated se auth.uid não existir", () => {
      const req = { data: {}, auth: { token: {} }, rawRequest: {} as any } as any;
      expect(() => validarPermissao(req, ["Professor"])).toThrowError("Usuário não autenticado.");
    });

    it("14. deve lançar permission-denied se roles for ausente", () => {
      const req = { data: {}, auth: { uid: "123", token: {} }, rawRequest: {} as any } as any;
      expect(() => validarPermissao(req, ["Professor"])).toThrowError("Usuário sem papel autorizado para esta ação.");
    });

    it("15. deve lançar permission-denied se tiver papéis mas nenhum bater", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Aluno"] } }, rawRequest: {} as any } as any;
      expect(() => validarPermissao(req, ["Professor"])).toThrowError("Usuário sem papel");
    });

    it("16. deve lançar permission-denied se tiver múltiplos papéis mas nenhum bater", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Aluno", "Bolsista"] } }, rawRequest: {} as any } as any;
      expect(() => validarPermissao(req, ["Professor", "Chefe_Geral"])).toThrowError("Usuário sem papel");
    });

    it("17. deve passar se o usuário tiver exatamente o papel", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Professor"] } }, rawRequest: {} as any } as any;
      expect(validarPermissao(req, ["Professor"])).toEqual(["Professor"]);
    });

    it("18. deve passar se o usuário tiver um dos papéis permitidos", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Professor"] } }, rawRequest: {} as any } as any;
      expect(validarPermissao(req, ["Professor", "Chefe_Geral"])).toEqual(["Professor"]);
    });

    it("19. deve passar se o usuário tiver o papel Chefe_Geral", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Chefe_Geral"] } }, rawRequest: {} as any } as any;
      expect(validarPermissao(req, ["Professor", "Chefe_Geral"])).toEqual(["Chefe_Geral"]);
    });

    it("20. deve retornar todos os papéis do usuário", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Professor", "Gestor_Almoxarifado"] } }, rawRequest: {} as any } as any;
      expect(validarPermissao(req, ["Professor"])).toEqual(["Professor", "Gestor_Almoxarifado"]);
    });
    
    it("21. deve falhar se papeis permitidos for array vazio", () => {
      const req = { data: {}, auth: { uid: "123", token: { roles: ["Professor"] } }, rawRequest: {} as any } as any;
      expect(() => validarPermissao(req, [])).toThrowError("Usuário sem papel");
    });
  });
  ...
  describe("validarGestorDoAlmoxarifado", () => {
    ...
    beforeEach(() => {
      ...
      mockGet = admin.firestore().collection("dummy").where("").limit(1).get as jest.Mock;
    });

    it("22. deve passar se for Chefe_Geral (escopo global), não consulta firestore", async () => {
      await validarGestorDoAlmoxarifado("uid_123", { roles: ["Chefe_Geral"] }, "almox_1");
      expect(mockGet).not.toHaveBeenCalled();
    });

    it("23. deve passar se for Chefe_Geral e Gestor_Almoxarifado ao mesmo tempo (curto circuito no chefe)", async () => {
      await validarGestorDoAlmoxarifado("uid_123", { roles: ["Chefe_Geral", "Gestor_Almoxarifado"] }, "almox_1");
      expect(mockGet).not.toHaveBeenCalled();
    });

    it("24. deve falhar se não tiver papel de gestor ou chefe", async () => {
      await expect(validarGestorDoAlmoxarifado("uid_123", { roles: ["Professor"] }, "almox_1"))
        .rejects.toThrowError("Usuário não é Gestor de Almoxarifado.");
      expect(mockGet).not.toHaveBeenCalled();
    });

    it("25. deve falhar se os roles não estiverem presentes", async () => {
      await expect(validarGestorDoAlmoxarifado("uid_123", {}, "almox_1"))
        .rejects.toThrowError("Usuário não é Gestor de Almoxarifado.");
    });

    it("26. deve falhar se for um array vazio de roles", async () => {
      await expect(validarGestorDoAlmoxarifado("uid_123", { roles: [] }, "almox_1"))
        .rejects.toThrowError("Usuário não é Gestor de Almoxarifado.");
    });

    it("27. deve consultar o firestore se for Gestor_Almoxarifado", async () => {
      mockGet.mockResolvedValueOnce({ empty: false });
      await validarGestorDoAlmoxarifado("uid_123", { roles: ["Gestor_Almoxarifado"] }, "almox_1");
      expect(mockGet).toHaveBeenCalledTimes(1);
    });

    it("28. deve falhar se o vinculo no firestore estiver vazio", async () => {
      mockGet.mockResolvedValueOnce({ empty: true });
      await expect(validarGestorDoAlmoxarifado("uid_123", { roles: ["Gestor_Almoxarifado"] }, "almox_1"))
        .rejects.toThrow(new HttpsError("permission-denied", "Gestor não está designado para este almoxarifado."));
    });

    it("29. deve verificar os parâmetros passados para a consulta onde = uid e id", async () => {
      const mockWhere = admin.firestore().collection("dummy").where as jest.Mock;
      mockGet.mockResolvedValueOnce({ empty: false });
      await validarGestorDoAlmoxarifado("meu_uid", { roles: ["Gestor_Almoxarifado"] }, "meu_almox");
    ...
    it("30. deve rejeitar caso a query rejeite", async () => {
      ...
      await expect(validarGestorDoAlmoxarifado("meu_uid", { roles: ["Gestor_Almoxarifado"] }, "meu_almox"))
  ...
  describe("atualizarCustomClaims", () => {
    ...
    beforeEach(() => {
      ...
      mockSetCustomUserClaims = admin.auth().setCustomUserClaims as jest.Mock;
    ...
    it("38. deve consultar coleções específicas usando os nomes exatos do projeto", async () => {
      ...
      mockGet.mockResolvedValue({ exists: false });
      await atualizarCustomClaims("uid_teste");
