...
exports.validarRevogacaoChefeGeral = validarRevogacaoChefeGeral;
exports.validarRevogacaoGestorAlmoxarifado = validarRevogacaoGestorAlmoxarifado;
exports.validarRevogacaoGestorPatrimonial = validarRevogacaoGestorPatrimonial;
const admin = __importStar(require("firebase-admin"));
const https_1 = require("firebase-functions/v2/https");
/**
 * RN-ROLE-03: Chefe Geral não pode ser o único.
 */
async function validarRevogacaoChefeGeral(uid) {
    ...
    if (chefesSnapshot.size <= 1) {
        ...
        if (unicoChefe && unicoChefe.id === uid) {
            throw new https_1.HttpsError("failed-precondition", "Ação negada: Você é o único Chefe Geral ativo no sistema.");
        }
    }
}
/**
 ...
 */
async function validarRevogacaoGestorAlmoxarifado(uid) {
    const db = admin.firestore();
    ...
    for (const doc of vinculosSnapshot.docs) {
        ...
        if (gestoresDoAlmoxarifado.size <= 1) {
            throw new https_1.HttpsError("failed-precondition", `Ação negada: O usuário é o único gestor do almoxarifado ${idAlmoxarifado}.`);
        }
    }
}
/**
 ...
 */
async function validarRevogacaoGestorPatrimonial(uid) {
    const db = admin.firestore();
    ...
    if (gestoresSnapshot.size <= 1) {
        ...
        if (unicoGestor && unicoGestor.id === uid) {
            throw new https_1.HttpsError("failed-precondition", "Ação negada: O usuário é o único Gestor de Bens Patrimoniais ativo no sistema.");
        }
    }
}
//# sourceMappingURL=revogarPapel.js.map