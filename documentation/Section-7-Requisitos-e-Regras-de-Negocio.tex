...
Um ganho de massa acima dessa tolerância bloqueia a devolução por anomalia (indicando possível contaminação ou erro de balança). Retorno abaixo da tara por até 5 g é aceito na própria devolução quando \texttt{confirmarEsgotamento = true}: consome todo o saldo restante, passa a VAZIO/INDISPONIVEL e registra FICOU\_VAZIO com ajuste metrológico. Diferença superior a 5 g ou ausência da confirmação bloqueia a devolução ordinária e exige \texttt{recalibrarTaraFrascoEsgotado}.
\end{regra}
