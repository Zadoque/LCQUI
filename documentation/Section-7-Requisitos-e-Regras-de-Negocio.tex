...
reconciliados por rotina server-side (sem apagar objeto referenciado). Firestore
e Storage \textbf{não} têm transação única: a reconciliação de órfãos é
explícita, e a geração validada é a garantia contra troca de conteúdo.
\end{regra}
...
(mesmo com claim atualizada). Post removido da apresentação e roteiro revogado
não vazam por URL, cache, erro, notificação ou leitura direta. O download
retorna o mínimo necessário (URL curta, identidade do objeto e geração) sem
conteúdo protegido; se o objeto estiver ausente ou a geração divergir, falha
...
ou compartilhamento atual, estado da turma (Q08) e geração do objeto na mesma
transação em que escreve (M9); a validação de Storage é etapa externa
reconciliável e não substitui a decisão de domínio.
\end{regra}
