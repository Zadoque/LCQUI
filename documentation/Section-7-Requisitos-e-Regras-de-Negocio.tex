...
\texttt{Turma/\{id\}/Alunos/\{uid\}} (M11); o espelho
\texttt{Usuarios/\{uid\}/Turmas/\{id\}}, cache ou claim antigos nunca autorizam.
Somente o professor dono cria e edita Post da turma. Aluno/Bolsista com vínculo
canônico atual cria e edita o próprio Comentário e lê o feed. Ninguém edita
conteúdo alheio; moderação não altera autoria. O Chefe intervém apenas por Q13:
modera e retira da apresentação com motivo e auditoria, sem assumir autoria e
sem criar Post; a intervenção é identificada institucionalmente na publicação.
\end{regra}
...
Criar Comentário exige autor com vínculo canônico atual, Post existente na
turma Ativo, \texttt{texto} de 1 a 2\,000 caracteres (Q10) escapado na
renderização, sem HTML arbitrário, e \texttt{idOperacao} M7. Editar é exclusivo
do autor: gera \texttt{Historico\_Comentario} (texto antigo/novo, operador,
timestamp), marca \texttt{editado}/\texttt{editado\_em} e é idempotente por M7.
...
retry M7 não duplica evento nem histórico. As notificações
\texttt{POST}/\texttt{COMENTARIO} são efeitos delimitados em
\texttt{Usuarios/\{uid\}/Notificacoes}, com docId determinístico M7,
\texttt{id\_turma}/\texttt{id\_alvo} mínimos e \textbf{sem} conteúdo protegido
no payload; o clique revalida o acesso corrente e a notificação, por si só,
nunca autoriza a leitura do conteúdo. ``Limpar tudo'' marca como lida e não
apaga.
\end{regra}
...
aluno ou arquivamento antes do commit nega o commit posterior; retry do mesmo
comando devolve o receipt e não reaplica efeito, e reuso incompatível de
identidade de comando é rejeitado (M7).
\end{regra}

\paragraph{Casos de regressão M12.1.}
Cobrir: turma Ativo e Arquivada (escrita acadêmica negada); professor dono e
professor terceiro; aluno/Bolsista matriculado atual e removido; Chefe Q13
moderando e Chefe tentando escrever em turma arquivada; edição concorrente de
Post/Comentário com histórico preservado; comentário moderado visto por autor
(original marcado), colega (aviso), professor dono e Chefe (auditoria); replay
