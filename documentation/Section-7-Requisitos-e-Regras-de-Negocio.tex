...
\paragraph{Tipos com emissão V1 efetivamente especificada.}
Dos 20 valores do enum, apenas os abaixo têm emissão V1 definida; os demais são
valores de taxonomia compartilhada, \textbf{sem} obrigação de emissão automática
nesta versão:
\begin{itemize}
  \item \texttt{ESCASSEZ\_ESTOQUE}: job M8, docId diário, \texttt{expira\_em = NULL};
  \item \texttt{FRASCOS\_VENCIDOS}: job de vencimento (M8);
...
\end{itemize}
Os demais valores permanecem na taxonomia, sem emissão automática redesenhada e
\textbf{sem} se inferir uma obrigação a partir da enumeração:
\begin{itemize}
  \item \texttt{ADICIONADO}, \texttt{REMOVIDO}, \texttt{TURMA\_ARQUIVADA}, \texttt{TURMA\_DESARQUIVADA};
  \item \texttt{REQUISICAO\_BEM}, \texttt{REQUISICAO\_EDICAO\_BEM}, \texttt{REQUISICAO\_ADICAO\_BEM};
  \item \texttt{BEM\_INSERVIVEL}, \texttt{ENTREGA\_ATRASADA};
  \item \texttt{FRASCOS\_VAZIOS}, \texttt{FRASCOS\_QUEBRADOS}, \texttt{FRASCOS\_EM\_QUARENTENA};
  \item \texttt{FRASCOS\_A\_SEREM\_PESADOS};
  \item \texttt{AUTO\_ATENDIMENTO\_RETIRADA}.
\end{itemize}
...
RN-M13-04 alvo e deep link & Seção~5; M9; M11; M12.1; M12.2 & CUE \texttt{entidade\_alvo/id\_alvo}; Alloy ponte de acesso & alvo/ACL & Roteador/URL do frontend \\
RN-M13-05 privacidade & Seção~5; Seção~11; M12.1 & CUE payload mínimo; Alloy ausência de conteúdo & privacidade & Projeção real do backend \\
...
\noindent Assertions planejadas (nomes exatos no modelo executável M13):
\begin{itemize}
