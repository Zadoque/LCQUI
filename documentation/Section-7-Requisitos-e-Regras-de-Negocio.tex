...
No empréstimo, registra-se o \textit{peso\_saida}. Na devolução, registra-se o \textit{peso\_retorno}. Quando quantitativamente validado, o consumo é documentado na tabela \textit{Emprestimo\_Reagente} como:\\
...
\begin{itemize}
    ...
    \item \textbf{Reencontro.} Só um frasco \texttt{EXTRAVIADO} pode ser reencontrado. É um \textbf{novo fato físico}: não desfaz o extravio e não reabre empréstimo encerrado. O gestor informa o estado físico constatado (\texttt{ABERTO} ou \texttt{FECHADO}; \texttt{FECHADO} só é aceito se nunca houve abertura conhecida) e o frasco passa a \texttt{em\_quarentena = TRUE} e \texttt{disponibilidade = INDISPONIVEL}. \texttt{saldo}, \texttt{saldo\_desconhecido}, \texttt{peso\_atual}, tara e \texttt{vencido} permanecem; uma medição válida posterior é a única forma de alterar o saldo. Se o frasco foi encontrado \texttt{ABERTO} sem data de abertura determinável, marca-se \texttt{abertura\_historica\_desconhecida = TRUE}, \texttt{data\_abertura = NULL}, \texttt{validade\_desconhecida = TRUE} e \texttt{validade\_efetiva = NULL}; nunca se inventa a data.
