...
O usuário informa o Peso Total (frasco + reagente) medido na balança e o Conteúdo Nominal do rótulo (volume em mL para líquidos, massa em g para sólidos). O sistema calcula e salva o peso do recipiente vazio:\\
\begin{itemize}
...
O usuário informa o Peso do Frasco Vazio e o Peso Total atual na balança. O sistema calcula o conteúdo atual:\\
\begin{itemize}
    \item Reagente Líquido ($mL$): $Volume_{atual} = \frac{Peso_{total} - Peso_{vazio}}{Densidade}$
    \item Reagente Sólido ($g$): $Massa_{atual} = Peso_{total} - Peso_{vazio}$
\end{itemize}

\textbf{3. Cadastro de Frasco já Aberto (Opção B - Conhece o conteúdo atual):}\\
O usuário informa o conteúdo atual visual (volume em mL para líquidos, massa em g para sólidos) e o Peso Total na balança. O sistema calcula o peso do frasco vazio para uso futuro:\\
\begin{itemize}
    \item Reagente Líquido ($mL$): $Peso_{vazio} = Peso_{total} - (Volume_{atual} \times Densidade)$
    \item Reagente Sólido ($g$): $Peso_{vazio} = Peso_{total} - Massa_{atual}$
\end{itemize}
...
Retorno abaixo da tara em até 5 g é aceito na própria devolução quando \texttt{confirmarEsgotamento = true}: consome todo o saldo restante, passa a VAZIO/INDISPONIVEL e registra FICOU\_VAZIO com ajuste metrológico. O esgotamento homologa a tara real do recipiente: o sistema \textbf{atualiza} \texttt{peso\_frasco\_vazio} e \texttt{peso\_atual} com o peso aferido na devolução, eliminando o resíduo metrológico. Diferença superior a 5 g ou ausência da confirmação bloqueia a devolução ordinária e exige recalibração formal, salvo quando acompanhada de \texttt{recalibrarTaraParcial = true} com \texttt{novoPesoVazioEstimado} e \texttt{motivoRecalibracao} --- caso em que o frasco permanece ABERTO, a nova tara estimada é persistida e o evento é auditado.
\end{regra}
...
Frasco & FECHADO & conteudo\_nominal obrigat\'orio ($> 0$), peso\_total obrigat\'orio ($> 0$); c\'alculo compuls\'orio de peso\_frasco\_vazio na inser\c{c}\~ao (tara nula rejeitada).\\
