...
a vaga, e o código de turma nunca autoriza exceção. Um ingresso excepcional
válido pode deixar \texttt{qtd\_alunos > capacidade}: esse estado é permitido e
não torna a turma inválida. Enquanto a ocupação for maior ou igual à capacidade,
o ingresso ordinário permanece bloqueado até surgir vaga; nenhum membro é
expulso e \texttt{qtd\_alunos} não é reduzido para satisfazer o limite.

Edição de capacidade (HQ-M11-001 = A): é proibido \textbf{editar/reduzir}
\texttt{capacidade} para valor menor que a ocupação já existente
(\texttt{qtd\_alunos}). O servidor rejeita, fail-closed, com erro recuperável e
orienta remover membros antes ou manter a capacidade. A proibição recai sobre a
edição do campo, não sobre o resultado de um ingresso excepcional válido: a
turma pode ter \texttt{qtd\_alunos > capacidade} sem ser inválida.
\texttt{qtd\_alunos} nunca é negativo e pode ser reconciliado com os vínculos
canônicos.
\end{regra}
...
Identidade e histórico: o ID do convite é imutável e distinto da chave de
pendência. O docId determinístico por HMAC identifica a \emph{pendência}
(e-mail normalizado, turma|GLOBAL): enquanto há convite pendente, ele é a chave
de unicidade; aceitação ou expiração liberam a pendência. Um novo convite para a
mesma dupla após a terminalidade é um \textbf{novo documento}, com novo ID,
token e hash, e não sobrescreve nem apaga o convite aceito/expirado, preservando
\texttt{aceitado\_por}, \texttt{aceitado\_em}, \texttt{status} e a auditoria
histórica. Reenvio de convite ainda pendente substitui hash e expiração no mesmo
documento; criação nova após terminalidade gera outro documento; retry da mesma
operação devolve o resultado persistido. O hash de token e o e-mail em claro
nunca são expostos pela chave.
...
Fronteira Auth, Firestore e e-mail: a aceitação é executada por um solicitante
já autenticado (\texttt{request.auth.uid} presente) com e-mail verificado
correspondente ao convite; nesse fluxo a conta Firebase Auth do solicitante
\textbf{já existe} (cadastro ou login prévio). A criação condicional pertence ao
Firestore, não ao Auth: \texttt{Usuarios}, \texttt{Aluno}, papel e vínculo e o
consumo do convite ocorrem na mesma transação Firestore. O envio de e-mail do
convite e a eventual sincronização de Custom Claims são etapas externas,
pós-commit, recuperáveis e idempotentes; nenhuma chamada externa ocorre dentro da
transação. Um provisionamento Auth fora do fluxo de aceitação (por exemplo,
convite de outro papel criado pela chefia) é operação distinta do solicitante e
não é pressuposto desta aceitação. Falha parcial não pode criar conta, papel,
vínculo ou espelho divergente nem consumir o convite sem os efeitos
correspondentes. Criar o registro do convite não é enviar o e-mail.
\end{regra}
...
ausência de vínculo ativo. A aceitação é única e idempotente (M7): retry do
mesmo ator, com o mesmo token e a mesma operação devolve o receipt persistido,
sem recriar vínculo nem incrementar \texttt{qtd\_alunos}; tentativa distinta, de
outro ator, com token diferente ou sobre convite já consumido é negação
específica e recuperável, não replay. Convite expirado, de e-mail divergente ou
de turma arquivada também é negado com erro específico. Aluno removido
anteriormente só reingressa por convite explícito, nunca pelo código; o fato de
remoção é lido do histórico canônico \texttt{Historico\_Alunos\_Turma}
(\texttt{exclusao\_aluno}) na própria transação, jamais de espelho
...
(e-mail, turma) e para o global; replay de aceite pelo mesmo ator e tentativa
distinta de outro ator/token; remoção concorrente e dupla; arquivamento com
ocupação, desarquivamento e escrita em turma arquivada; convite global sem
matrícula; ingresso de removido por código (negado) e por convite (válido);
exceção de capacidade com e sem justificativa; \texttt{qtd\_alunos > capacidade}
alcançado por exceção válida (estado válido) versus edição/redução de capacidade
abaixo da ocupação (rejeitada, HQ-M11-001 = A); novo convite após terminalidade
sem sobrescrever o aceito/expirado; contador nunca negativo; espelho ou cache
...
% M12.1 -- fonte normativa de Posts, Comentarios, edicao/moderacao e historicos
\subsection{Posts, comentários, edição, moderação e históricos (M12.1)}
\label{sec:regras-posts-comentarios-m12-1}

Esta subseção é a fonte normativa de Post, Comentário, seus históricos de
edição/moderação e as leituras acadêmicas associadas (RF19, RF20, UI-11). A
criação/edição de Post com roteiro é \textbf{fronteira com M12.2}: valida-se o
acesso atual ao roteiro e preserva-se o snapshot \texttt{roteiro\_anexo}, sem
formalizar upload, compartilhamento, emissão de URL ou a ACL completa de
Roteiros. A caixa de notificações (UI-12) e seus efeitos completos ficam fora
de M12.1; contrata-se aqui apenas a emissão \texttt{POST}/\texttt{COMENTARIO}
como efeito delimitado. A autorização continua regida por M9, a idempotência
por M7 e o vínculo de participação por M11 (Seção~\ref{sec:regras-turmas-m11}).

\begin{regra}[Identidade, autoria e propriedade]
O Post mora em \texttt{Turma/\{id\_turma\}/Posts/\{postId\}} e tem autor
\texttt{id\_professor}; o Comentário mora em
\texttt{.../Comentarios/\{id\}} e tem autor \texttt{id\_usuario}. A autoridade
de participação acadêmica é o vínculo canônico atual
\texttt{Turma/\{id\}/Alunos/\{uid\}} (M11) \textbf{ou} o professor dono da turma;
o espelho \texttt{Usuarios/\{uid\}/Turmas/\{id\}}, cache ou claim antigos nunca
autorizam. Claim, token ou espelho atualizados \textbf{não recriam
participação}: um ex-aluno removido não edita nem lê o próprio Comentário,
ainda que a claim esteja corrente. Somente o professor dono cria e edita Post
da turma. Aluno/Bolsista com vínculo canônico atual, ou o professor dono, cria
e edita o próprio Comentário e lê o feed. Ninguém edita conteúdo alheio;
moderação não altera autoria. O Chefe intervém apenas por Q13: modera e retira
da apresentação com motivo e auditoria, sem assumir autoria e sem criar Post; a
intervenção é identificada institucionalmente na publicação.
\end{regra}

\begin{regra}[Estado da turma (Q08)]
Turma \texttt{Arquivada} é somente leitura para toda escrita acadêmica: criar,
editar, remover ou moderar Post e criar, editar ou moderar Comentário são
negados, \textbf{sem exceção administrativa}. O Chefe, inclusive sob Q13, não
escreve em turma arquivada; nenhuma intervenção de moderação é deduzida como
exceção não documentada.
\end{regra}

\begin{regra}[Criação, edição e remoção de Post]
Criar Post exige professor dono, turma Ativo, \texttt{titulo} de 1 a 150 e
\texttt{descricao} de 1 a 10\,000 caracteres (Q10), roteiro opcional validado na
fronteira M12.2 e \texttt{idOperacao} M7. Editar Post é do professor dono:
preserva a versão anterior em \texttt{Historico\_Posts\_Turma}
(antigo/novo, operador, motivo quando houver, timestamp), marca
\texttt{editado}/\texttt{editado\_em} e é idempotente por M7; a concorrência é
resolvida pela reexecução da transação, sem sobrescrita silenciosa e com a
versão anterior preservada no histórico. Remover da apresentação é
\textbf{soft}: exige motivo, operador e timestamp, não apaga o documento nem o
histórico (RF25), oculta o item do feed de colegas e o mantém acessível ao
professor dono, ao Chefe Q13 e ao histórico. Não existe exclusão física de Post
para simular moderação.
\end{regra}

\begin{regra}[Criação, edição e moderação de Comentário]
Criar Comentário exige autor com vínculo canônico atual ou professor dono (M11),
Post existente na turma Ativo, \texttt{texto} de 1 a 2\,000 caracteres (Q10)
escapado na renderização, sem HTML arbitrário, e \texttt{idOperacao} M7. Editar
é exclusivo do autor e exige o mesmo vínculo/ownership no commit: gera
\texttt{Historico\_Comentario} (texto antigo/novo, operador, timestamp), marca
\texttt{editado}/\texttt{editado\_em} e é idempotente por M7. Após a remoção do
aluno, o autor não edita nem lê o próprio Comentário, ainda que a claim seja
atualizada; somente vínculo canônico atual ou ownership recreiam participação.
Moderar é do professor dono ou do Chefe por Q13: grava atomicamente
\texttt{moderado = true}, \texttt{motivo\_moderacao} (1 a 2\,000 caracteres),
\texttt{moderado\_por} e \texttt{moderado\_em}; \textbf{não} apaga o
\texttt{texto} original nem o histórico. O autor vê o original marcado; colegas
veem o aviso institucional de ocultação; professor dono e Chefe veem original e
histórico para auditoria. A edição posterior pelo autor \textbf{não} desfaz a
moderação: o estado moderado persiste até nova decisão explícita de moderação,
fail-closed.
\end{regra}

\begin{regra}[Leitura, máscara e endpoints]
A leitura direta da coleção \texttt{Comentarios} e de seu histórico é negada ao
cliente (Seção~\ref{sec:acesso-detalhado}); \texttt{listarComentariosPost}
autorizado revalida vínculo canônico atual, estado da turma e existência do
Post, e devolve máscara institucional a colegas e original ao autor; o
professor dono e o Chefe recebem original e histórico de auditoria. Nenhum
cache, notificação, log, mensagem de erro ou projeção pode devolver o original
a terceiros. O feed (\texttt{listarFeed}) é escopado à Turma, paginado por
data e ID com mais recentes primeiro; item removido da apresentação não aparece
para colegas, mas permanece no histórico do professor dono e da chefia.
\end{regra}

\begin{regra}[Histórico, auditoria e notificações]
Edição e moderação geram evento histórico imutável com operador, motivo e
timestamp; intervenção Q13 é registrada em \texttt{Registro\_de\_Auditoria}; o
retry M7 não duplica evento nem histórico. Cada mutação consome exatamente um
\texttt{idOperacao} e grava o receipt na mesma transação; o retry idêntico
(mesmo ator, tipo e payload canônico) devolve o resultado persistido e não
reaplica efeito. As notificações \texttt{POST}/\texttt{COMENTARIO} são efeitos
delimitados em \texttt{Usuarios/\{uid\}/Notificacoes}, com docId determinístico
M7 vinculado ao \texttt{idOperacao}, \texttt{id\_turma}/\texttt{id\_alvo}
mínimos e \textbf{sem} conteúdo protegido no payload; o clique revalida o
acesso corrente e a notificação, por si só, nunca autoriza a leitura do
conteúdo. ``Limpar tudo'' marca como lida e não apaga.
\end{regra}

\begin{regra}[Fronteira com M12.2 (roteiro anexo)]
O Post guarda o snapshot imutável \texttt{roteiro\_anexo} (id do roteiro, nome
do arquivo, tamanho e \texttt{storage\_path}); publicar com roteiro exige
validar o acesso atual do professor ao roteiro antes de persistir o snapshot.
O download é mediado por endpoint autorizado que revalida o vínculo atual e
emite URL temporária, sem leitura cliente da coleção raiz. Upload,
compartilhamento, revogação e a ACL completa de Roteiros são formalizados em
M12.2; um Post cujo anexo não possa ser validado com segurança não é publicado.
\end{regra}

\begin{regra}[Composição M7/M9]
Toda mutação lê autorização, vínculo canônico atual e estado da turma na mesma
transação (M9): professor dono para Post, autor para o próprio Comentário,
professor dono ou Chefe Q13 para moderação. Revogação de vínculo, remoção de
aluno ou arquivamento antes do commit nega o commit posterior. Primeira
execução grava o receipt sob o \texttt{idOperacao}; retry idêntico (mesmo ator,
tipo e payload canônico) devolve o resultado persistido e não reaplica efeito;
reuso do mesmo \texttt{idOperacao} com ator, tipo ou payload divergente é
rejeitado e não herda receipt alheio (M7). A identidade de participação é
revalidada na transação: claim atualizada sem vínculo/ownership não edita nem
lê (M9/M11).
\end{regra}

\paragraph{Casos de regressão M12.1.}
Cobrir: turma Ativo e Arquivada (escrita acadêmica negada, incluindo edição e
moderação e para o Chefe); professor dono e professor terceiro; aluno/Bolsista
matriculado atual e removido (claim atualizada não recria edição nem leitura);
Chefe Q13 moderando e Chefe tentando escrever em turma arquivada; edição
concorrente de Post/Comentário com histórico preservado; primeira execução com
receipt, retry idêntico sem reaplicar e reuso incompatível rejeitado; comentário
moderado visto por autor (original marcado), colega (aviso), professor dono e
Chefe (auditoria); replay
de criação/edição/moderação; Post com e sem roteiro (fronteira M12.2);
notificação apontando para conteúdo sem acesso; leitura direta negada e
endpoint devolvendo máscara. Esses casos são critérios normativos e não atestam
a implementação.

% M12.2 -- fonte normativa de Roteiros, compartilhamento, Storage e anexo a Post
\subsection{Roteiros, compartilhamento, upload/download e anexo a Post (M12.2)}
\label{sec:regras-roteiros-m12-2}

Esta subseção é a fonte normativa de \textit{Roteiro\_Experimento},
compartilhamento entre professores, upload/Storage, visualização/download e
associação opcional a Post (RF21, RF22, RF23, UI-11). Ela fecha a fronteira
abstrata \texttt{acessoRoteiroValidado}/\texttt{roteiro\_anexo} de M12.1 (Seção
7.6) sem reabrir sua prova executável. A autorização segue M9, a idempotência
segue M7, a participação acadêmica segue M11 e a decisão \textbf{Q09}
(revogação de roteiro anexado a Post) está consolidada em
\texttt{MODIFICACOES\_CONSOLIDADAS\_LCQUI.md} e é incorporada aqui.

\begin{regra}[Identidade, autoria e referência do arquivo]
\texttt{Roteiro\_Experimento/\{id\}} tem dono imutável
\texttt{id\_professor\_upload}, \texttt{nome} (1--150), \texttt{descricao} e a
referência canônica ao objeto no Storage: \texttt{storage\_path},
\texttt{content\_type}, \texttt{tamanho\_bytes}, \texttt{owner\_uid},
\texttt{geracao} e \texttt{criado\_em}. \texttt{file\_url} é referência legada:
não é autoridade, não é credencial e não deve ser persistida como URL pública
permanente. O download usa URL temporária de validade curta, emitida por
endpoint autorizado; uma URL já emitida não pode ser revogada antes de expirar,
o que é uma limitação registrada e não uma autorização permanente. Somente o
proprietário altera ou compartilha o roteiro; a autoria nunca é transferida.
\end{regra}

\begin{regra}[Upload seguro e publicabilidade]
O upload exige professor ativo e é validado no servidor (App Check, Auth, papel
e escopo); o arquivo é PDF estritamente menor que 15 MiB, com bytes
\texttt{\%PDF-} verificados, no bucket/caminho autorizado, com titularidade,
geração e ausência de sobrescrita entre a validação e o vínculo. Um Roteiro só
se torna \textbf{publicável} (visível em ``Meus'' e elegível a
compartilhamento/anexo) depois que o objeto é validado e o registro recebe a
geração validada; o upload provisório não é publicável. Cancelamento ou falha
não cria Roteiro publicável; objetos órfãos de tentativas falhas são
reconciliados por rotina server-side (sem apagar objeto referenciado).

Firestore e Storage \textbf{não} têm transação única. A geração validada é
\textbf{persistida no documento do Roteiro no Firestore} e é lida e comparada
\textbf{dentro da transação do Firestore} que altera a associação (criar o
Roteiro, compartilhar/revogar, anexar/desvincular a Post); a inspeção do objeto
(existência, bytes \texttt{\%PDF-}, tamanho) ocorre \textbf{fora} dessa
transação, no Storage, como etapa externa. O Firestore \textbf{não} lê nem
trava a geração real do Storage de forma atômica: não existe transação única
entre os dois serviços. Por isso o objeto vinculado e a geração são
\textbf{imutáveis} e não podem ser sobrescritos depois da validação, e a
validação binária antecede o vínculo. Há uma \textbf{janela residual} entre
validar o objeto no Storage e gravar o vínculo no Firestore; a obrigação é
impedir a sobrescrita do objeto vinculado nessa janela e reconciliar
falhas/órfãos de forma explícita, sem afirmar atomicidade entre serviços.
\end{regra}

\begin{regra}[Compartilhamento e revogação (Q09)]
Somente o proprietário autoriza ou revoga um professor destinatário ativo; há
no máximo uma relação por \texttt{(id\_roteiro, id\_professor)}, representada na
3FN por \texttt{Roteiro\_Professor\_Compartilhado} e no Firestore pelo array
\texttt{professores\_compartilhados}, gerido exclusivamente pelo servidor
(\texttt{arrayUnion}/\texttt{arrayRemove}). Repetir o compartilhamento é
idempotente (M7): não duplica vínculo, não gera efeito duplicado. O
destinatário pode anexar o roteiro a uma turma própria e baixá-lo enquanto o
compartilhamento estiver vigente, mas \textbf{não} redistribui, não altera a
autoria e não coopera em nome do dono. A notificação
\texttt{ROTEIRO\_COMPARTILHADO} é efeito mínimo delimitado (destinatário,
roteiro, autor); o ciclo completo e a caixa ficam em M13. Revogação (Q09):
impede novas associações do roteiro pelo destinatário, mas \textbf{preserva os
Posts históricos} já publicados e seus snapshots; não recalcula o passado.
\end{regra}

\begin{regra}[Leitura e download]
A biblioteca tem abas ``Meus'', ``Compartilhados comigo'' e ``Eu compartilhei'',
com leitura mínima (nome, autor, descrição, ações permitidas). O servidor
revalida: para professor, propriedade ou compartilhamento atual; para aluno, o
\textbf{vínculo canônico atual} à turma e um Post acessível que referencia o
roteiro. Turma \texttt{Arquivada} é somente leitura: membros com vínculo atual
ainda leem/baixam o anexo de Post histórico, mas um \textbf{ex-aluno} não
(mesmo com claim atualizada). A remoção do Post da apresentação, a perda do
vínculo (inclusive ex-aluno), a revogação do compartilhamento e o objeto
ausente ou de geração divergente impedem \textbf{novas emissões} de URL; nenhum
cache, erro, notificação ou leitura direta devolve o conteúdo por um caminho
novo. Uma URL temporária \textbf{já emitida} antes da remoção, da perda de
vínculo ou da revogação permanece utilizável até o seu prazo curto expirar: a
revogação do token já entregue \textbf{não} é instantânea nem retroativa, e o
endpoint não promete invalidá-lo. Cada nova emissão revalida, no momento,
propriedade, compartilhamento atual, vínculo canônico atual e Post acessível; o
controle vale para URLs novas e não é afrouxado pelo prazo da URL anterior. O
download retorna o mínimo necessário (URL curta, identidade do objeto e geração)
sem conteúdo protegido; se o objeto estiver ausente ou a geração divergir, falha
fechado, sem fallback para \texttt{file\_url}. O Chefe acessa por Q13 apenas no
escopo de \textbf{moderação/auditoria}: a intervenção administrativa excepcional
sobre um Post que referencia o roteiro, registrada em
\texttt{Registro\_de\_Auditoria} (Q13). Sem esse vínculo concreto e ativo entre o
Chefe, o roteiro e o Post sob intervenção, o Chefe \textbf{não} emite URL nova;
um booleano livre ou o papel isolado não autorizam, e o fim/revogação do escopo
impede apenas novas emissões, preservando as URLs já emitidas até expirarem. O
escopo Q13 é exclusivo de quem possui o papel de Chefe (RN-ROLE-01, que proíbe
acumular \emph{papéis}). Como M9/M11 não eliminam atomicamente o vínculo
canônico de aluno ao mudar o papel, a rota acadêmica de download exige, além do
vínculo canônico atual, o \textbf{papel acadêmico autorizado}; um Chefe que
reteve vínculo legado não baixa por essa rota e não contorna Q13.
\end{regra}

\begin{regra}[Elegibilidade para compartilhamento]
Só um Roteiro \textbf{publicável} --- objeto validado e geração fixada --- é
elegível a compartilhamento e anexo; um Roteiro \texttt{PROVISORIO} ou
\texttt{VALIDADO} não pode ser compartilhado, e um compartilhamento existente
nunca aponta para Roteiro não publicável. Repetir o compartilhamento de um
Roteiro publicável permanece idempotente (M7).
\end{regra}

\begin{regra}[Associação com Post]
No máximo um roteiro opcional por Post. O professor dono da turma precisa de
\texttt{acessoRoteiroValidado} no commit, \textbf{inclusive na edição} que
mantém ou troca o anexo; remover o anexo não exige acesso. O
\texttt{roteiro\_anexo} é snapshot \textbf{imutável} de metadados e geração
(\texttt{id\_roteiro}, \texttt{nome\_arquivo}, \texttt{tamanho\_bytes},
\texttt{storage\_path}, \texttt{geracao}); ele preserva o histórico e rotula a
publicação, mas \textbf{não} é autorização futura — a leitura revalida o acesso
atual. Editar/desvincular atualiza o snapshot e registra a versão anterior no
histórico de M12.1 (id antigo/novo), sem apagar o objeto anterior (retenção
RF25; objeto vira candidato a reconciliação se ficar sem referência). Se o
compartilhamento foi revogado, a edição que mantém o anexo falha fechado (o
professor deve remover o anexo ou obter novo acesso); o Post existente e o
snapshot permanecem para leitura do aluno com vínculo atual.

M12.2 \textbf{refina aditivamente} a fronteira abstrata
\texttt{roteiro\_anexo}/\texttt{acessoRoteiroValidado} de M12.1 (Seção~7.6), sem
reabrir a prova executável de M12.1. O contrato M12.1 apenas declara o encaixe do
anexo e \textbf{não} prova a geração do snapshot; a presença de \texttt{geracao}
no anexo e sua ligação à referência canônica do Roteiro
(\texttt{storage\_path}/\texttt{geracao}/\texttt{tamanho\_bytes}) são provadas
pelo contrato executável M12.2 como extensão aditiva do contrato M12.1.
\end{regra}

\begin{regra}[Composição M7/M9]
Upload/cadastro, compartilhar, revogar, associar a Post e desvincular usam
\texttt{idOperacao} e receipt M7: retry idêntico devolve o resultado e não
duplica Roteiro, vínculo de compartilhamento, histórico nem notificação; reuso
incompatível é rejeitado. Toda mutação lê usuário ativo, papel/versão, ownership
ou compartilhamento atual, estado da turma (Q08) e geração \textbf{persistida no
documento} do Roteiro (\texttt{geracao}) na mesma transação em que escreve (M9);
a inspeção do objeto no Storage é etapa externa reconciliável, ocorre fora da
transação do Firestore e não substitui a decisão de domínio nem promete
atomicidade entre serviços.
\end{regra}

\paragraph{Matriz de operações M12.2.}
\begin{itemize}
	\item \textbf{Cadastrar roteiro}: ator professor ativo; pré: PDF $<$15 MiB
	validado, caminho/titularidade/geração; efeito: Roteiro publicável; falha:
	objeto órfão reconciliável, sem registro utilizável.
	\item \textbf{Compartilhar/revogar}: ator proprietário; pré: roteiro
	\textbf{publicável}, destinatário professor ativo, relação única; efeito: ACL
	atualizada (idempotente) + notificação mínima; falha: nenhuma ACL parcial.
	\item \textbf{Listar biblioteca}: ator proprietário/destinatário atual;
	efeito: projeção mínima; revogado/ex-aluno não veem.
	\item \textbf{Baixar}: professor com propriedade/compartilhamento atual, ou
	aluno com vínculo atual e Post acessível; Chefe por Q13 somente sob escopo de
	moderação/auditoria registrado sobre Post que referencia o roteiro; efeito: URL curta
	com geração; falha fechado se objeto/geração ausente; ex-aluno/arquivada
	segundo a regra.
	\item \textbf{Anexar/editar/desvincular Post}: ator professor dono; pré:
	acesso atual ao roteiro no commit (exceto remoção do anexo), turma Ativo;
	efeito: snapshot imutável + histórico M12.1; falha: sem fato parcial.
	\item \textbf{Retry M7}: devolve receipt; não duplica efeito.
\end{itemize}

\paragraph{Casos de regressão M12.2.}
Cobrir: proprietário, destinatário compartilhado, professor terceiro, aluno
matriculado atual, ex-aluno, Chefe Q13; turma Ativo e Arquivada; upload válido,
PDF inválido, maior que 15 MiB, objeto ausente/geração divergente; cancelamento
e órfão; compartilhar/revogar idempotentes; download por proprietário,
destinatário, aluno e negado a terceiro/ex-aluno; revogação com Post histórico
preservado e com edição que mantém o anexo (falha fechado); associação,
substituição e desvinculação de anexo com histórico; Post removido e roteiro
revogado sem vazamento; retry e reuso incompatível. Esses casos são critérios
normativos e não atestam a implementação. A formalização executável de M12.2 é
rodada separada.
...
% M13 -- fonte normativa da Notificacao unificada (caixa, leitura, alvo e dedup)
\subsection{Notificação unificada (M13)}
\label{sec:regras-notificacoes-m13}

Esta subseção é a fonte normativa da entidade única \textit{Notificacao} e da
caixa (UI-12). Ela compõe a idempotência M7
(Seção~\ref{sec:idempotencia-m7}), a emissão de escassez e o docId diário de M8
(Seção~\ref{sec:m8-estoque-escassez-notificacoes}), a autorização corrente M9
(Seção~\ref{sec:m9-autorizacao}), a participação acadêmica M11
(Seção~\ref{sec:regras-turmas-m11}) e os efeitos delimitados de M12.1/M12.2
(Seções~\ref{sec:regras-posts-comentarios-m12-1} e
\ref{sec:regras-roteiros-m12-2}) sem reabrir suas provas. A implementação
Firebase (\texttt{functions/}, \texttt{frontend/}, Rules) permanece fora desta
rodada; a evidência é estrutural (CUE) e relacional/temporal abstrata (Alloy).

\begin{regra}[Identidade, caixa única e leitura (RN-M13-01)]
A notificação é a entidade única \textit{Notificacao} da 3FN, implementada em
\texttt{Usuarios/\{uid\}/Notificacoes/\{id\}}. O \texttt{uid} do caminho e o
campo \texttt{id\_destinatario} correspondem ao destinatário autenticado: existe
\textbf{um único sino/caixa por conta} (UID), reunindo todos os
\texttt{papel\_destinatario} de que a pessoa participa. O papel selecionado no
header é apenas \textbf{contexto de apresentação}; ele não filtra
silenciosamente a caixa própria nem concede privilégio algum. O campo
\texttt{papel\_destinatario} é contexto de exibição e \textbf{nunca} uma
concessão de papel. A caixa inclui \texttt{Bolsista} (Q12) e
\texttt{Chefe\_Geral}; o Bolsista agrega alertas de empréstimo independentemente
do papel visual corrente. Nenhum usuário lê a caixa de outro UID.
\end{regra}

\begin{regra}[Mutação, marcação e ``Limpar tudo'' (RN-M13-02)]
Criação, marcação individual de leitura e ``Limpar tudo'' são
\textbf{server-owned}; o cliente apenas lê os próprios documentos, sujeitos à
proteção de conteúdo. Vale \texttt{lida = false} $\Rightarrow$
\texttt{lida\_em = null}; ao marcar, \texttt{lida = true} e \texttt{lida\_em}
recebe o instante do servidor. Repetir a marcação é idempotente: não altera um
instante lógico já gravado nem cria novo fato. ``Limpar tudo'' marca como lida
toda notificação ativa elegível daquele UID, inclusive de outros
\texttt{papel\_destinatario}, por processamento \textbf{paginado e
reentrante}; não executa \texttt{DELETE}, não apaga o histórico e não age na
caixa de terceiro. Cada rodada encerra em um \textbf{corte estável} dos itens
existentes no início; avisos emitidos após esse corte permanecem disponíveis
para a próxima rodada. Isso é uma regra de execução do contrato de paginação,
não uma promessa de transação ilimitada nem de varredura atômica de toda a
coleção. Falha parcial ou retry retoma do corte, sem marcar um item novo por
engano.
\end{regra}

\begin{regra}[Vida temporal e expiração (RN-M13-03)]
\texttt{expira\_em = null} significa \textbf{ausência de expiração temporal
automática}; é o caso de \texttt{ESCASSEZ\_ESTOQUE}, alerta operacional
persistente. Quando \texttt{expira\_em} é alcançado, o aviso sai do conjunto
\textbf{ativo} sem apagar o fato (o documento permanece para o histórico).
Aviso expirado ou já lido \textbf{nunca} confere autorização ao objeto de
origem: ler ou expirar não cria acesso. Não se atribui duração arbitrária aos
demais tipos; usam-se apenas prazos determinados na fonte. Em particular,
\texttt{DATA\_DEVOLUCAO\_REAGENTE} é preventivo e cobre a janela
``amanhã~$\rightarrow$~hoje'' com docId \texttt{\{id\_emprestimo\}--\{janela\}}
(\texttt{VENCE\_AMANHA}/\texttt{VENCE\_HOJE}); um empréstimo que muda de janela
cria um segundo documento e o anterior não é removido, apenas expirado.
\end{regra}

\begin{regra}[Alvo, deep link e revalidação (RN-M13-04)]
\texttt{entidade\_alvo} pertence a uma lista fechada de rotas conhecidas e
\texttt{id\_alvo} é o identificador de um recurso, \textbf{nunca} uma URL
arbitrária. Abrir o aviso relê a autorização \textbf{corrente} de M9 e o
contrato do domínio: vínculo canônico atual em Turma/Post (M11/M12.1), estado de
remoção/moderação, propriedade/compartilhamento de Roteiro e geração (M12.2) e
escopo Q13 do Chefe. Revogação, remoção da apresentação, perda de vínculo ou
objeto inacessível produzem \textbf{resposta neutra}, sem texto protegido; a
notificação \textbf{não cria} participação nem acesso. Uma URL de Storage já
emitida mantém o limite temporal curto da Seção~\ref{sec:regras-roteiros-m12-2}
e \textbf{não é revogada retroativamente}; o aviso em si jamais é uma URL ou
credencial.
\end{regra}

\begin{regra}[Privacidade do payload (RN-M13-05)]
O payload é mínimo: identificadores e tipo. Não carrega o corpo original de
comentário moderado, conteúdo de Post protegido, PDF, e-mail sensível, token nem
URL assinada. Listagem, resumo, cache, mensagem de erro ou contador do sino
\textbf{não} revela conteúdo de recurso que o usuário deixou de poder acessar. A
leitura direta dos próprios documentos é compatível com eventual projeção segura
no backend; declara-se explicitamente que \textbf{Firestore Rules autorizam ou
negam documentos completos e não mascaram campos} de um documento. A máscara de
comentário moderado (M12.1) continua sendo responsabilidade do endpoint
autorizado, nunca do registro de notificação.
\end{regra}

\begin{regra}[Emissão e deduplicação (RN-M13-06)]
A emissão reutiliza os contratos já provados: M7 para efeitos por
\texttt{idOperacao}/\texttt{evento} (incluindo \texttt{POST}/\texttt{COMENTARIO}
de M12.1), M12.2 para \texttt{ROTEIRO\_COMPARTILHADO} e a prova/docId diário de
M8 para \texttt{ESCASSEZ\_ESTOQUE} (destinatário vinculado ao almoxarifado e
\texttt{expira\_em = null}; M8 \textbf{não} é reconstruído aqui).
\texttt{DATA\_DEVOLUCAO\_REAGENTE} conserva o docId
\texttt{\{id\_emprestimo\}--\{janela\}}; os contratos de patrimônio e demais
alertas permanecem como definidos. Retry da mesma identidade não cria aviso
extra; a deduplicação é \textbf{por destinatário}, de modo que a mesma operação
pode ter fan-out para destinatários distintos sem notificar o mesmo UID duas
vezes. \textbf{Não} existe fórmula universal de docId nem obrigação de emissão
automática para todos os 20 valores do enum: apenas os tipos com emissão V1
efetivamente especificada devem ser emitidos. Somente a duplicidade esperada
(\texttt{ALREADY\_EXISTS}/``code 6'') é no-op; \textbf{erros de outra natureza
não são engolidos} e devem propagar.
\end{regra}

\begin{regra}[Condições específicas de contexto (RN-M13-07)]
\texttt{id\_turma} é \textbf{obrigatório} para os tipos acadêmicos
(\texttt{COMENTARIO}, \texttt{POST}, \texttt{ADICIONADO}, \texttt{REMOVIDO},
\texttt{TURMA\_ARQUIVADA}, \texttt{TURMA\_DESARQUIVADA}) e \textbf{nulo} para os
avisos operacionais de almoxarifado/patrimônio. O contexto de Roteiro é
preservado sem lhe atribuir \texttt{id\_turma} apenas por existir. O enum 3FN
(Seção~4) e o dicionário Firestore (Seção~5) coincidem nos 20 valores, sem
renomear por conveniência.
\end{regra}

\begin{explicacao}[Reconciliação da nullabilidade de \texttt{id\_turma}]
A nota antiga da Seção~4, que listava como exceção ``\textit{id\_turma} da
notificação de professor quando o tipo não é \texttt{COMENTARIO}'', é
\textbf{reconciliada}: a regra acadêmica posterior (RN-M13-07) torna
\texttt{id\_turma} obrigatório para todos os tipos acadêmicos listados e nulo
para os operacionais. A regra posterior prevalece; a redação antiga é preservada
apenas como histórico.
\end{explicacao}

\begin{regra}[Projeção do anexo M12.2 (RN-M13-08)]
O snapshot \texttt{roteiro\_anexo} do Post inclui a \texttt{geracao} validada
(\texttt{id\_roteiro}, \texttt{nome\_arquivo}, \texttt{tamanho\_bytes},
\texttt{storage\_path}, \texttt{geracao}), conforme a referência canônica da
Seção~\ref{sec:regras-roteiros-m12-2}. Esta é uma emenda textual mínima ao
dicionário da Seção~5, sem alterar a semântica provada de M12.1 nem receipts
antigos.
\end{regra}

\paragraph{Obrigações V1 de emissão por tipo.}
Os 20 valores do enum não implicam obrigação automática de emissão, mas várias
fontes exigem emissões na V1. A tabela de reconciliação completa (fonte de
obrigação, evento/destinatário, cobertura estrutural M13, prova de emissão
existente e fronteira ainda não provada) está no worklog
\texttt{documentation/worklogs/formal-spec/M13\_EXECUTABLE\_VALIDATION.md}. São
obrigatórias na V1, ainda que sua prova de emissão não esteja formalizada:
\begin{itemize}
  \item \texttt{POST} e \texttt{COMENTARIO}: efeitos M12.1 vinculados ao \texttt{idOperacao};
  \item \texttt{ROTEIRO\_COMPARTILHADO}: efeito M12.2;
  \item \texttt{ESCASSEZ\_ESTOQUE}: job M8, docId diário, \texttt{expira\_em = NULL};
  \item \texttt{FRASCOS\_VENCIDOS}: job de vencimento (Seção~10.7), gestores vinculados;
  \item \texttt{DATA\_DEVOLUCAO\_REAGENTE}: job preventivo, docId \texttt{\{id\_emprestimo\}--\{janela\}};
  \item \texttt{ENTREGA\_ATRASADA}: job de atraso (Seção~10.7), gestores vinculados;
  \item \texttt{REQUISICAO\_BEM}, \texttt{REQUISICAO\_ADICAO\_BEM} e \texttt{REQUISICAO\_EDICAO\_BEM}: resposta ao Professor e aviso aos gestores na abertura (Seção~4/7.8);
  \item \texttt{TURMA\_ARQUIVADA} e \texttt{TURMA\_DESARQUIVADA}: notificação aos membros ao arquivar/desarquivar (Seções~8--9);
  \item \texttt{AUTO\_ATENDIMENTO\_RETIRADA}: notificação à chefia (Q14).
\end{itemize}
Estar fora da prova de emissão M13 \textbf{não} significa estar fora da V1. Os
valores a seguir permanecem taxonomia compartilhada \textbf{sem} obrigação de
emissão automaticamente decidida:
\begin{itemize}
  \item \texttt{ADICIONADO}, \texttt{REMOVIDO};
  \item \texttt{BEM\_INSERVIVEL};
  \item \texttt{FRASCOS\_VAZIOS}, \texttt{FRASCOS\_QUEBRADOS};
  \item \texttt{FRASCOS\_A\_SEREM\_PESADOS};
  \item \texttt{FRASCOS\_EM\_QUARENTENA}.
\end{itemize}
Para eles a fonte não decide destinatário, frequência, prazo nem chave de dedup,
e essa lacuna não é convertida em prova nem em dispensa funcional.

\paragraph{Matriz de rastreabilidade M13.}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{>{\raggedright\arraybackslash}p{0.14\textwidth} >{\raggedright\arraybackslash}p{0.18\textwidth} >{\raggedright\arraybackslash}p{0.17\textwidth} >{\raggedright\arraybackslash}p{0.21\textwidth} >{\raggedright\arraybackslash}p{0.16\textwidth}}
\toprule
\textbf{Regra} & \textbf{Fonte} & \textbf{Contrato} & \textbf{Prova planejada} & \textbf{Fronteira} \\
\midrule
\endfirsthead
\toprule
\textbf{Regra} & \textbf{Fonte} & \textbf{Contrato} & \textbf{Prova planejada} & \textbf{Fronteira} \\
...
\bottomrule
\endfoot
RN-M13-01 caixa única & Seção~4; Seção~5; UI-12 & CUE \texttt{\#M13Contrato}; Alloy caixa e papel & caixa/papel (ver abaixo) & Papel visual real do frontend \\
RN-M13-02 marcação e Limpar tudo & Seção~4; Seção~5; UI-12; M8 & CUE \texttt{lida/lida\_em}; Alloy transições de lote & lote/idempotência & Falha parcial real, paginação real \\
RN-M13-03 expiração & Seção~4; Seção~5; regra de professor & CUE \texttt{expira\_em}; Alloy ativo/expirado & temporal & Relógio real America/Sao\_Paulo \\
RN-M13-04 alvo, rotas e Q13 & Seção~5; M9; M11; M12.1; M12.2 & CUE \texttt{entidade\_alvo/id\_alvo}; Alloy rotas e ponte & alvo/rotas/Q13 & Roteador/URL do frontend \\
RN-M13-05 privacidade e máscara & Seção~5; Seção~11; M12.1 & CUE payload mínimo; Alloy visão do comentário & privacidade/projeção & Projeção real do backend \\
RN-M13-06 emissão/dedup & M7; M8; M12.1; M12.2 & CUE comando/alvo; Alloy chaves & dedup/chaves & Erros reais do Firestore/Admin SDK \\
RN-M13-07 contexto & Seção~4; Seção~5; regra de professor & CUE \texttt{id\_turma} condicional & contexto/turma & Dados legados sem contexto \\
RN-M13-08 anexo com geração & Seção~7.7; M12.2 & CUE \texttt{geracao} no snapshot & geração do anexo & Objeto real no Storage \\
\bottomrule
\end{longtable}
\normalsize

\noindent Assertions do modelo executável M13 (nomes exatos):
\begin{itemize}
  \item RN-M13-01: \texttt{NaoLeCaixaAlheia}, \newline \texttt{PapelVisualNaoFiltraCaixa}, \newline \texttt{BolsistaVeCaixa};
  \item RN-M13-02: \texttt{MarcarSoProprias}, \newline \texttt{MarcaIdempotente}, \newline \texttt{MarcaNaoReverte}, \newline \texttt{LoteSoProprias}, \newline \texttt{SemDelete}, \newline \texttt{LimparTudoCorteEstavel};
  \item RN-M13-03: \texttt{ExpiracaoDistingueNull}, \newline \texttt{ExpiradoForaDoAtivo};
  \item RN-M13-04: \texttt{AlvoInvalidoNaoNavega}, \newline \texttt{AlertaNaoContornaAcl}, \newline \texttt{RotaAcademicaExigePapel}, \newline \texttt{ChefeLegadoNaoUsaAcademico}, \newline \texttt{SemVinculoNaoRestaura}, \newline \texttt{ProfessorDonoMantemAcesso}, \newline \texttt{CompartilhamentoSoProfessor}, \newline \texttt{ChefeAdminNaoHerdaAcademico}, \newline \texttt{ChefeSemEscopoNaoEmiteUrl}, \newline \texttt{EscopoEncerradoImpedeNovaEmissao}, \newline \texttt{EncerramentoEscopoPreservaRoteiro}, \newline \texttt{RoteiroAlunoExigePostEAnexo}, \newline \texttt{PonteRotaChefeRoteiroRefinaUrl};
  \item RN-M13-05: \texttt{NotificacaoNaoExpoeOriginal}, \newline \texttt{ColegaVeAviso}, \newline \texttt{AutorVeOriginal}, \newline \texttt{AuditorVeOriginal};
  \item RN-M13-06: \texttt{EmissaoMesmaIdentidadeNaoDuplica}, \newline \texttt{FanOutNaoDuplicaDestinatario}, \newline \texttt{ComposicaoM7RetryNaoDuplica}, \newline \texttt{EmissaoChaveDistintaEmite}, \newline \texttt{ErroEmissaoNaoViraSucesso};
  \item RN-M13-07: \texttt{IdTurmaAcademicoObrigatorio}, \newline \texttt{IdTurmaOperacionalNulo};
  \item RN-M13-08: reuso da prova M12.2 (\texttt{AnexoCompostoUsaGeracaoCanonica}).
\end{itemize}

\paragraph{Casos de regressão M13.}
Cobrir: leitura da caixa própria e negação da caixa alheia; usuário multi-role
com papel visual diferente do papel da notificação (inclusive Bolsista por Q12);
marcação individual e ``Limpar tudo'' idempotentes, sem \texttt{DELETE},
inclusive de notificações de outros papéis; corte estável de ``Limpar tudo''
com nova emissão concorrente após o corte; falha parcial e retry de ``Limpar
tudo''; \texttt{expira\_em = null} versus vencido; aviso expirado ou lido que não
autoriza o objeto; revogação de vínculo/compartilhamento após a emissão (corta
novas emissões, preserva o fato e a URL já emitida até expirar); alvo removido ou
inacessível com resposta neutra; \texttt{entidade\_alvo} inválida ou URL
arbitrária que não navega; Chefe sem escopo Q13 que não acessa; colisão de dedup
(mesma identidade M7 ou chave M8 não duplica, chaves distintas podem emitir);
erro de emissão distinto de ``code 6'' que propaga. Esses casos são critérios
normativos e \textbf{não} atestam a implementação.

\paragraph{Fronteira da prova.}
A formalização executável M13 (CUE $\rightarrow$ IR $\rightarrow$ Alloy
$\rightarrow$ receipt $\rightarrow$ Rust $\rightarrow$ LaTeX) verifica as
propriedades relacionais e temporais abstratas acima no escopo declarado. Ela
\textbf{não} certifica \texttt{functions/}, \texttt{frontend/}, Firestore/
Storage Rules, Auth, Storage real, entrega de e-mail, relógio de produção,
concorrência sob carga, paginação real do Firestore nem atomicidade entre
serviços; esses pontos permanecem como dívida de implementação e só a
implementação real poderá exercitá-los.
