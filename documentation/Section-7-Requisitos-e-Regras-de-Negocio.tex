...
\end{itemize}
O frasco inicia com \texttt{condicao\_inicial\_cadastro = FECHADO}, \texttt{estado\_fisico\_frasco = FECHADO} e \texttt{saldo\_desconhecido = false}.
...
Um retorno com peso abaixo da tara real conhecida é tratado como \textbf{anomalia metrológica} e não como definição de esgotamento; a diferença (por exemplo, até 5\,g) serve apenas para sinalizar o caso e exigir confirmação e justificativa humana, nunca para inferir automaticamente que o frasco ficou vazio. O esgotamento depende exclusivamente da confirmação explícita do gestor. Não existe recalibração parcial de tara por estimativa: recalibrar tara exige recipiente efetivamente vazio e peso vazio real medido, com motivo humano conforme a lista fechada de justificativas e registro histórico.
\end{regra}
...
\begin{itemize}
    ...
    \item A ação de descarte institucional (\texttt{descartarFrasco}) é permitida para: (1) frascos com estado físico \texttt{VAZIO}, (2) frascos \texttt{QUEBRADO}, e (3) frascos com \texttt{vencido = true} e \texttt{uso\_vencido\_autorizado = false} (situação de \textit{PENDENTE\_DE\_DESCARTE}), mesmo que ainda possuam conteúdo químico remanescente nos estados \texttt{ABERTO} ou \texttt{FECHADO}, formalizando a baixa do passivo químico perigoso. O sistema impõe que o frasco não esteja \texttt{disponibilidade = EMPRESTADO} no momento do descarte.
\end{itemize}
\end{regra}
...
Frasco & FECHADO & conteudo\_nominal obrigat\'orio ($> 0$), peso\_total obrigat\'orio ($> 0$); tara te\'orica derivada na inser\c{c}\~ao, sem car\'ater de medi\'c{c}\~ao f\'isica definitiva.\\
