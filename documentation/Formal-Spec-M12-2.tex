...
um Roteiro \texttt{PROVISORIO} ou \texttt{VALIDADO} não pode ser compartilhado
(\texttt{CompartilhamentoSoDePublicavel}, \texttt{NaoCompartilhaNaoPublicavel},
e testemunhas alcançáveis \texttt{WitnessProvisorioNaoCompartilha} e
\texttt{WitnessValidadoNaoCompartilha}).
...
Chefe, \textbf{somente sob escopo Q13 registrado} (intervenção de
moderação/auditoria sobre um Post que referencia o roteiro, com Chefe exclusivo e
sem vínculo de aluno), de modo que um Chefe ativo e um roteiro publicável
\textbf{sem} caso de auditoria não emitem
(\texttt{ChefeSemEscopoNaoEmite}, \texttt{EscopoQ13SoDeChefeComPostReferenciado});
um \textbf{ex-aluno} é negado ainda que a claim esteja atualizada
...
\texttt{WitnessUrlAtivaAposRevogacao}), sem prometer revogação retroativa. O
\textbf{fim do escopo Q13} também impede novas emissões do Chefe, preservando
apenas as URLs já emitidas (\texttt{EncerrarEscopoImpedeNovaEmissao},
\texttt{UrlEmitidaSobreviveAoEncerramentoEscopo},
\texttt{WitnessEncerrarEscopoImpedeEmissao}). A geração da URL vinculada é a
geração canônica.
...
revogação Q09, turma arquivada, ex-aluno, Post removido, anexo/histórico,
elegibilidade de compartilhamento (só publicável), escopo Q13 do Chefe e
...
ligação do anexo à referência canônica, emissão/uso de URL, aluno/ex-aluno,
escopo Q13 e unicidade de UID. A evidência \textbf{não} certifica \texttt{functions/},
frontend, Firestore/Storage Rules, Auth, Storage real, bytes binários,
atomicidade entre serviços nem concorrência.

\begingroup\small\raggedright
\input{generated/entities/formal_m12_2_roteiros.tex}
\input{generated/invariants/formal_m12_2.tex}
\par\endgroup
