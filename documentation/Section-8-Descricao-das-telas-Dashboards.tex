...
Para frasco em quarentena, a tela de decisão exige motivo/laudo humano (mínimo de 20 caracteres após trim) e oferece ``VOLTAR A DISPONIVEL'' (quando as demais regras permitirem) ou ``PENDENTE DE DESCARTE''. A operação é transacional, relê o estado atual, valida que o frasco ainda está em quarentena e o escopo, persiste uma única decisão e registra histórico; uma segunda operação concorrente é reavaliada sobre o novo estado e rejeitada se a pré-condição desapareceu. A saída da quarentena não altera \texttt{vencido} e não oferece ``revalidar validade'': essa operação não existe.
