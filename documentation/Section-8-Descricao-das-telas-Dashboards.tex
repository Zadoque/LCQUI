...
\begin{itemize}
    \item \textbf{Notificações:} Botão com ícone de sino, presente globalmente no header (N-11) e consolidado por conta ativa (UID), independentemente do papel selecionado no momento. Abre um modal \textit{dropdown} contendo todos os alertas direcionados àquele usuário: requisições de bens respondidas (Professor), reagentes com prazo atrasado ou vencendo (Professor/Bolsista), e alertas de escassez (Gestores). Cada notificação exibe visualmente o escopo/papel a que se destina. Inclui opção de marcar como lida e um botão ``Limpar tudo''.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Novo Reagente -> Abre modal exigindo dados de resumo e especificação separados conforme UI-05; CAS e concentração têm obrigatoriedade condicional, não universal. Campos: Nome, CAS, Concentração, grau de pureza (texto opcional), fórmula química, se é controlado pela pf ou pela eb, a classe de inflamabilidade, link da ficha de segurança, tipo (sólido ou líquido: na etapa de Especificação pede densidade, pergunta se requer pesagem frequente, se sim, pede a frequencia em dias).
