...
\begin{regra}[M9 --- UI não é controle de segurança]
Botões, rotas protegidas, seleção de papel, cache de claims e estados de
carregamento servem somente à UX. A tela deve ocultar ações incompatíveis,
recarregar token após aviso de alteração e mostrar negação do servidor sem
vazar dados; ela não conclui autorização. Resultado de toda mutação e toda
leitura sensível depende da Seção~7/M9 e da validação server-side/Rules. Uma
mudança de papel, vínculo, conta ativa ou ownership durante a sessão invalida a
ação seguinte, mesmo que o botão ainda esteja visível.
\end{regra}
...
\begin{itemize}
    \item \textbf{Notificações:} Botão com ícone de sino, presente globalmente no header (N-11) e consolidado por conta ativa (UID), independentemente do papel selecionado no momento. Abre um modal \textit{dropdown} contendo todos os alertas direcionados àquele usuário: requisições de bens respondidas (Professor, REQUISICAO\_BEM), solicitações novas (Gestores Patrimoniais, REQUISICAO\_ADICAO\_BEM ou REQUISICAO\_EDICAO\_BEM), reagentes com prazo atrasado ou vencendo (Professor/Bolsista), e alertas de escassez (Gestores). Cada notificação exibe visualmente o escopo/papel a que se destina. Inclui opção de marcar como lida e um botão ``Limpar tudo''.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Novo Reagente -> Abre modal exigindo dados de resumo e especificação separados conforme UI-05; CAS e concentração têm obrigatoriedade condicional, não universal. Campos: Nome, CAS, Concentração, grau de pureza (texto opcional), fórmula química, se é controlado pela pf ou pela eb, a classe de inflamabilidade, link da ficha de segurança, tipo (sólido ou líquido): na etapa de Resumo pede se requer pesagem frequente e, se sim, a frequência em dias; na etapa de Especificação pede densidade, obrigatória para líquidos.
        ...
        \item Adicionar Frasco: abre um modal para escolher o reagente a ser adicionado. A pesquisa ocorre localmente sobre o catálogo JSON já carregado (Seção~\ref{sec:catalogo-json-reagentes}), começa no primeiro caractere e é tolerante a caixa e acentos; não há consulta Firestore por tecla. O seletor de modo de busca (nome, CAS ou fórmula química) é uma escolha \textbf{opcional de UX} para orientar a digitação, não uma pré-condição de consulta nem uma limitação do Firestore. Após selecionar o reagente a ser adicionado, fecha o modal de escolher reagente e passa a preencher o formulário para adicionar o frasco de reagente, o fórmulário deve conter:
         ...
         O número do lote, que é \textbf{opcional} (não obrigar o gestor a inventar lote);\\
         A \textbf{condição física inicial} do frasco: \texttt{FECHADO} ou \texttt{JA\_ABERTO}. A \textbf{condição operacional} (em quarentena ou não) é uma pergunta separada: se marcada quarentena, aparece um campo de texto obrigatório explicando o motivo.
        ...
        líquido ->
        \begin{itemize}
    \item Se é do tipo fechado, pede apenas o volume nominal do líquido e o peso total com o frasco. (calcula e mostra a tara teórica, sem caráter de medição física definitiva, e não pode ser editada.)
    \item Se é do tipo \texttt{JA\_ABERTO}, exige apenas dados reais disponíveis: peso total atual medido e, quando conhecido ou legível, o conteúdo nominal do rótulo. Não há tara conhecida nem estimativa de volume. Exibir ``Saldo atual: desconhecido'' e ``Tara do recipiente: ainda não conhecida'' até o esgotamento, conforme UI-06.
        \end{itemize}
        sólido -> exige peso total e massa nominal do rótulo quando conhecida (fechado; NULL se desconhecida), ou peso total atual medido e conteúdo nominal quando conhecido/legível (já aberto). O saldo inicial é desconhecido; não usar tara conhecida nem estimativa de massa, conforme UI-06.
    ...
    \begin{itemize}
        ...
        \item \textbf{Registrar Devolução:} Modal crucial. O gestor informa o frasco retornado e insere o \textbf{Peso de Retorno} da balança. O modal apresenta a confirmação explícita ``O frasco ficou vazio nesta devolução?'', disponível tanto para frascos cadastrados como FECHADO quanto para os cadastrados como JA\_ABERTO; a confirmação do gestor é a autoridade do esgotamento. O sistema faz a diferença, aplica a densidade (se for líquido), e mostra em negrito o \textit{Volume Ultilizado em mL}, ou então o \textit{Peso Ultizado em gramas}, atualizando medida no frasco e seus status automaticamente antes de fechar o modal. Confirmado o esgotamento, o peso de retorno passa a ser a tara real e o frasco vai para VAZIO/INDISPONIVEL.
    \end{itemize}
...
Reagentes usam busca textual local sobre o catálogo JSON materializado (Resumo e Especificação): a pesquisa pode começar no primeiro caractere, normaliza caixa e acentos e não dispara consulta Firestore por tecla. Filtros de estado físico, natureza, tipo de substância e similares são opcionais no cliente, não pré-condições obrigatórias. A lista inicial é ordenada por nome (com desempate por ID) e renderiza uma janela de até 50 resultados com expansão progressiva; a janela reinicia ao mudar busca, filtro ou versão publicada. O cliente compara a versão publicada \texttt{Sistema\_Catalogo\_Reagentes/estado} com a versão local e baixa/atualiza o catálogo apenas quando necessário, refazendo a busca após a troca. Patrimônio exige prédio, letra ou status. Usar debounce de 300 ms como parâmetro inicial de interface. Zero correspondências no catálogo local significa ausência naquele recorte, não necessariamente no catálogo; busca exata por LCQUI-N, CAS, matrícula ou plaqueta tem modo próprio, valida o formato e consulta índice/chave específica, sem varrer toda a base. Não abrir automaticamente detalhes enquanto o usuário ainda digita.

Detalhes de reagente mostram resumo, especificações separadas, composição, densidade/unidade, fabricante, controle PF/EB e link FDS; frascos são paginados por almoxarifado e estado. Estado atual mostra saldo em g separado de mL, contagens de fechados, abertos, emprestados e quarentena e instante do cálculo via endpoint agregado protegido. Período histórico mostra exclusivamente movimentações e consumo FLOW, sem posição diária de estoque. Selecionar frasco abre código, localização, lote opcional, validade, pesagem, disponibilidade e histórico. Emprestado não revela dados do retirante a quem só consulta catálogo. Ações de gestão exigem papel e vínculo; Professor/Bolsista consultam seus empréstimos, e Aluno tem leitura de catálogo conforme seção 3, sem retirada.
...
Adicionar Frasco inicia pelo seletor de resumo e especificação UI-05; a busca é local sobre o catálogo JSON (Seção~\ref{sec:catalogo-json-reagentes}) e os modos nome/CAS/fórmula são opcionais de UX, não pré-condições. Exige almoxarifado autorizado e ativo, localização detalhada (campo opcional até 500 caracteres, recomendado para localizar a unidade), condição física inicial FECHADO/JA\_ABERTO e lote opcional (não obrigar o gestor a inventar lote). A condição operacional (em quarentena ou não) é uma decisão separada da condição física inicial: selecionar Quarentena torna o motivo obrigatório. Validade fechada é opcional se marcada desconhecida; prazo após abertura, quando informado, é inteiro positivo. Nunca tratar validade desconhecida como validade ilimitada. O servidor calcula validade efetiva; decisão sobre operação com validade desconhecida está em Q04.

Fechado: conteúdo nominal positivo quando conhecido (mL líquido/g sólido), NULL quando desconhecido; peso total positivo em g. Sem nominal, tara NULL e saldo desconhecido, preservado após abertura/pesagem bruta. Tara calculada é somente leitura e identificada como tara teórica derivada (peso total menos conteúdo vezes densidade para líquido, ou menos conteúdo para sólido), não sendo medição física definitiva do recipiente vazio. Já Aberto: \textit{conteudo\_nominal} continua sendo a quantidade original do rótulo/fabricante (não exigir que represente conteúdo restante) e é opcional quando desconhecido/ilegível (\texttt{NULL}, nunca zero); o frasco inicia com \texttt{condicao\_inicial\_cadastro = JA\_ABERTO}, \texttt{peso\_frasco\_vazio = NULL} e \texttt{saldo\_desconhecido = true}. Não existem os modos CONHECE\_TARA, ESTIMA\_VOLUME ou ESTIMA\_MASSA, nem campos de tara conhecida, estimativa de volume ou estimativa de massa. Exigir apenas o peso total atual medido em g e o conteúdo nominal quando conhecido/legível; exibir ``Saldo atual: desconhecido'' e ``Tara do recipiente: ainda não conhecida'' até o esgotamento. Data de abertura anterior desconhecida não deve ser fabricada como hoje (Q05); usar \texttt{abertura\_historica\_desconhecida} e \texttt{validade\_desconhecida} conforme a seção 7.

Se vencido, exigir destino: quarentena com motivo, pendente de descarte ou autorização excepcional explícita conforme Q04. Exibir revisão final com identidade, unidades, destino e dados de validade. Salvar cria código, frasco, histórico e auditoria de forma idempotente; apenas o servidor atribui LCQUI-N. O retorno mostra código e instrução de localizar/preencher a etiqueta física correspondente. Não gravar código sugerido por impressão anterior.

Editar propriedades permite localização, dados corrigíveis e decisões de estado autorizadas; não altera código, identidade química, peso de cadastro ou densidade. Pesagem de rotina exige novo peso em g, motivo e revisão da diferença; confirmar relê o frasco, exige disponibilidade e registra ajuste/evaporação separadamente do consumo em empréstimo. Tara incompatível bloqueia conclusão. Aplicar a tolerância híbrida Q06 e o esgotamento confirmado pelo gestor conforme a seção 7; recalibrar tara exige recipiente efetivamente vazio e peso vazio real medido, com motivo humano, e não aceitar consumo negativo silenciosamente.
...
Devolução seleciona empréstimo EM\_USO/ATRASADO do frasco, apresenta retirante, prazo, peso de saída e densidade fixada. Exige peso de retorno em g e identificação de quem devolve; operador é obtido da sessão. Prévia mostra diferença em g e consumo em g/mL. Peso superior ao de saída aplica tolerância híbrida Q06; ganho tolerado gera consumo zero e ajuste. A devolução apresenta a confirmação explícita ``O frasco ficou vazio nesta devolução?'' (Sim/Não), disponível tanto para frascos cadastrados como FECHADO quanto para os cadastrados como JA\_ABERTO; a confirmação do gestor é a autoridade do esgotamento, e a comparação com a tara anterior não é autoridade exclusiva. Confirmada e sem tara real anterior, o peso de retorno vira a tara real: o frasco transita para VAZIO/INDISPONIVEL, o saldo vira conhecido como 0\,g (sólido) ou 0\,mL (líquido), registra-se FICOU\_VAZIO e o conteúdo nominal não é alterado, sem inventar devolução fictícia. Sem confirmação de vazio e com retorno abaixo da tara real, a devolução física é aceita com anomalia automática, saldo desconhecido, consumo pendente e bloqueio para nova retirada. Se já existe tara real divergente, preservá-la e encaminhar a discrepância ao contrato de recalibração; VAZIO continua com saldo zero. Ao confirmar, servidor recalcula consumo, atraso e vencimento; resultado devolve consumo validado ou explicitamente pendente e estado atualizado. Qualquer gestor vinculado pode receber, mesmo que outro tenha registrado a saída. Frasco vencido na devolução exige destino, sem impedir o registro do retorno físico.

Alertas abrem lista filtrada de vazios, quebrados, vencidos, quarentena, pesagem ou escassez. Selecionar item abre UI-04 com ações cabíveis. Marcar vazio/quebrado exige motivo e confirmação; empréstimo ativo deve ser encerrado de modo auditado antes da transição incompatível. A quarentena é uma dimensão operacional separada do estado físico e da validade: ``COLOCAR EM QUARENTENA'' exige motivo humano; para frasco em quarentena, o gestor decide ``VOLTAR A DISPONIVEL'' ou ``PENDENTE DE DESCARTE'', com motivo/laudo, conforme UI-14. A decisão ``PENDENTE DE DESCARTE'' gera autorização operacional estruturada e mantém o frasco INDISPONIVEL; o descarte não ocorre diretamente da quarentena. Sair da quarentena não revalida, renova ou recalcula validade. Descartar exige confirmação explícita de descarte físico concluído, motivo e registro do operador/instante; é terminal, sem excluir documento. ``Removido'' significa retirada do catálogo operacional por estado/arquivamento, nunca exclusão de identidade; política específica em Q07. Erros administrativos suportados são tratados por ``Corrigir operação'' (UI-15), sem apagar o histórico.
...
O seletor de status não é um editor genérico: apresenta apenas a transição
permitida pelo estado corrente. Em V1, Ativo pode ser marcado Inservivel;
somente um Bem Inservivel apresenta o rito de baixa, e Ja\_dado\_baixa é somente
leitura. Conservação continua seletor independente. A tela exibe a versão lida
e, na confirmação, informa conflito sem sobrescrever a unidade; atualização de
nome/local no catálogo pode atualizar projeções de leitura, mas não apresenta
isso como edição do Bem. Os controles são UX: o backend relê M9, lock, versão,
status, arquivo e idempotência M7.

\subsubsection{UI-10: turmas, membros e convites}
Nova Turma exige nome (100), matéria existente, ano inteiro, semestre 1/2 e capacidade inteira positiva; responsável vem do contexto autorizado. Código único é gerado no servidor e exibido com Copiar, feedback de sucesso/falha da área de transferência e alternativa de seleção manual. Detalhe de turma tem nome/período, matéria, professor, ocupação/capacidade, feed e Membros. Membros exibem nome/foto para alunos matriculados; matrícula/e-mail ficam restritos à gestão autorizada. Na edição da turma, o campo capacidade exige inteiro positivo e não aceita valor menor que a ocupação corrente (``capacidade não pode ser menor que N alunos matriculados''): a UI mostra erro junto ao campo, preserva o valor atual e não envia; o backend relê \texttt{qtd\_alunos} na transação e rejeita fail-closed, orientando remover membros antes ou manter a capacidade.
...
Aceitar convite exige sessão com e-mail verificado correspondente, convite pendente não expirado, nome e matrícula válida se faltarem. Validar conta/papéis existentes, não sobrescrever identidade. Aceitação cria vínculo e espelho atomicamente e consome convite uma vez. Convite expirado deixa de contar como pendente, orienta solicitar novo e é materializado como \texttt{expirado} por reconciliação idempotente; lotação sem exceção válida mantém erro recuperável. Ingresso por código no painel Aluno exige código não vazio até 20 caracteres, turma ativa, vaga e ausência de remoção impeditiva; aluno já matriculado recebe acesso à turma sem duplicar contagem. Removido anteriormente precisa de convite explícito. Convite global não cria matrícula; a entrada na turma ocorre depois por código ou por outro convite. Remover membro abre confirmação com nome e turma, preserva comentários e histórico, remove vínculo/espelho e reduz contador na mesma transação, nunca abaixo de zero. Arquivar turma com alunos é permitido e preserva todos os vínculos; a turma arquivada permanece somente leitura. Um ingresso excepcional válido pode exceder a capacidade (\texttt{qtd\_alunos > capacidade}) sem invalidar a turma; nesse caso o ingresso ordinário fica bloqueado até surgir vaga, sem expulsar membros. Editar/reduzir a capacidade para abaixo da ocupação preexistente é rejeitado fail-closed (HQ-M11-001 = A), orientando remover membros antes ou manter a capacidade. O contrato normativo está na Seção~\ref{sec:regras-turmas-m11}.

\subsubsection{UI-11: roteiros, posts e comentários}
Novo Roteiro exige nome (150), descrição e PDF estritamente inferior a 15 MiB. Upload tem seleção/arrastar arquivo, nome/tamanho, progresso, Cancelar e Tentar novamente; não habilitar Salvar antes de validar upload, bytes \texttt{\%PDF-}, caminho, titularidade e geração no servidor. O Roteiro só fica publicável após essa validação; cancelamento/falha não cria registro utilizável e objeto órfão é reconciliado pelo servidor. Biblioteca tem Meus, Compartilhados comigo e Eu compartilhei; cada linha mostra nome, autor, descrição e ações permitidas. Visualizar abre leitor com fallback Baixar; download revalida propriedade, compartilhamento atual, ou vínculo canônico atual e Post acessível, e emite URL temporária com a geração validada. URL de arquivo não é autorização permanente nem credencial; ex-aluno e roteiro revogado fora de Post acessível não baixam. A remoção do Post, a perda de vínculo e a revogação impedem \textbf{novas} emissões, mas a URL curta já entregue permanece válida até expirar (sem revogação retroativa do token emitido). A geração persistida no documento é conferida na transação de vínculo; a inspeção do objeto no Storage é externa e não é atômica com o Firestore. Turma arquivada continua legível ao membro com vínculo atual.

Compartilhar abre pesquisa de professores ativos, exclui proprietário e seleciona IDs distintos. Confirmar cria relações únicas; repetir não duplica acesso. Revogar compartilhamento exige confirmação e segue Q09: impede novas associações pelo destinatário, mas preserva os Posts históricos e seus snapshots. Quem recebeu pode anexar à própria turma enquanto o compartilhamento vigora, mas não alterar autoria ou compartilhar em nome do dono. Na edição de Post, manter ou trocar o anexo exige acesso atual ao roteiro no commit; se o compartilhamento foi revogado, a edição que mantém o anexo falha fechado e o professor deve remover o anexo ou obter novo acesso.

Novo/Editar Post exige título (150) e descrição não vazia; roteiro é opcional e único por post. Clipe abre seletor com Meus/Compartilhados ou Novo Roteiro, preservando rascunho ao voltar. Publicar valida professor responsável, turma e acesso atual ao roteiro; guarda post, histórico de edição quando aplicável e notificações. Editar é exclusivo do professor dono e preserva a versão anterior no histórico; remover da apresentação é lógico, exige motivo e não apaga o Post nem o histórico (RF25). Turma arquivada é somente leitura para posts, edições e moderação (Q08). O roteiro anexo é fronteira M12.2: publicar valida o acesso atual e guarda o snapshot, mas upload, compartilhamento e emissão de URL ficam em M12.2. Feed pagina por data/ID com mais recentes primeiro; item removido da apresentação não aparece a colegas; navegação de notificação carrega o post alvo diretamente em vez de baixar todo o feed.

Comentários são carregados por post aberto, com texto obrigatório, autor/data e indicação de edição. Limite de texto segue Q10; rejeitar só espaços, renderizar texto escapado e não HTML arbitrário. Enviar mostra pendente e confirma após servidor validar matrícula/professor e post. Q11: autor pode editar, preservando histórico e etiqueta de edição; professor responsável (ou Chefe por Q13) modera com motivo obrigatório (1--2\,000), sem apagar o original; a edição do autor não desfaz a moderação. Comentário moderado mostra o original marcado ao autor, o aviso institucional aos colegas e o original com histórico ao professor dono e ao Chefe. A lista é entregue por endpoint autorizado que revalida o vínculo canônico atual e o estado da turma; nenhum cache, notificação ou mensagem de erro devolve o original a terceiros. Responder no feed é comentário no mesmo post, sem criar hierarquia de respostas ausente no modelo. Remoção do aluno não apaga comentários anteriores, mas revoga o acesso corrente à turma.

\subsubsection{UI-12: notificações, empréstimos próprios e relatórios}
Sino abre painel com Não lidas/Todas, tipo, resumo e instante. A caixa é única por conta (UID), reúne todos os papéis do usuário e é consultada pelo destinatário autenticado; Q12 inclui Bolsista e agrega alertas críticos de empréstimo independentemente do papel visual selecionado, e o papel visual não filtra a própria caixa. Clicar revalida acesso ao alvo e abre o contrato correspondente; recurso indisponível, removido ou revogado apresenta mensagem neutra sem expor dados. Limpar tudo pede confirmação de marcar notificações ativas do usuário como lidas, inclusive de outros papéis; mantém histórico, usa processamento paginado/reentrante e não apaga documentos; avisos emitidos após o corte de cada rodada seguem para a próxima. Alertas vencidos ou já lidos não autorizam ações por si mesmos. Fonte normativa na Seção~\ref{sec:regras-notificacoes-m13}.
...
\subsubsection{UI-14: gestão de quarentena}
Colocar um frasco em quarentena é uma ação de inspetoria independente de cadastro, abertura, devolução ou reencontro. ``COLOCAR EM QUARENTENA'' exige gestor autorizado, motivo humano (mínimo de 20 caracteres após trim) e transação que relê o estado atual e valida o escopo do almoxarifado; o efeito é \texttt{em\_quarentena = true} e \texttt{disponibilidade = INDISPONIVEL}, preservando \texttt{vencido}, \texttt{estado\_fisico\_frasco}, \texttt{saldo\_desconhecido} e validade. Nunca se usa \texttt{vencido = true} apenas para bloquear.

Para frasco em quarentena, a tela de decisão exige motivo/laudo humano (mínimo de 20 caracteres após trim) e oferece ``VOLTAR A DISPONIVEL'' (quando as demais regras permitirem) ou ``PENDENTE DE DESCARTE''. A decisão ``PENDENTE DE DESCARTE'' cria a autorização operacional estruturada de descarte, com o frasco em INDISPONIVEL; só depois disso o descarte é permitido. A operação é transacional, relê o estado atual, valida que o frasco ainda está em quarentena e o escopo, persiste uma única decisão e registra histórico; uma segunda operação concorrente é reavaliada sobre o novo estado e rejeitada se a pré-condição desapareceu. A saída da quarentena não altera \texttt{vencido} e não oferece ``revalidar validade'': essa operação não existe.

\subsubsection{UI-15: corrigir operação (erros administrativos)}
Para erros administrativos suportados, o histórico do frasco e as telas de gestão oferecem ``Corrigir operação''. O modal apresenta a operação original, o efeito que será corrigido e um campo de motivo; a confirmação exige motivo humano (mínimo de 20 caracteres após trim). Exemplo suportado: retirada atribuída ao usuário errado. A correção é compensatória e auditável: preserva o lançamento incorreto, registra a correção vinculada à operação original com o gestor responsável e o instante do servidor, sem usar \texttt{DELETE} e sem apagar o histórico. Não se desenha ``desfazer universal'' nem reversão de evento físico irreversível; consumos, pesagens e movimentações físicas não são apagados nem reescritos. Se houver operações dependentes que impeçam a correção simples, registrar pendência em vez de inventar reversão.
...
\paragraph{Devolução com inconsistência metrológica.}
Exibir: ``O peso informado está abaixo da tara real registrada. A medição será preservada. A devolução física pode ser concluída, mas o frasco ficará indisponível até resolver a inconsistência.'' Oferecer repetir pesagem, confirmar vazio após inspeção ou concluir devolução com anomalia. Se a tara for apenas referência teórica, explicar essa diferença. Nunca orientar ajuste fictício do número exibido pela balança. Mostrar consumo pendente separadamente de zero validado, leitura original e resolução auditável. Tara real anterior divergente requer o contrato de recalibração e não impede devolver fisicamente.

\paragraph{Atualização do estoque e pesagem ordinária.}
Dashboard atual segue a Seção~\ref{sec:estoque-atual-cache}: endpoint protegido, cache frontend curto, instante do cálculo, invalidação após mutação, botão Atualizar e tratamento de limite por UID. Período histórico oferece FLOW, sem gráficos de posição diária. Pesagem de rotina aceita observação opcional; descrição automática é identificada como sistema. Quarentena manual e recalibração continuam exigindo suas próprias justificativas humanas.

\subsubsection{UI-16: extravio e reencontro de frascos}
O detalhe do frasco (UI-04/UI-14) oferece as ações ``Registrar extravio'' e, para frasco \texttt{EXTRAVIADO}, ``Registrar reencontro''. Ambas exigem gestor autorizado com vínculo ao almoxarifado e motivo humano (mínimo de 20 caracteres após trim); a confirmação apresenta o efeito esperado. ``Registrar extravio'' mostra que o frasco sairá do estoque disponível, que um empréstimo ativo será encerrado extraordinariamente sem \texttt{peso\_retorno} e que nenhum consumo será inventado; a tela não pede peso de retorno. ``Registrar reencontro'' exige o estado físico constatado (\texttt{ABERTO}, \texttt{FECHADO}, \texttt{VAZIO} ou \texttt{QUEBRADO}) e informa que o frasco irá para \textbf{quarentena} (\texttt{INDISPONIVEL}), sem voltar automaticamente a \texttt{DISPONIVEL} e sem reabrir empréstimo encerrado. A tela só oferece os estados constatados coerentes com o último estado conhecido antes do extravio e rejeita transições impossíveis (\texttt{ABERTO}/\texttt{VAZIO}/\texttt{QUEBRADO} não podem virar \texttt{FECHADO}; \texttt{VAZIO}/\texttt{QUEBRADO} não podem virar \texttt{ABERTO}); quando um frasco vindo de \texttt{FECHADO} é encontrado \texttt{ABERTO}, avisa que a data de abertura passa a ser desconhecida. Se o frasco parece vazio sem que já fosse \texttt{VAZIO}, a tela não oferece \texttt{VAZIO}: registra a observação, mantém a quarentena e encaminha a confirmação quantitativa para M6. Um frasco extraviado ou em quarentena aparece marcado como indisponível e com o bloqueio de nova retirada; a tela oferece ``Registrar reencontro'' para extraviados e a decisão de saída da quarentena (UI-14) para os que estão nela. O motivo e a trilha (extravio, reencontro, quarentena) são exibidos no histórico, sem apagar eventos anteriores. Nenhuma tela oferece ``revalidar validade'' nem recalcula vencimento pelo relógio para reencontro. ``VOLTAR A DISPONIVEL'' nunca é oferecido para frasco \texttt{VAZIO} ou \texttt{QUEBRADO}.

\subsubsection{UI-17: resolução metrológica da devolução anômala}
Em uma devolução com pendência quantitativa, a tela mostra separadamente a leitura original \texttt{peso\_retorno} (preservada), o \texttt{peso\_saida}, a tara usada e sua origem (\texttt{REFERENCIA\_TEORICA}/\texttt{MEDIDA\_REAL}/\texttt{NULL}), a perda por evaporação declarada e a indicação de consumo pendente — nunca um valor zero validado. As ações oferecidas são exatamente as três rotas metrológicas tipadas: ``Repetir pesagem'', ``Confirmar esgotamento após inspeção'' e ``Recalibrar tara real'' (somente com recipiente vazio). Não existe ``Corrigir operação'' como ação de resolução metrológica, editor genérico de campo/valor nem ``ajustar peso'' livre. A correção administrativa (\texttt{corrigirOperacao}) existe apenas para tipos não metrológicos suportados e não encerra pendência quantitativa. A conversão para volume de líquidos usa a densidade histórica do empréstimo e é apresentada em mL separadamente de g. O servidor revalida tudo; a tela não substitui a medição nem promete liberação. Resolver a pendência \textbf{não} libera a quarentena: a saída continua na UI-14/M5. Quando o sistema não puder provar o instante histórico (houve movimentação física posterior), a tela mantém a pendência e registra a nova observação sem validar o consumo.

\subsubsection{UI-18: prevenção de erro em pesagens (dupla digitação independente)}
Toda leitura manual de balança que se torne fato quantitativo relevante usa \textbf{dupla digitação independente do mesmo operador}, como barreira preventiva antes da persistência. Aplica-se a \texttt{peso\_saida}, \texttt{peso\_retorno}, nova pesagem de resolução, peso vazio para estabelecer tara real e peso vazio para recalibrar tara (e, por coerência, às demais pesagens documentadas: cadastro, pesagem de rotina, esgotamento). Regras de interface:
\begin{itemize}
    \item \textbf{Dupla digitação independente:} o operador lê a balança e digita; a primeira entrada deixa de ser fonte copiável; o operador lê/digita novamente; o sistema normaliza as duas entradas e só prossegue quando forem idênticas. A segunda entrada \textbf{não} é preenchida automaticamente, não usa autofill, não copia o primeiro valor e não assume default \texttt{0}. Divergência bloqueia a confirmação, informa que as entradas não conferem e exige nova leitura, reiniciando a confirmação para não induzir cópia. Não se trata de conferência por dois profissionais nem de segundo aprovador (não altera M9).
    \item \textbf{Unidade fixa:} a massa de balança é sempre registrada em \textbf{g}; a interface exibe ``g'' permanentemente e não permite selecionar kg, mg, mL ou outra unidade nesse campo. Entrada decimal conforme o locale é normalizada de modo determinístico, sem que vírgula/ponto alterem a ordem de grandeza silenciosamente. Não há conversão automática de massa para volume no campo de peso.
    \item \textbf{Contexto visual:} junto ao campo, exibir código LCQUI do frasco, nome do reagente/especificação, \texttt{peso\_saida} original, \texttt{peso\_retorno} original quando existir, tara conhecida e \texttt{origem\_tara} (\texttt{NULL}/\texttt{REFERENCIA\_TEORICA}/\texttt{MEDIDA\_REAL}), estado do frasco e unidade. Para devolução, após entrada válida, mostrar de forma derivada (somente conferência, nunca substituta da leitura): diferença bruta saída$-$retorno, tolerância Q06 aplicável, classificação preliminar, consumo estimado/pendente e evaporação informada.
    \item \textbf{Alertas de plausibilidade x validação rígida:} valores estruturalmente inválidos (\texttt{NaN}, infinito, massa não positiva quando exigida, segunda digitação divergente, evaporação $>$ perda bruta) são \textbf{bloqueados antes do commit}; medições fisicamente inesperadas mas possíveis (ganho acima de Q06, retorno abaixo da tara persistida) \textbf{não} são alteradas, arredondadas nem substituídas — exigem nova conferência e seguem o contrato de anomalia quando persistidas.
    \item \textbf{Revisão final antes do commit:} etapa com frasco, operação, peso confirmado, peso de saída, tara/origem, diferença, Q06, evaporação e resultado esperado (normal / ganho tolerado / anomalia / vazio confirmado). A ação final comunica que a leitura será registrada como fato histórico; quando gerar anomalia, exige confirmação explícita equivalente a ``Li novamente o valor na balança e confirmo que esta é a leitura observada'' — barreira preventiva, nunca justificativa de modificação posterior.
\end{itemize}
O registro é contemporâneo à operação, com autoria e \texttt{timestamp} server-owned. A V1 não possui identificador estruturado de balança; a identificação automática do instrumento é estudo futuro (Seção~12).

\subsubsection{UI-19: estoque atual, escassez e notificações de almoxarifado}
A dashboard de estoque exibe uma \textbf{fotografia corrente} obtida por endpoint agregado protegido sobre \textit{Frasco\_Reagente} (Seção~\ref{sec:estoque-atual-cache}), nunca uma posição diária nem o resultado de resumo FLOW. Regras de interface:
\begin{itemize}
    \item \textbf{Instante e estado:} exibir o instante do cálculo (limite conservador da idade dos dados) e um estado explícito de atualização (em andamento, atual, desatualizado, erro, sem permissão). Não apresentar dado antigo como se fosse instantâneo.
    \item \textbf{Ação manual:} oferecer ``Atualizar''; não fazer \textit{polling} a cada $30$ segundos nem recálculo periódico automático. Uma ação de atualização é uma consulta sob demanda.
    \item \textbf{Rate limit:} a consulta usa a callable protegida com App Check, Auth, RBAC/escopo e limite de $5$ solicitações/minuto por UID, inclusive em \textit{cache hit}. A interface trata a rejeição por limite como estado próprio (aguardar e tentar novamente), sem mascarar erro nem insistir automaticamente.
    \item \textbf{Cache frontend:} o cliente pode manter cache apenas dentro da validade restante informada pelo servidor e descartá-lo após mutação; mudar de filtro/escopo \textbf{cancela a resposta anterior obsoleta} antes de exibir a nova.
    \item \textbf{Fotografia não autoriza retirada:} os números exibidos não reservam nem autorizam uma retirada. A disponibilidade é revalidada \textbf{transacionalmente} no comando real (\texttt{registrarRetirada}); um frasco pode ficar indisponível entre a exibição e a confirmação.
    \item \textbf{Saldo desconhecido:} saldo desconhecido, ausente ou não mensurável aparece \textbf{visualmente distinto de zero} (por exemplo, ``saldo desconhecido''), e massa (g) e volume (mL) são apresentados em colunas/totais separados, nunca somados entre si. Contagens de frascos com saldo conhecido e com saldo desconhecido são exibidas separadamente.
    \item \textbf{Escassez:} a configuração de escassez é por \textit{almoxarifado} $\times$ \textit{resumo} $\times$ \textit{especificação} e define um limiar em \textbf{número de frascos aptos}. A tela de configuração diferencia \texttt{ativo} de \texttt{notificacao\_ativa}; \texttt{notificacao\_ativa = false} é apresentado como \textbf{silenciamento de alerta} (configuração preservada), e não como inexistência de estoque ou remoção do par.
    \item \textbf{Notificações:} os alertas \texttt{ESCASSEZ\_ESTOQUE} são exibidos ao gestor vinculado ao almoxarifado; o painel distingue não lidas de todas, e ``Limpar tudo'' \textbf{marca como lidas} as notificações ativas, preservando o histórico (sem exclusão).
\end{itemize}
