...
O detalhe do frasco (UI-04/UI-14) oferece as ações ``Registrar extravio'' e, para frasco \texttt{EXTRAVIADO}, ``Registrar reencontro''. Ambas exigem gestor autorizado com vínculo ao almoxarifado e motivo humano (mínimo de 20 caracteres após trim); a confirmação apresenta o efeito esperado. ``Registrar extravio'' mostra que o frasco sairá do estoque disponível, que um empréstimo ativo será encerrado extraordinariamente sem \texttt{peso\_retorno} e que nenhum consumo será inventado; a tela não pede peso de retorno. ``Registrar reencontro'' exige o estado físico constatado (\texttt{ABERTO} ou \texttt{FECHADO}) e informa que o frasco irá para \textbf{quarentena} (\texttt{INDISPONIVEL}), sem voltar automaticamente a \texttt{DISPONIVEL} e sem reabrir empréstimo encerrado. Um frasco extraviado ou em quarentena aparece marcado como indisponível e com o bloqueio de nova retirada; a tela oferece ``Registrar reencontro'' para extraviados e a decisão de saída da quarentena (UI-14) para os que estão nela. O motivo e a trilha (extravio, reencontro, quarentena) são exibidos no histórico, sem apagar eventos anteriores. Nenhuma tela oferece ``revalidar validade'' nem recalcula vencimento pelo relógio para reencontro.
