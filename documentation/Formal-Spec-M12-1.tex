...
autor. A participação acadêmica é o vínculo canônico atual (M11) \textbf{ou} o
professor dono da turma, nunca o espelho, cache ou claim. Claim atualizada
\textbf{não} recria participação: um ex-aluno removido não edita nem lê o
próprio Comentário, ainda que a claim esteja corrente (contraexemplo coberto por
\texttt{WitnessClaimAtualSemVinculo}). Somente o professor dono cria/edita Post;
o autor edita o próprio Comentário; o Chefe intervém apenas por Q13
(moderação/remoção com motivo e auditoria), sem assumir autoria nem criar Post.
Turma \texttt{Arquivada} nega toda escrita acadêmica, inclusive moderação e
para o Chefe (Q08), por propriedade transversal
(\texttt{TurmaArquivadaNegaEscrita}).
...
não são provadas aqui e são contratadas na
Seção~\ref{sec:regras-roteiros-m12-2} (M12.2). A \texttt{POST}/\texttt{COMENTARIO} entra apenas como
efeito mínimo de notificação, vinculada ao \texttt{idOperacao}, sem conteúdo
protegido e sem autorizar leitura; a caixa, fan-out, expiração e marcação de
lida são M13. A composição M7 modela primeira execução (grava receipt sob o
\texttt{idOperacao}), retry idêntico (devolve o resultado e não reaplica) e
reuso incompatível (rejeitado, sem herdar receipt alheio); M9 faz a revogação
de vínculo impedir o commit posterior e a identidade de participação é
revalidada na transação.
...
\input{generated/entities/formal_m12_1_posts.tex}
\input{generated/invariants/formal_m12_1.tex}
\par\endgroup
