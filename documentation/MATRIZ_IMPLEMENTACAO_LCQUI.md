...
| IMP-ROLE-003 | Autorização | Autorização persistida no commit | S7 M9; S10.2–10.4; S11 | M9 | `auth.ts: resolverAutoridadePersistidaTx/validarAutoridadePersistida` (fundação M9 etapa A + endurecimento A.1); `usuarios.ts: alterarPapel/convidarUsuario/revogarUsuarioPapel` migrados; `auth.ts: validarPermissao` legado | SIM | DIVERGENTE | Fundação M9 backend criada e endurecida: a primitiva transacional lê TODOS os seis documentos canônicos de papel do UID, valida `id_usuario`, aplica a matriz fechada sobre o conjunto persistido e exige que a claim projete exatamente esse conjunto na versão corrente, além de `Usuarios.ativo` — aplicada no commit de `alterarPapel`/`convidarUsuario`/`revogarUsuarioPapel` (fecha TOCTOU nessas operações). Permanece divergente porque as demais callables ainda usam `validarPermissao` JWT-only com escopo fora da transação, e porque `convidarUsuario` mantém efeito externo Auth entre pré-checagem e commit. | `auth.test.ts` (parser/helpers legados); `integration/m9-backend-authority.integration.test.ts` (`TEST-INT-M9-AUTH-001–021`, `-OP-001–004`, PASS Node 20); `integration/usuarios.integration.test.ts` | Migrar cada domínio para a primitiva M9 transacional; matriz de escopo/ownership; tratamento do efeito externo de `convidarUsuario` (M7/outbox) |
...
| IMP-IDEM-001 | Idempotência | Receipt e retry universal de mutações críticas | S7 M7; S10.10 | M7 | `idempotencia.ts` (fundação canônica M7, etapa A): `canonicalize`, `hashPayload`, `resolverOperacaoTx` e primitivas de receipt; `usuarios.ts: Operacoes` legado | SIM | DIVERGENTE | Fundação canônica M7 criada e testada (canonicalização recursiva, identidade `(uid, tipo_operacao, payload_hash)`, replay/`already-exists`, estados PENDENTE/CONCLUIDA/FALHOU e receita fail-closed). Permanece divergente porque nenhuma mutação de produto foi migrada: `usuarios.ts` ainda usa o receipt legado (`chave = sha256([ator,idOperacao])`, `hash_entrada = JSON.stringify(dados)`, sem `tipo_operacao`/`payload_hash`) e `idOperacao` segue opcional/aleatório; os demais domínios não têm receipt. | `domain/idempotencia.test.ts` (`TEST-UNIT-M7-CANON/HASH/ID`); `integration/m7-idempotencia.integration.test.ts` (`TEST-INT-M7-RECEIPT-001–012`, PASS Node 20); integração de usuários (legado) | Migrar cada mutação crítica para a primitiva M7; remover fallback de `idOperacao`; aplicar em reagentes/patrimônio/turmas/posts/roteiros/notificações/relatórios/etiquetas |
