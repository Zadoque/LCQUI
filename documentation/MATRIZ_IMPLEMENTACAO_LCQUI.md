...
| IMP-ROLE-003 | Autorização | Autorização persistida no commit | S7 M9; S10.2–10.4; S11 | M9 | `auth.ts: validarPermissao/validarGestorDoAlmoxarifado`; callables | SIM | DIVERGENTE | `validarPermissao` aceita somente JWT; conta ativa, versão e papel persistido não são relidos na transação da maioria das mutações. Consultas de escopo também ocorrem fora da unidade transacional. | `auth.test.ts` | Matriz M9 completa, revogação entre leitura e commit |
...
| ID | Domínio | Feature | Fonte normativa | Milestone(s) | Implementação encontrada | Auditado | Estado | Divergência / evidência | Testes existentes relacionados | Teste futuro necessário |
|---|---|---|---|---|---|---|---|---|---|---|
| IMP-PAT-001 | Patrimônio | Cadastro/manutenção de resumo e bem | RF06; S5; S7 M10; S8 UI-09; S9 PAT-01 | M10 | aprovação de adição; telas com escrita direta | SIM | DIVERGENTE | Não há mutação canônica completa; faltam versão inicial/incremento, enums/limites e foto validada; Rules permitem escrita direta e UI usa `addDoc/updateDoc`. | `patrimonio.test.ts` | Lifecycle M10 + versão otimista + M9 |
