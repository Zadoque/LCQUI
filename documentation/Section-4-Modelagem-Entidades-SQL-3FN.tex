...
Quando \textit{em\_quarentena = TRUE}, deve existir justificativa associada à entrada em quarentena, materializada em \textit{detalhe\_status} não nulo. O tipo físico continua \textit{TEXT NULL}, pois a exigência é condicional. Quando a justificativa obrigatória é fornecida por usuário/gestor, o texto deve possuir no mínimo 20 caracteres após remover espaços do início e do fim (\texttt{trim}). Não avaliar qualidade semântica nem acrescentar limite máximo geral. Mensagens produzidas automaticamente pelo servidor não são entradas humanas; a restrição de comprimento não é global para todo \textit{detalhe\_status} não nulo.
\end{regra}
...
Massa consumida = $\max(0, peso_{saida}-peso_{retorno})$ em g. Medida utilizada é não negativa (g para sólido, mL via densidade para líquido). Medida usada no frasco acumula apenas gramas. Tolerância normal: $\max(1\,g,0{,}005\,peso_{saida})$; higroscópico: $\max(2\,g,0{,}02\,peso_{saida})$. A fórmula de tolerância incide estritamente sobre o \textbf{Peso Bruto de Saída ($peso_{saida}$)}. Ganho dentro da tolerância preserva peso físico real, consumo zero e evento AJUSTE/ganho\_massa\_higroscopia --- restrito estritamente a frascos com \texttt{eh\_higroscopico = true}; é terminantemente proibido registrar tal evento para frascos não-higroscópicos ou com ganhos fora da tolerância legal. Ganho acima da tolerância ($peso_{retorno} > peso_{saida} + \Delta_{\max}$) \textbf{não aborta a operação via rollback}: o sistema conclui a devolução em modalidade extraordinária, transicionando o empréstimo para \texttt{DEVOLVIDO\_COM\_ANOMALIA} e o frasco compulsoriamente para \texttt{disponibilidade = INDISPONIVEL} e \texttt{em\_quarentena = true}, com \texttt{detalhe\_status} registrando suspeita de contaminação/adulteração. Retorno abaixo da tara ($peso_{retorno} < peso_{vazio}$): diferença até 5 g com \texttt{confirmarEsgotamento = true} permite devolução para VAZIO/INDISPONIVEL e evento FICOU\_VAZIO com ajuste metrológico; nesse caso a massa consumida é todo o saldo restante, $\max(0, peso_{saida}-peso_{vazio})$, e o sistema \textbf{atualiza} \texttt{peso\_frasco\_vazio} e \texttt{peso\_atual} para o valor aferido na devolução ($peso_{retorno}$), eliminando o resíduo metrológico e homologando a tara real do recipiente. Diferença superior a 5 g ou ausência da confirmação bloqueia a devolução ordinária e exige recalibração formal de tara com justificativa, salvo quando acompanhada da \textit{flag} \texttt{recalibrarTaraParcial = true} com \texttt{novoPesoVazioEstimado} e \texttt{motivoRecalibracao} --- caso em que o frasco permanece ABERTO com a nova tara estimada registrada e o evento auditado em \textit{Historico\_Frasco\_Reagente} do tipo \texttt{AJUSTE/recalibracao\_tara\_parcial}. Não converter ajuste em consumo negativo.
\end{regra}
