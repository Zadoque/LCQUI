...
REENCONTRO & Novo fato físico, não desfaz o extravio: de \texttt{EXTRAVIADO} para o estado constatado \texttt{ABERTO}/\texttt{FECHADO}, com \texttt{em\_quarentena = TRUE} e \texttt{disponibilidade = INDISPONIVEL}. Não reabre empréstimo encerrado e não recalcula validade. \texttt{REENCONTRADO} $\neq$ \texttt{DISPONIVEL}.\\
QUARENTENA + \texttt{VOLTAR\_A\_DISPONIVEL} & Encerra a quarentena (\texttt{em\_quarentena = FALSE}, \texttt{disponibilidade = DISPONIVEL}) somente se o estado físico for \texttt{ABERTO}/\texttt{FECHADO}, sem pendência metrológica e sem \texttt{vencido} sem uso autorizado; não revalida validade.\\
