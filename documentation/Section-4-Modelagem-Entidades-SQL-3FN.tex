...
\campo{versao\_permissoes}{INTEGER}{DEFAULT 0, NOT NULL}
\campo{claims\_pendentes}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\end{entidade}
\begin{regra}
\textit{ativo}, \textit{versao\_permissoes} e \textit{claims\_pendentes} pertencem à
identidade autorizativa, não a um perfil de apresentação. Papéis são relações
normalizadas nas entidades de papel; Custom Claims são apenas projeção de
transporte. A autoridade e o algoritmo de decisão estão na Seção~7,
``Autorização e usuários (M9)''.
\end{regra}
...
\begin{regra}[Identidade, ciclo de vida e versão da unidade]
\textit{numero\_patrimonio} é a identidade externa permanente da unidade física,
e não é reutilizado após a baixa. Sua forma canônica é
\texttt{trim().toUpperCase()}; a entrada vazia após \texttt{trim()} é rejeitada e
o valor canônico, de no máximo 30 caracteres, é o único valor comparado,
armazenado, indexado e usado em locks e chaves únicas. Assim \texttt{ABC-123},
\texttt{abc-123} e \texttt{\ \ ABC-123\ \ } designam a mesma unidade.

\textit{estado\_conservacao} e \textit{status} são dimensões independentes. O
ciclo V1 fechado é \texttt{Ativo -> Inservivel -> Ja\_dado\_baixa};
\texttt{Ativo -> Ja\_dado\_baixa} e \texttt{Inservivel -> Ativo} são negados.
\texttt{Ja\_dado\_baixa} é terminal para fatos da unidade: não há exclusão física,
troca de foto/local/resumo/status ou reutilização de plaqueta; somente retenção,
consulta e auditoria são permitidas. A segunda transição exige o rito SEI e o
comprovante PDF validado.

\textit{versao} começa em 1 e incrementa uma vez em cada mutação canônica da
unidade (edição aprovada, mudança de local, reclassificação, troca de foto,
mudança de conservação/status e baixa). Ela não incrementa por fan-out de
\textit{nome\_equipamento}, \textit{predio}, \textit{andar}, \textit{sala} ou
\textit{letra\_inicial\_nome}: tais campos são projeções derivadas e não fatos
editados da unidade. Essa semântica evita conflito falso sem ocultar conflito
de edição canônica.
\end{regra}
...
\subcampo{A requisição docente não aceita Ja\_dado\_baixa; a baixa é exclusiva do Gestor pelo rito institucional SEI (RF12).}
...
\begin{explicacao}
A alteração de nome requer a indicação do modelo catalográfico em \textit{novo\_id\_resumo\_bem\_patrimonial}. Caso o resumo não exista no catálogo, a aprovação do gestor cria um novo documento de resumo usando o texto em \textit{novo\_nome}, sem renomear o catálogo compartilhado pré-existente (conforme C001).
\end{explicacao}
...
\campo{numero\_patrimonio\_normalizado}{VARCHAR(30)}{NOT NULL}
\campo{status}{ENUM}{pendente, aprovada, rejeitada, NOT NULL}
\campo{feita\_em}{TIMESTAMP}{NOT NULL}
...
\end{explicacao}
\begin{regra}
RF10b: \textit{UNIQUE(numero\_patrimonio\_normalizado) WHERE status = 'pendente'} --
...
\campo{id\_requisicao}{VARCHAR(100)}{NOT NULL}
\campo{tipo}{ENUM}{EDICAO, ADICAO, NOT NULL}
\campo{chave\_recurso}{VARCHAR(100)}{NOT NULL}
...
\begin{itemize}
  ...
  \item Para requisição de \textbf{adição}: docId = \texttt{bem\_adicao\_\{numero\_patrimonio\_normalizado\}}
\end{itemize}

O lock é removido na transação de aprovação/rejeição ordinária. O lock \texttt{Locks\_Requisicao\_Patrimonio/bem\_edicao\_\{id\}} DEVE ser removido pela transação também em caso de rejeição por divergência de versão, referência inválida ou outro desfecho terminal documentado, solucionando deadlocks permanentes. Antes de removê-lo, a transação exige \textit{id\_requisicao}, \textit{tipo} e \textit{chave\_recurso} correspondentes; lock ausente ou pertencente a outra requisição é falha de integridade que não libera lock alheio. Lock e requisição são criados atomicamente; não há commit parcial dessa transação. Limpeza só pode remover lock órfão após verificar pelo \textit{id\_requisicao} que não há requisição pendente correspondente; jamais remove um lock apenas porque a requisição pendente tem mais de 24 horas.
\end{regra}
...
\campo{qtd\_limiar\_escassez}{INTEGER}{CHECK (qtd\_limiar\_escassez >= 0), NOT NULL}
\campo{ativo}{BOOLEAN}{DEFAULT TRUE, NOT NULL}
...
Há escassez para o par se, e somente se, \texttt{qtdAptos} $<$ \texttt{qtd\_limiar\_escassez}, onde \texttt{qtdAptos} é a quantidade de frascos aptos; a comparação é estritamente menor e nunca satisfeita por saldo em g/mL. A definição canônica de aptidão, as fronteiras e a semântica das flags estão na Seção~\ref{sec:m8-estoque-escassez-notificacoes}.
\end{explicacao}
...
\campo{origem\_tara}{ENUM}{REFERENCIA\_TEORICA, MEDIDA\_REAL, NULL}
...
\campo{situacao\_localizacao}{ENUM}{LOCALIZADO,EXTRAVIADO, NOT NULL}
...
\campo{saldo\_desconhecido}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\campo{condicao\_inicial\_cadastro}{ENUM}{FECHADO,JA\_ABERTO, NOT NULL}
\end{entidade}
...
\begin{explicacao}
\textit{conteudo\_nominal} representa o conteúdo declarado no cadastro: para líquidos, é o volume nominal em mL; para sólidos, é a massa nominal em g. Se NULL, o valor original do rótulo é desconhecido ou ilegível (não usar zero para representar desconhecido).
...
Os campos \textit{peso\_no\_cadastrado}, \textit{peso\_atual}, \textit{peso\_frasco\_vazio} e \textit{medida\_usada} são expressos em gramas (peso atual é observado; tara pode ser referência derivada e medida usada é consumo validado), independentemente de o conteúdo ser sólido ou líquido, já que a operação real do laboratório consiste em pesar o frasco; o volume, quando aplicável, é sempre derivado por conversão via densidade.
...
O campo \textit{condicao\_inicial\_cadastro} (enum \texttt{FECHADO} ou \texttt{JA\_ABERTO}) é uma dimensão \textbf{histórica e imutável}: registra se o recipiente entrou fisicamente fechado no controle do LCQUI (\texttt{FECHADO}) ou se já estava aberto quando passou ao seu controle (\texttt{JA\_ABERTO}). Não é reescrito quando o estado atual muda e \textbf{não} se confunde com \textit{saldo\_desconhecido}: \textit{condicao\_inicial\_cadastro} é o fato histórico de ingresso, enquanto \textit{saldo\_desconhecido} é a propriedade metrológica \emph{atual} de desconhecimento da quantidade remanescente. Um frasco cadastrado \texttt{JA\_ABERTO} inicia com \textit{estado\_fisico\_frasco = ABERTO}, \textit{peso\_frasco\_vazio = NULL} e \textit{saldo\_desconhecido = TRUE}; nenhuma tara artificial é derivada. Um frasco cadastrado \texttt{FECHADO} inicia com \textit{estado\_fisico\_frasco = FECHADO} e saldo conhecido somente quando houver dados quantitativos suficientes; a tara inicial eventualmente derivada do peso total menos o conteúdo nominal é apenas referência enquanto houver produto, não medição física definitiva do recipiente vazio.

\textit{saldo\_desconhecido} marca o desconhecimento \emph{atual} da quantidade remanescente e não é flag histórica: cadastro \texttt{JA\_ABERTO} implica \texttt{TRUE}; abertura de FECHADO preserva o conhecimento ou desconhecimento anterior; frasco esvaziado com tara real medida passa a \texttt{FALSE}. Somente \texttt{VAZIO} confirmado pela rota metrológica válida resolve o desconhecimento (saldo zero, flag falsa). \texttt{QUEBRADO}, \texttt{DESCARTADO}, localização \texttt{EXTRAVIADO} e quarentena preservam \texttt{saldo\_desconhecido}, seja verdadeiro ou falso. Estado físico, localização e conhecimento metrológico são dimensões distintas: extravio não substitui o estado físico, e quebra, descarte e indisponibilidade não são medição nem fabricam saldo zero, tara ou pesagem. Resolver desconhecimento exige evidência metrológica válida; retirar do estoque não determina a quantidade física anteriormente existente. \texttt{DESCARTADO} continua terminal operacionalmente.

\textit{situacao\_localizacao} é dimensão ortogonal ao estado físico e assume \texttt{LOCALIZADO} ou \texttt{EXTRAVIADO}. O extravio preserva o último estado físico conhecido, revoga a autorização corrente de descarte e registra a decisão anterior no histórico. O reencontro grava \texttt{LOCALIZADO}, impõe quarentena e não restaura autorização; nova autorização é necessária para o descarte. Assim, combinações como \texttt{QUEBRADO + EXTRAVIADO} e \texttt{ABERTO + EXTRAVIADO} são representáveis, enquanto \texttt{estado\_fisico\_frasco = EXTRAVIADO} não pertence ao domínio físico.

\textit{abertura\_historica\_desconhecida} é flag \emph{histórica}: quando \texttt{TRUE}, \textit{data\_abertura} é NULL e o estado físico corrente não pode ser \texttt{FECHADO}. A flag não é apagada apenas porque o estado atual passou a \texttt{VAZIO}, \texttt{QUEBRADO} ou \texttt{DESCARTADO}, nem porque a localização passou a \texttt{EXTRAVIADO}: significa que já houve abertura e que a data da primeira abertura é desconhecida. Quando não se pode determinar a validade efetiva exata por ausência dessa data histórica (\textit{abertura\_historica\_desconhecida = TRUE} combinada com \textit{validade\_apos\_aberto\_dias}), usa-se \textit{validade\_desconhecida = TRUE} e \textit{validade\_efetiva = NULL}, salvo outra informação suficiente. \textit{validade\_desconhecida = TRUE} \textbf{não} implica \textit{vencido = FALSE}: pode haver prova de vencimento sem data exata (por exemplo, \textit{validade\_fechado} conhecida e já expirada), de modo que ``conhecer a data exata de validade'' e ``ser possível concluir que já venceu'' são conclusões distintas.
\end{explicacao}

\begin{regra}[Lote opcional e justificativa de quarentena (M2.1a)]
O cadastro permite \textit{id\_lote = NULL}, inclusive para registros legados, abertos ou vencidos; sem lote, a referência direta à especificação continua obrigatória. A ausência de lote, isoladamente, não exige quarentena nem indisponibilidade. Quando houver lote, verificar sua existência e correspondência com a especificação informada.

Quando \textit{em\_quarentena = TRUE}, deve existir justificativa associada à entrada em quarentena, materializada em \textit{detalhe\_status} não nulo. O tipo físico continua \textit{TEXT NULL}, pois a exigência é condicional. Quando a justificativa obrigatória é fornecida por usuário/gestor, o texto deve possuir no mínimo 20 caracteres após remover espaços do início e do fim (\texttt{trim}). A exigência de mínimo incide apenas sobre os contratos humanos enumerados: entrada manual em quarentena; saída ou liberação de quarentena; decisão de quarentena no cadastro vencido; decisão de quarentena na primeira abertura; extravio; reencontro; descarte institucional; recalibração de tara real; autoatendimento; justificativa metodológica Q04/Q14; e correção administrativa/operacional de ação do gestor. Não avaliar qualidade semântica nem acrescentar limite máximo geral. Mensagens produzidas automaticamente pelo servidor não são entradas humanas; a restrição de comprimento não é global para todo \textit{detalhe\_status} não nulo.
\end{regra}

\begin{regra}[Conhecimento metrológico atual -- HQ-M2-004]
Um cadastro FECHADO admite conteúdo nominal NULL quando desconhecido ou ilegível, nunca zero. Nesse caso, peso\_frasco\_vazio e origem\_tara são NULL e saldo\_desconhecido é true. Abertura ou pesagem bruta isolada não resolvem esse desconhecimento. Sem tara real, segue o mesmo ciclo de vida de JA\_ABERTO em situação metrológica equivalente: não exigir tara fabricada para quebra, descarte, extravio ou reencontro. EXTRAVIADO preserva a flag; \texttt{VAZIO} e \texttt{QUEBRADO} são estados fisicamente não utilizáveis e indisponíveis, normalmente encaminhados para descarte, mas \textbf{não} são estados terminais da máquina; apenas \texttt{DESCARTADO} é terminal. Nada disso inventa peso ou quantidade química.
\end{regra}

\begin{regra}[Pendência de descarte técnico e terminalidade -- HQ-M2-008/B]
Um frasco em quarentena não vai diretamente a \texttt{DESCARTADO}. A quarentena é encerrada por decisão humana explícita em \texttt{resolverQuarentenaFrasco}: \texttt{VOLTAR\_A\_DISPONIVEL} ou \texttt{PENDENTE\_DE\_DESCARTE}. A decisão \texttt{PENDENTE\_DE\_DESCARTE} mantém \texttt{disponibilidade = INDISPONIVEL}, define \texttt{em\_quarentena = FALSE}, cria uma autorização operacional estruturada server-owned (projeção \texttt{Pendencias\_Descarte\_Frasco/\{idFrasco\}}) e um evento imutável \texttt{PENDENTE\_DE\_DESCARTE} no histórico. Somente com essa autorização ativa \texttt{descartarFrasco} aceita a rota técnica, consumindo a projeção na mesma transação. \texttt{PENDENTE\_DE\_DESCARTE} não é coluna de \texttt{Frasco\_Reagente} (o frasco permanece com 29 campos) nem entidade 3FN; é projeção operacional derivada de decisão humana. \texttt{DESCARTADO} é terminal.
\end{regra}
...
\campo{tipo}{ENUM}{
  \shortstack[l]{%
    ...
    USO\_VENCIDO\_AUTORIZADO, EXTRAVIO, REENCONTRO, CORRECAO\_OPERACIONAL%
  }
}{NOT NULL}
...
O histórico registra as transições que sustentam a reconstrução auditável do ciclo do frasco, incluindo \texttt{FICOU\_VAZIO} (esgotamento confirmado pelo gestor), \texttt{EXTRAVIO} e \texttt{REENCONTRO} (extravio e reencontro preservando as dimensões anteriores), e \texttt{ENTROU\_EM\_QUARENTENA} (entrada em quarentena como dimensão ortogonal à validade). Erros operacionais do gestor são corrigidos por \textbf{correção compensatória e auditável}: o lançamento incorreto é preservado e uma correção vinculada à operação original é registrada, com referência à operação corrigida, gestor que corrigiu, \textit{timestamp} do servidor, motivo humano de no mínimo 20 caracteres após \texttt{trim} e o efeito efetivo corrigido. Não se usa DELETE e não se inventa reversão de evento físico irreversível; distingue-se erro administrativo corrigível (por exemplo, retirada atribuída ao usuário errado, corrigida pelo vínculo compensatório do retirante sem devolução fictícia, consumo fictício ou nova pesagem artificial) de evento físico irreversível.
\end{explicacao}
...
\campo{status}{ENUM}{EM\_USO,ATRASADO,DEVOLVIDO,DEVOLVIDO\_COM\_ATRASO,DEVOLVIDO\_COM\_ANOMALIA,ENCERRADO\_EXTRAORDINARIO, NOT NULL}
...
\campo{consumo\_validado}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\campo{anomalia\_metrologica}{TEXT}{NULL}
\campo{peso\_retorno\_efetivo}{NUMERIC(10,3)}{NULL}
\campo{id\_resolucao\_metrologica}{INTEGER}{FK, NULL}
\campo{unidade\_medida\_utilizada}{ENUM}{ml,g, NOT NULL}
\campo{densidade\_aplicada}{NUMERIC(10,5)}{NULL}
\campo{peso\_perda\_evaporacao}{NUMERIC(10,3)}{DEFAULT 0, NOT NULL}
\campo{uso\_vencido\_aceito}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\campo{vencido\_na\_retirada}{BOOLEAN}{NOT NULL}
...
\textit{peso\_saida} e \textit{peso\_retorno} são sempre leituras de balança em gramas. \textit{unidade\_medida\_utilizada} qualifica exclusivamente \textit{medida\_utilizada} -- o consumo validado (NULL enquanto pendente), reportado em massa (g) para sólidos ou em volume (ml) para líquidos, via $V = \Delta m / \rho$. \textit{densidade\_aplicada} é o snapshot histórico da densidade da especificação no instante da retirada, imutável, obrigatório para líquidos e NULL para sólidos; participa da conversão de consumo e de evaporação sem depender da densidade corrente da especificação.
...
A devolução com anomalia encerra a custódia como \texttt{DEVOLVIDO\_COM\_ANOMALIA} e deixa a pendência quantitativa aberta (\texttt{consumo\_validado = FALSE}, \texttt{peso\_retorno\_efetivo = NULL}, \texttt{id\_resolucao\_metrologica = NULL}). A pendência é encerrada apenas por três rotas metrológicas tipadas (repetir pesagem; confirmar esgotamento após inspeção; recalibrar tara real quando aplicável); não existe resolvedor público genérico nem correção administrativa metrológica na V1. Resolver a pendência não libera automaticamente a quarentena, e \texttt{peso\_retorno} original nunca é apagado. A condição de pendência é \texttt{status = DEVOLVIDO\_COM\_ANOMALIA} e \texttt{consumo\_validado = FALSE}.

O campo \textit{vencido\_na\_retirada} é um \textbf{snapshot factual server-owned e imutável}: registra se o frasco estava vencido no instante em que a retirada efetivamente criou o empréstimo, considerando o estado resultante de eventual primeira abertura executada na mesma operação. Ele \textbf{não} é sinônimo de \textit{uso\_vencido\_aceito}, não é autorização e não é recalculado na devolução. A devolução classifica o retorno pelo estado canônico persistido \textit{Frasco\_Reagente.vencido} (mantido pelo job de vencimento): se o frasco está vencido agora e não estava na retirada, o retorno ``venceu durante o empréstimo''; se já estava vencido na retirada, ``devolução de frasco vencido''; se a validade é desconhecida e não há vencimento afirmativo, ``validade desconhecida''; caso contrário, devolução normal. Quando \textit{vencido = TRUE} e \textit{validade\_desconhecida = TRUE} coexistem, a classificação de vencimento tem precedência e usa \textit{vencido\_na\_retirada}, sem mensagens contraditórias simultâneas.
\end{explicacao}
...
\begin{itemize}
\item \textit{volume\_total\_disponivel\_aferido\_ml} e \textit{massa\_total\_disponivel\_aferida\_g};
...
\item \textit{qtd\_frascos\_saldo\_desconhecido};
...
\campo{capacidade}{INTEGER}{CHECK (capacidade > 0), NOT NULL}
\campo{codigo\_turma}{VARCHAR(20)}{UNIQUE, NOT NULL}
\campo{data\_criacao}{TIMESTAMP}{NOT NULL}
\campo{qtd\_alunos}{INTEGER}{CHECK (qtd\_alunos >= 0), NOT NULL}
\campo{versao}{INTEGER}{NOT NULL}
\end{entidade}
...
O ingresso ordinário de aluno (por código ou por convite nominal sem exceção)
exige que a quantidade de alunos matriculados ativos seja estritamente menor
que a capacidade da turma:
\[
\text{COUNT}(\text{Aluno\_x\_Turma WHERE id\_turma} = X) < \text{Turma.capacidade}
\]
No modelo 3FN a grandeza é o \texttt{COUNT}; no Firestore a autoridade
transacional é \texttt{Turma.qtd\_alunos}, lida e escrita na mesma transação que
cria ou remove o vínculo canônico
\texttt{Turma/\{id\}/Alunos/\{uid\}}, e não uma recontagem fora da transação.
As duas leituras representam a mesma grandeza; divergência entre elas é
inconsistência a reconciliar, nunca vaga adicional.

Caso a capacidade seja atingida, o sistema rejeita o ingresso ordinário. Apenas
o professor responsável pela turma pode ultrapassar a capacidade por convite
nominal de exceção, registrando \texttt{exceder\_capacidade} e
\texttt{justificativa\_excecao} e revalidando a exceção na aceitação; o código
de turma não autoriza exceção. Um ingresso excepcional válido pode deixar
\texttt{qtd\_alunos > capacidade} sem invalidar a turma; o ingresso ordinário
permanece bloqueado até surgir vaga, sem expulsar membros. É proibido
\textbf{editar/reduzir} \texttt{capacidade} para valor menor que
\texttt{qtd\_alunos} (HQ-M11-001 = A), fail-closed. Removido anteriormente não
reingressa por código, apenas por convite explícito, e a remoção decrementa o
contador uma única vez, nunca abaixo de zero. O contrato completo de ingresso,
remoção, reingresso e arquivamento está na Seção~\ref{sec:regras-turmas-m11}.
\end{regra}
...
\campo{tipo}{ENUM}{edicao, moderacao, NOT NULL}
\campo{editado\_por}{INTEGER}{FK, NOT NULL}
\campo{motivo}{TEXT}{NULL; obrigatório quando tipo = 'moderacao'}
...
\begin{explicacao}
O campo \textit{editado\_por} registra o UID do operador (autor da edição ou moderador), e \textit{tipo} distingue \textit{edicao} de \textit{moderacao}. Normalmente é o Professor dono da turma, mas o Chefe Geral também pode moderar e remover posts em caráter excepcional de moderação institucional (Q13), caso em que \textit{motivo} é obrigatório e o evento é auditado. O \textit{id\_turma} não é armazenado nesta entidade 3FN porque pode ser obtido por \textit{Historico\_Posts\_Turma.id\_post} $\rightarrow$ \textit{Post.id\_turma}; se a consulta direta por turma for necessária no Firestore, esse vínculo poderá ser denormalizado na coleção de histórico. Contrato em Seção~\ref{sec:regras-posts-comentarios-m12-1}.
\end{explicacao}
...
\campo{tipo}{ENUM}{edicao, moderacao, NOT NULL}
\campo{novo\_texto}{TEXT}{NULL; obrigatório quando tipo = 'edicao'}
\campo{texto\_antigo}{TEXT}{NULL; obrigatório quando tipo = 'edicao'}
\campo{motivo}{TEXT}{NULL; obrigatório quando tipo = 'moderacao'}
\campo{editado\_em}{TIMESTAMP}{NOT NULL}
\campo{editado\_por}{INTEGER}{FK, NOT NULL}
\end{entidade}
\begin{explicacao}
Cada edição do autor ou moderação pelo professor dono/Chefe (Q13) gera um
evento imutável: \textit{tipo} distingue \textit{edicao} de \textit{moderacao};
edição registra texto antigo e novo; moderação registra motivo e operador. A
\textit{texto} corrente do Comentário não é apagada. Contrato em
Seção~\ref{sec:regras-posts-comentarios-m12-1}.
\end{explicacao}
...
\campo{storage\_path}{TEXT}{NOT NULL}
\campo{content\_type}{VARCHAR(100)}{NOT NULL}
\campo{tamanho\_bytes}{INTEGER}{CHECK (tamanho\_bytes > 0), NOT NULL}
\campo{owner\_uid}{INTEGER}{FK, NOT NULL}
\campo{geracao}{VARCHAR(120)}{NOT NULL}
\campo{criado\_em}{TIMESTAMP}{NOT NULL}
\campo{nome}{VARCHAR(150)}{NOT NULL}
\campo{descricao}{TEXT}{NOT NULL}
\campo{file\_url}{TEXT}{NULL; legado}
\end{entidade}
\begin{explicacao}
\textit{storage\_path} é a referência canônica ao objeto; \textit{file\_url} é
referência legada, sem valor de autorização nem credencial. \textit{owner\_uid}
coincide com \textit{id\_professor\_upload}. \textit{geracao} é a versão
validada e imutável do objeto, fixada antes do vínculo; \textit{content\_type} e
\textit{tamanho\_bytes} registram o formato e o tamanho validados (PDF
estritamente menor que 15 MiB). Uma URL de download é temporária e não se torna
credencial permanente. Contrato em
Seção~\ref{sec:regras-roteiros-m12-2}.
\end{explicacao}
...
\end{entidade}
\begin{explicacao}
A chave primária composta impede duplicar o compartilhamento do mesmo roteiro
para o mesmo professor. No Firestore a relação corresponde ao array
\texttt{professores\_compartilhados} em \textit{Roteiro\_Experimento}, gerido
exclusivamente pelo servidor. Revogação remove a relação e não reescreve Posts
históricos (Q09). Contrato em
Seção~\ref{sec:regras-roteiros-m12-2}.
\end{explicacao}
...
\noindent REQUISICAO\_ADICAO\_BEM e REQUISICAO\_EDICAO\_BEM notificam os gestores na abertura; REQUISICAO\_BEM notifica o Professor solicitante na resposta, aprovada ou rejeitada.
...
No Firestore, esta entidade é implementada como sub-coleção \texttt{Usuarios/\{uid\}/Notificacoes}, eliminando a necessidade de queries em múltiplas coleções para usuários multi-role. O \texttt{uid} do caminho coincide com \texttt{id\_destinatario} e há uma única caixa por conta; \textit{papel\_destinatario} é apenas contexto de apresentação e não concede papel nem filtra a própria caixa. A ação de ``Limpar tudo'' na interface marca \texttt{lida = true} e \texttt{lida\_em = NOW()} em todas as notificações ativas do usuário, por processamento paginado/reentrante, preservando o histórico de auditoria. Os campos \textit{entidade\_alvo} e \textit{id\_alvo} viabilizam a navegação direta (\textit{deep linking}) para o objeto de origem ao clicar no aviso, sempre com revalidação do acesso corrente. O campo \textit{expira\_em} é nulo para notificações operacionais persistentes (como \texttt{ESCASSEZ\_ESTOQUE}), que não possuem expiração temporal automática. A fonte normativa completa é a Seção~\ref{sec:regras-notificacoes-m13}.
\end{explicacao}
...
\campo{descricao}{TEXT}{NOT NULL}
\campo{criado\_em}{TIMESTAMP}{NOT NULL}
\campo{editado}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\campo{editado\_em}{TIMESTAMP}{NULL}
\campo{removido\_da\_apresentacao}{BOOLEAN}{DEFAULT FALSE, NOT NULL}
\campo{motivo\_remocao}{TEXT}{NULL; obrigatório quando removido\_da\_apresentacao}
\campo{removido\_por}{INTEGER}{FK, NULL}
\campo{removido\_em}{TIMESTAMP}{NULL}
\end{entidade}
\begin{explicacao}
``Remover da apresentação'' é operação lógica (soft): preserva o Post e seu
histórico (RF25), oculta o item do feed de colegas e o mantém acessível ao
professor dono, ao Chefe por Q13 e à auditoria. Não há exclusão física. A
edição preserva a versão anterior em \textit{Historico\_Posts\_Turma}. O
snapshot imutável \texttt{roteiro\_anexo} é projeção Firestore da fronteira
M12.2 e não substitui \textit{id\_roteiro\_experimento}. Contrato em
Seção~\ref{sec:regras-posts-comentarios-m12-1}.
\end{explicacao}
...
\campo{moderado\_em}{TIMESTAMP}{NULL; obrigatório quando moderado}
\end{entidade}
\begin{explicacao}
\textit{texto} guarda o conteúdo corrente (escapado na renderização) e a
edição preserva a versão anterior em \textit{Historico\_Comentario}. A
moderação não sobrescreve \textit{texto}: marca \textit{moderado = true} com
motivo, moderador e instante atômicos. A edição posterior pelo autor não desfaz
a moderação. Contrato em
Seção~\ref{sec:regras-posts-comentarios-m12-1}.
\end{explicacao}
...
\begin{itemize}
    ...
    \item id\_turma da notificação em tipos operacionais/patrimônio; obrigatório nos tipos acadêmicos (\texttt{COMENTARIO}, \texttt{POST}, \texttt{ADICIONADO}, \texttt{REMOVIDO}, \texttt{TURMA\_ARQUIVADA}, \texttt{TURMA\_DESARQUIVADA}), conforme a Seção~\ref{sec:regras-notificacoes-m13};
...
QUARENTENA & Exige detalhe de motivo e bloqueia nova retirada; não pode ser descartada diretamente.\\
QUARENTENA + decisão PENDENTE\_DE\_DESCARTE & Estado operacional derivado (não é coluna): \texttt{em\_quarentena = FALSE}, \texttt{INDISPONIVEL} e autorização técnica ativa; habilita \texttt{descartarFrasco}.\\
\texttt{situacao\_localizacao = EXTRAVIADO} & Não pode estar EMPRESTADO: empréstimo ativo é encerrado extraordinariamente; \texttt{disponibilidade = INDISPONIVEL}. Preserva saldo, \texttt{saldo\_desconhecido}, \texttt{peso\_atual}, tara, \texttt{vencido}, \texttt{abertura\_historica\_desconhecida} e o estado físico anterior. Extravio não é consumo, esgotamento nem peso zero; a decisão anterior permanece recuperável pela trilha histórica.\\
REENCONTRO & Novo fato físico, não desfaz o extravio. O estado constatado (\texttt{ABERTO}, \texttt{FECHADO}, \texttt{VAZIO} ou \texttt{QUEBRADO}) é validado contra o último estado conhecido antes do extravio (trilha histórica): \texttt{FECHADO} só se antes era \texttt{FECHADO} e sem abertura conhecida; \texttt{ABERTO} se antes era \texttt{ABERTO} ou \texttt{FECHADO} (neste caso marca \texttt{abertura\_historica\_desconhecida}); \texttt{VAZIO} só se antes era \texttt{VAZIO}; \texttt{QUEBRADO} representa quebra física constatada. Transições impossíveis (\texttt{ABERTO}/\texttt{VAZIO}/\texttt{QUEBRADO} $\rightarrow$ \texttt{FECHADO}, \texttt{VAZIO}/\texttt{QUEBRADO} $\rightarrow$ \texttt{ABERTO}) são rejeitadas. Sempre \texttt{em\_quarentena = TRUE} e \texttt{disponibilidade = INDISPONIVEL}. Não reabre empréstimo encerrado e não recalcula validade. \texttt{REENCONTRADO} $\neq$ \texttt{DISPONIVEL}.\\
QUARENTENA + \texttt{VOLTAR\_A\_DISPONIVEL} & Encerra a quarentena (\texttt{em\_quarentena = FALSE}, \texttt{disponibilidade = DISPONIVEL}) somente se o estado físico for \texttt{ABERTO}/\texttt{FECHADO}, sem pendência metrológica e sem \texttt{vencido} sem uso autorizado; não revalida validade. Nunca libera \texttt{VAZIO}, \texttt{QUEBRADO} ou \texttt{DESCARTADO}.\\
...
Devolução & Só pode ocorrer para empréstimo EM\_USO ou ATRASADO e aplica tolerância híbrida Q06: ganho tolerado produz consumo zero e evento AJUSTE; ganho anômalo (acima da tolerância) conclui em \texttt{DEVOLVIDO\_COM\_ANOMALIA} e quarentena compulsória, sem abortar a transação.\\
\bottomrule
\end{longtable}

\subsubsection{Fechamento dos estados automáticos}
\textit{vencido} é definido pelo servidor a partir da validade efetiva. Na abertura do frasco, \textit{validade\_efetiva} é recalculada conforme a regra de validade pós-abertura. \textit{ATRASADO} é promovido pelo job servidor quando a data de devolução prevista é ultrapassada. Nenhum desses estados pode ser inventado pelo frontend. O vencimento é recalculado sincronicamente no cadastro e na primeira abertura, conforme seus contratos próprios, para que o resultado não dependa de o job diário já ter executado. A \textbf{devolução} e o \textbf{reencontro}, ao contrário, apenas leem o estado persistido \textit{Frasco\_Reagente.vencido}: não recalculam vencimento pelo relógio, pois a passagem do tempo pertence ao job de vencimento (Seção~10.7).
...
A perda bruta é $\Delta_{bruto}=\max(0, peso_{saida}-peso_{retorno}^{efetivo})$ em g e é usada na detecção Q06; a evaporação é registrada separadamente do consumo e obedece a $0 \le \text{Evap} \le \Delta_{bruto}$; quando a constraint é satisfeita, $massa_{consumida}=\Delta_{bruto}-\text{Evap} \ge 0$. Evaporação acima da perda bruta é rejeitada (sem clamp). Medida utilizada é não negativa (g para sólido, mL via densidade para líquido). Medida usada no frasco acumula apenas gramas. Tolerância normal: $\max(1\,g,0{,}005\,peso_{saida})$; higroscópico: $\max(2\,g,0{,}02\,peso_{saida})$. A fórmula de tolerância incide estritamente sobre o \textbf{Peso Bruto de Saída ($peso_{saida}$)}. A tolerância existe para \textbf{detecção de anomalia} e tratamento próprio da ocorrência, e \textbf{não} é a autoridade física que define se o recipiente ficou vazio. Ganho dentro da tolerância preserva peso físico real, consumo zero e evento AJUSTE/ganho\_massa\_higroscopia --- restrito estritamente a frascos com \texttt{eh\_higroscopico = true}; é terminantemente proibido registrar tal evento para frascos não-higroscópicos ou com ganhos fora da tolerância legal. Ganho acima da tolerância ($peso_{retorno} > peso_{saida} + \Delta_{\max}$) \textbf{não aborta a operação via rollback}: o sistema conclui a devolução em modalidade extraordinária, transicionando o empréstimo para \texttt{DEVOLVIDO\_COM\_ANOMALIA} e o frasco compulsoriamente para \texttt{disponibilidade = INDISPONIVEL} e \texttt{em\_quarentena = true}, com \texttt{detalhe\_status} registrando suspeita de contaminação/adulteração. A quarentena é dimensão ortogonal: o bloqueio ocorre por \texttt{em\_quarentena = true} e \texttt{disponibilidade = INDISPONIVEL}, \textbf{nunca} setando \texttt{vencido = true}. A determinação de que o recipiente ficou vazio é confirmada \textbf{explicitamente pelo gestor} na devolução (decisão ``O frasco ficou vazio nesta devolução? [ ] Sim''), tanto para frascos originalmente \texttt{FECHADO} quanto \texttt{JA\_ABERTO}; a comparação com a tara anterior ou o enquadramento na tolerância \textbf{não} é autoridade exclusiva do esgotamento. Sem tara real anterior, ao confirmar o esgotamento, \textit{peso\_retorno} passa a ser a tara real: \texttt{peso\_frasco\_vazio = peso\_retorno}, \texttt{peso\_atual = peso\_retorno}, \texttt{estado\_fisico\_frasco = VAZIO}, \texttt{disponibilidade = INDISPONIVEL}, \texttt{saldo\_desconhecido = false}, com evento \texttt{FICOU\_VAZIO}; para frasco \texttt{JA\_ABERTO} esse é o primeiro valor real de tara conhecido, e o saldo passa a conhecido 0 g (sólido) ou 0 mL (líquido), sem alterar \textit{conteudo\_nominal}. Não existe recalibração parcial por estimativa: recalibrar tara exige recipiente efetivamente vazio com peso vazio real medido, justificativa humana e histórico auditável; não existe correção administrativa metrológica na V1. Tara real anterior divergente é preservada até recalibração auditada; vazio confirmado mantém saldo zero. Medição inconsistente encerra custódia, preserva peso observado e mantém consumo pendente, nunca negativo; ver contrato metrológico na Seção 10.
\end{regra}
\begin{regra}[Q14 e Q04: rastreabilidade na retirada]
Autoatendimento é calculado pelo servidor: só o único gestor ativo do almoxarifado pode retirar para si, com justificativa humana obrigatória de pelo menos 20 caracteres após trim e notificação auditável à chefia; se houver outro gestor ativo, bloquear. Para vencido ou validade desconhecida liberado com termo nas finalidades PESQUISA\_TCC\_POS ou ESTUDO\_DEGRADACAO\_RESIDUOS, exigir justificativa metodológica de 20--2000 caracteres, com mínimo verificado após trim, versão do TCR, instante servidor, UID autenticado do retirante que aceitou e referência ao Registro\_de\_Auditoria correspondente. O backend valida aceite vinculado ao frasco, finalidade e operação, nunca declaração do gestor em nome de terceiro. Sem essas condições, rejeitar. TCR registra ciência e responsabilidade, sem substituir regras institucionais de segurança química. Quarentena e pendência de descarte continuam bloqueadas.
\end{regra}
