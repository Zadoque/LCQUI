...
A devolução com anomalia encerra a custódia como \texttt{DEVOLVIDO\_COM\_ANOMALIA} e deixa a pendência quantitativa aberta (\texttt{consumo\_validado = FALSE}, \texttt{peso\_retorno\_efetivo = NULL}, \texttt{id\_resolucao\_metrologica = NULL}). A pendência é encerrada apenas por contratos tipados da Seção~10.5 (repetir pesagem; confirmar esgotamento após inspeção; recalibrar tara real quando aplicável; correção administrativa já suportada); não existe resolvedor público genérico. Resolver a pendência não libera automaticamente a quarentena, e \texttt{peso\_retorno} original nunca é apagado. A condição de pendência é \texttt{status = DEVOLVIDO\_COM\_ANOMALIA} e \texttt{consumo\_validado = FALSE}.
