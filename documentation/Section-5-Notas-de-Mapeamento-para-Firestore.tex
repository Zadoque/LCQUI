...
\item[Operações técnicas] \texttt{Operacoes/id}: \texttt{uid}, \texttt{tipo\_operacao}, \texttt{payload\_hash} (strings O, hash SHA-256 canônico do payload), \texttt{status} (PENDENTE/CONCLUIDA/FALHOU), \texttt{resultado} (map N), \texttt{criado\_em}, \texttt{atualizado\_em} (Timestamp O). Chave é vinculada ao ator/ação e payload; repetição com entrada diferente é rejeitada com ALREADY\_EXISTS (idempotência semântica rigorosa). Leitura restrita ao solicitante por resposta mínima; escrita servidor.
