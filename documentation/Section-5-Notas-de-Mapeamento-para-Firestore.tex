...
\subsection{Depreciação proposta de \textit{letra\_inicial} em Reagentes}
\textit{letra\_inicial} de \textit{Resumo\_Reagente} foi reintroduzido como
denormalização para viabilizar um filtro de igualdade na antiga estratégia de
busca no Firestore. A estratégia vigente de reagentes é a busca em memória sobre
o catálogo JSON materializado (Seção~\ref{sec:catalogo-json-reagentes}): o catálogo inteiro é carregado no
cliente e filtrado localmente, sem consulta Firestore por tecla e sem filtro
prévio obrigatório. Portanto \textit{letra\_inicial} \textbf{não é requisito da
busca atual} e não deve ser mantido apenas para viabilizá-la.

\textbf{Auditoria de referências (prova).} Fora de Reagentes,
\textit{letra\_inicial} aparece em \textit{Aluno} e \textit{letra\_inicial\_nome}
em \textit{Bem\_Patrimonial}; ambos são estratégias próprias de busca e não são
afetados. Em Reagentes, as referências remanescentes são esta subseção, o quadro
de coleções, o complemento físico ``Reagentes'' e a nota gerada do mapeamento
CUE de Firestore. Nenhuma obrigação de produto, autorização, integridade de
referência ou índice composto depende do campo apenas para Reagentes.

\textbf{Decisão desta rodada.} Registrar a depreciação proposta e manter o campo
e o mapeamento existentes como \textbf{legado}, sem tratá-los como requisito da
busca JSON e sem acrescentar índice para esse caso. A remoção da exigência de
persistência e geração do campo da Seção~5 e do mapeamento CUE de Firestore só
ocorre em rodada formal separada, porque altera o baseline (CUE/IR/hashes).
Nenhum dado de produção é removido por esta rodada.

\begin{regra}[Consistência enquanto o campo existir]
Enquanto o campo permanecer como legado, \textit{letra\_inicial} continua
recalculado e reescrito pela mesma \textit{Cloud Function} que grava
\textit{nome}, nunca aceito diretamente do cliente -- ver princípio ``nunca
confiar no \textit{client-side}'' na seção \textit{Tecnologia e Relatórios}.
\end{regra}
...
\subsection{Unicidade canônica e permanente de \texttt{numero\_patrimonio} no Firestore}
O PostgreSQL implementa a regra RF10b via índice único parcial
\texttt{UNIQUE(numero\_patrimonio\_normalizado) WHERE status = 'pendente'},
...
A canonicalização única é \texttt{normalizarNumeroPatrimonio(s) =
s.trim().toUpperCase()}. O resultado não pode ser vazio, deve respeitar o limite
de 30 caracteres e é armazenado como \texttt{numero\_patrimonio} no Bem e como
\texttt{numero\_patrimonio\_normalizado} na requisição. Lock, consulta exata e
documento permanente \texttt{Chaves\_Unicas/Bem\_Patrimonial\_\_\{numero\}} usam
exclusivamente essa chave. A criação lê lock e chave permanente e grava lock e
requisição na mesma transação. A aprovação relê a chave e, no mesmo commit,
grava Bem, chave, resposta, histórico de cadastro e liberação do lock. A chave
não é apagada na baixa; o lock sim termina com a requisição. Uma consulta prévia
pode melhorar a mensagem de erro, mas não substitui a chave de unicidade. Locks
vinculados a requisições pendentes não expiram por idade.
...
\begin{enumerate}
    ...
    \item \texttt{Frasco\_Reagente}: \texttt{id\_almoxarifado (ASC)} + \texttt{id\_resumo\_reagente (ASC)} +\newline
    \texttt{id\_especificacao\_reagente (ASC)} + \texttt{situacao\_localizacao (ASC)} +\newline
    \texttt{disponibilidade (ASC)} + \texttt{estado\_fisico\_frasco (ASC)} +\newline
    \texttt{em\_quarentena (ASC)} + \texttt{vencido (ASC)} + \texttt{uso\_vencido\_autorizado (ASC)}\newline
    -- suporta a agregação de estoque atual e o job agendado de escassez (aptidão inclui vencido com uso autorizado).
...
Resumo\_Reagente & Coleção raiz. Denorm: \texttt{letra\_inicial} (legado) & Catálogo de reagentes; a busca vigente é em memória sobre o catálogo JSON (Seção~\ref{sec:catalogo-json-reagentes}) e não exige \texttt{letra\_inicial}. Campo mantido como legado; depreciação proposta em rodada formal própria.\\
...
Turma & Coleção raiz. Denorm: \texttt{nome\_materia} & Acesso direto por \texttt{codigo\_turma} (único, gerado no servidor e reservado em \texttt{Chaves\_Unicas/Turma\_\_\{codigo\}}); nome da matéria necessário na listagem sem JOIN. \texttt{qtd\_alunos} é a autoridade transacional de vaga; não recontar \texttt{Alunos} fora da transação.\\
...
Convite\_Aluno & Coleção raiz & Unicidade transacional de convite \texttt{pendente} por (e-mail normalizado, \texttt{id\_turma}), inclusive a chave global com \texttt{id\_turma} NULL, validada em \texttt{Chaves\_Unicas}; docId determinístico por HMAC, sem e-mail em claro.\\
...
\item[\texttt{ativo}] boolean; O. Estado autoritativo da conta. \texttt{false} nega toda autorização, mesmo com token ainda válido ou claim antiga.
\item[\texttt{versao\_permissoes}] inteiro não negativo; O. Incrementado na mesma transação de toda alteração de papel, vínculo ou estado autorizativo; é a barreira contra claim/token obsoleto nas leituras diretas permitidas.
\item[\texttt{claims\_pendentes}] boolean; O. Indica que a projeção no Firebase Auth ainda precisa ser reconciliada; nunca é autorização transitória.
\item[\texttt{atualizado\_em}] Timestamp; O. Instante do servidor da última alteração autorizativa.
\end{description}
...
\item[\texttt{numero\_patrimonio}] string; O. Plaqueta institucional canônica, preservando zeros iniciais; é \texttt{trim().toUpperCase()} da entrada e identifica unicamente a unidade física. Máximo 30 caracteres.
...
\item[\texttt{versao}] number; O. Inteiro iniciado em 1 para controle otimista de fatos canônicos da unidade. Fan-out puramente derivado não o incrementa.
\end{description}
...
\item[\texttt{numero\_patrimonio\_normalizado}] string; O. \texttt{trim().toUpperCase()} da plaqueta proposta; é a única chave usada em lock, \texttt{Chaves\_Unicas}, comparação e aprovação.
\item[\texttt{status}] string enum; O. Estado controlado pelo domínio; transições são validadas no servidor. Valores: pendente, aprovada, rejeitada.
\item[\texttt{feita\_em}] Timestamp; O. Data/instante de feita em, com ordem temporal validada e autoria do relógio do servidor para eventos.
...
\item[\texttt{id\_requisicao}] string; O. Requisição pendente proprietária do lock; precisa coincidir antes da liberação.
\item[\texttt{tipo}] string enum; O. EDICAO ou ADICAO; precisa coincidir com o caminho de resposta.
\item[\texttt{chave\_recurso}] string; O. ID do bem para edição ou plaqueta normalizada para adição.
...
\item[\texttt{tipo}] string enum; O. edicao ou moderacao; distingue a natureza do evento histórico.
\item[\texttt{motivo}] string; C quando tipo = moderacao, até 2000 caracteres não vazios; obrigatório para moderação.
...
\item[\texttt{tipo}] string enum; O. edicao ou moderacao; distingue o evento histórico.
\item[\texttt{novo\_texto}] string; C quando tipo = edicao. Texto resultante da edição do autor.
\item[\texttt{texto\_antigo}] string; C quando tipo = edicao. Texto anterior preservado; nunca apagado pela moderação.
\item[\texttt{motivo}] string; C quando tipo = moderacao, até 2000 caracteres não vazios.
...
\begin{description}
\item[\texttt{id}] docId string; O. Chave do documento, gerada pelo servidor; não duplicar como contador relacional.
\item[\texttt{id\_professor\_upload}] string; O. Dono imutável; existência e escopo verificados no servidor.
\item[\texttt{storage\_path}] string; O. Referência canônica ao objeto no bucket; nunca exposta como credencial. O download usa URL temporária emitida por endpoint autorizado.
\item[\texttt{content\_type}] string; O. Formato validado (PDF).
\item[\texttt{tamanho\_bytes}] inteiro positivo; O. Tamanho validado (PDF estritamente menor que 15 MiB).
\item[\texttt{owner\_uid}] string; O. Titular do objeto; coincide com \texttt{id\_professor\_upload}.
\item[\texttt{geracao}] string; O. Geração/versão validada e imutável do objeto; impede troca/sobrescrita entre validação e vínculo.
\item[\texttt{criado\_em}] Timestamp; O. Instante do servidor da criação do Roteiro publicável.
\item[\texttt{nome}] string; O. Nome de apresentação; não substitui a chave do documento. Máximo 150 caracteres.
\item[\texttt{descricao}] string; O. Valor de descricao, validado pelo formulário e pelo servidor conforme regras do domínio.
\item[\texttt{file\_url}] string; N. Referência legada; não é autoridade, credencial nem URL pública permanente. Não usar como fallback de download.
\item[\texttt{professores\_compartilhados}] array de strings; O, padrão vazio. UIDs autorizados, geridos exclusivamente pelo servidor com arrayUnion/arrayRemove; sem escrita direta cliente. Relação única por (roteiro, professor); revogação remove o UID e preserva Posts históricos (Q09).
\end{description}
...
\noindent O \texttt{uid} do caminho coincide com \texttt{id\_destinatario} e há uma única caixa por conta; \textit{papel\_destinatario} é contexto de apresentação sem conceder papel nem filtrar a própria caixa. A leitura direta é restrita ao próprio UID; Rules autorizam ou negam documentos completos e \textbf{não mascaram campos}, portanto o payload é mínimo, sem conteúdo protegido, e a projeção segura de conteúdo é responsabilidade de endpoint autorizado. Contrato completo em Seção~\ref{sec:regras-notificacoes-m13}.
...
\item[\texttt{descricao}] string; O. Valor de descricao, validado pelo formulário e pelo servidor conforme regras do domínio.
\item[\texttt{criado\_em}] Timestamp; O. Data/instante de criado em, com ordem temporal validada e autoria do relógio do servidor para eventos.
\item[\texttt{editado}] boolean; O, padrão false. Marcado quando o autor edita o Post; a versão anterior fica em HistoricoPosts.
\item[\texttt{editado\_em}] Timestamp; N. Instante do servidor da última edição; preenchido atomicamente com HistoricoPosts e \texttt{editado}.
\item[\texttt{removido\_da\_apresentacao}] boolean; O, padrão false. Remoção lógica (soft) que oculta o Post do feed de colegas sem apagar documento ou histórico (RF25).
\item[\texttt{motivo\_remocao}] string; C quando removido\_da\_apresentacao, até 2000 caracteres não vazios.
\item[\texttt{removido\_por}] string; C quando removido\_da\_apresentacao. UID do professor dono ou do Chefe (Q13), preenchido pelo servidor.
\item[\texttt{removido\_em}] Timestamp; C quando removido\_da\_apresentacao. Instante do servidor gravado atomicamente.
\item[\texttt{roteiro\_anexo}] map; N. Snapshot imutável da publicação: id\_roteiro (string O), nome\_arquivo (string O, até 150), tamanho\_bytes (inteiro positivo O), storage\_path (string O), \texttt{geracao} (string O, geração validada do objeto). Backend valida acesso ao roteiro antes de publicar. Revogação posterior da ACL não remove o anexo histórico. Aluno recebe os metadados no feed; download via obterUrlDownloadRoteiro valida vínculo atual e emite URL assinada por 15 minutos, sem leitura cliente da coleção raiz. Fronteira formal de M12.2 (Seção~\ref{sec:regras-roteiros-m12-2}); a presença de \texttt{geracao} é exigida pela regra RN-M13-08 (Seção~\ref{sec:regras-notificacoes-m13}).
\end{description}
...
\item[\texttt{criado\_em}] Timestamp; O. Data/instante de criado em, com ordem temporal validada e autoria do relógio do servidor para eventos.
\item[\texttt{editado}] boolean; O, padrão false. Marcado quando o autor edita o Comentário; a versão anterior fica em HistoricoComentarios.
\item[\texttt{editado\_em}] Timestamp; N. Instante do servidor da última edição do autor; a edição não desfaz moderação.
...
\begin{description}
\item[Usuarios] \texttt{ativo} (boolean, O), \texttt{versao\_permissoes} (inteiro, O), \texttt{claims\_pendentes} (boolean, O), \texttt{atualizado\_em} (Timestamp, O): controle servidor de ativação e sincronização de autorização. Não replicar papéis editáveis no navegador. A autorização crítica consulta este documento e as relações persistidas na mesma transação do efeito (Seção~7/M9).
...
\item[Turma] \texttt{qtd\_alunos} (inteiro não negativo, O) é contador transacional do vínculo ativo, não count lido fora da transação. \texttt{versao} (inteiro positivo, O) permite detectar edições concorrentes. \texttt{codigo\_turma} é único e reservado na mesma transação em \texttt{Chaves\_Unicas/Turma\_\_\{codigo\}}; arquivar não libera o código. Arquivar preserva vínculos, posts e histórico e bloqueia escritas incompatíveis (Q08).
...
\item[Reagentes] O estado físico escalar e a higroscopicidade são canônicos no resumo. A descoberta e a listagem de Resumos e Especificações usam o catálogo JSON materializado (Seção~\ref{sec:catalogo-json-reagentes}): busca textual e filtros de catálogo ocorrem em memória sobre o catálogo inteiro, sem consulta Firestore por tecla e sem filtro prévio obrigatório. \texttt{letra\_inicial} é legado e não é requisito dessa busca. Especificação embute \texttt{composicao} (array de maps com os campos da composição; IDs de substâncias únicos). O resumo pai fornece seu ID; manter ID de resumo em Lote e Frasco para resolver o caminho completo da especificação.
...
\item[Requisições] Adição inclui \texttt{justificativa\_resposta} (string N, obrigatória após resposta). Edição inclui \texttt{versao\_bem\_origem} (inteiro O), \texttt{novo\_nome}, \texttt{novo\_status}, \texttt{novo\_estado\_conservacao}, \texttt{novo\_id\_local}, \texttt{nova\_photo\_url} e \texttt{novo\_id\_resumo\_bem\_patrimonial} (opcionais canônicos; o último para reclassificação). Baixa não pertence à requisição de edição: segue a operação própria RF12/M10, de Bem Inservível para \texttt{Ja\_dado\_baixa}, com rito SEI e comprovante PDF. Projeções nome/plaqueta são identificadores de leitura, não autorização. Resposta nunca inventa foto ausente.
...
\begin{explicacao}[Busca de reagentes sobre a projeção JSON]
O Firestore não oferece busca por substring (sem \texttt{LIKE}, sem
\texttt{icontains}). Por isso, a descoberta e a listagem de
\textit{Resumo\_Reagente} e \textit{Especificacao\_Reagente} usam o catálogo JSON
materializado descrito na Seção~\ref{sec:catalogo-json-reagentes}: \textbf{Firestore permanece a fonte
canônica}; o \textbf{catálogo JSON publicado é uma projeção autorizada} de
busca/apresentação; o \textbf{cache local é descartável}. A busca textual e os
filtros de catálogo ocorrem \textbf{em memória sobre o catálogo inteiro}, sem
consulta Firestore por tecla e sem filtro prévio obrigatório. As estratégias de
Bens Patrimoniais, Alunos, Professores e demais domínios são independentes e
seguem as subseções seguintes; os modos exatos de busca por código (LCQUI-N, CAS,
matrícula, plaqueta) e a política de fallback por ID permanecem os da Seção~\ref{sec:catalogo-json-reagentes}.
\end{explicacao}

\subsubsection{Reagentes}
A busca de reagentes é feita sobre o catálogo JSON materializado (Seção~\ref{sec:catalogo-json-reagentes}),
carregado integralmente no cliente (ou servido do cache local quando a versão
publicada não mudou). A pesquisa pode começar a partir do primeiro caractere,
sem filtro prévio obrigatório e sem consulta Firestore por tecla; os filtros de
estado físico, natureza química, tipo de substância e afins são \textbf{opções
do cliente}, aplicadas em memória, jamais pré-condições de consulta. A busca é
tolerante a caixa e acentos e preserva símbolos e dígitos: \texttt{dioxano} e
\texttt{1,4} encontram \texttt{1,4-Dioxano}. Não se promete equivalência entre
\texttt{$\alpha$} e \texttt{alpha} nem remoção geral de pontuação sem contrato e
testes próprios; nenhuma normalização pode descartar prefixos químicos nem
alterar a identidade do nome.

\textbf{Ordenação e navegação por inicial.} A lista inicial é apresentada
ordenada: os nomes são comparados com o collator
\texttt{Intl.Collator('pt-BR',} \texttt{\{numeric:true,}
\texttt{sensitivity:'base'\})} e o ID desempata. Não se afirma uma
posição universal de símbolos ou dígitos pelo collator. A navegação por inicial
é opcional e derivada do nome: \texttt{A--Z} quando a primeira unidade
significativa normalizada (após \textit{trim}, caixa e dobra de acentos) for uma
letra latina A--Z; caso contrário a categoria explícita \texttt{\#}. Exemplos:
\texttt{1,4-Dioxano}, \texttt{2-Propanol}, \texttt{$\alpha$-D-Glicose},
\texttt{$\beta$-Mercaptoetanol} e \texttt{(+)-Cânfora} pertencem a \texttt{\#};
\texttt{o-Xileno} pertence a \texttt{O}. \texttt{A--Z} e \texttt{\#} são apenas
navegação derivada do nome, nunca campo obrigatório de \textit{Resumo\_Reagente}
no Firestore. Nomes vazios ou inválidos não entram na navegação, coerentes com a
validação de cadastro que já os rejeita.

\textbf{Janela de renderização.} A apresentação inicial renderiza até 50
resultados, com expansão progressiva por rolagem (ou virtualização, como opção
de implementação; para a escala da V1 uma lista progressiva simples basta). O
limite de 50 é \textbf{janela de DOM}, nunca limite do catálogo: toda a pesquisa,
ordenação e filtros consideram o catálogo inteiro em memória, e nunca se filtra
apenas os primeiros 50 nem se confunde tamanho de DOM com custo Firestore. Ao
mudar query, filtro ou versão publicada, a janela reinicia; os estados de
carregamento, erro e versão antiga claramente identificada são preservados.
Zero correspondências significa ausência no snapshot publicado ou no cache
disponível, não prova de ausência na fonte canônica.
