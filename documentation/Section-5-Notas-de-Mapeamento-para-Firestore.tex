...
A coleção \texttt{Contador\_Codigo\_Frasco} não existe no modelo 3FN porque bancos relacionais possuem primitivas de \texttt{SEQUENCE/SERIAL}. Para gerar o identificador atômico \texttt{LCQUI-X}, utiliza-se o documento singleton \texttt{Contador\_Codigo\_Frasco/singleton} com o campo \texttt{ultimo\_codigo\_gerado}, operado atomicamente via \texttt{runTransaction} no backend. Isso garante unicidade estrita sem limites pesados de cota, visto que o \textit{throughput} manual da bancada é perfeitamente atendido pelo limite de 1 \textit{write/s} do Firestore.
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \textbf{\nolinkurl{Turmas/\{turmaId\}/Alunos/\{alunoId\}} (Visão da Turma):} Utilizada pelo professor para gerenciar os matriculados e validar a regra de capacidade máxima (\texttt{COUNT < Turma.capacidade}).
