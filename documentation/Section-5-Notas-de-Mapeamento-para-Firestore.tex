...
\item[\texttt{saldo\_desconhecido}] boolean; O, padrão false. Desconhecimento \textbf{atual} da quantidade remanescente; não é flag histórica. Frasco \texttt{JA\_ABERTO} inicia com true; frasco \texttt{FECHADO} e depois normalmente aberto passa a false; esvaziado com tara real medida passa a false. VAZIO, QUEBRADO e DESCARTADO não mantêm desconhecimento; EXTRAVIADO e quarentena preservam o valor anterior. True não soma em \texttt{volume\_total\_disponivel\_aferido\_ml} nem em \texttt{massa\_total\_disponivel\_aferida\_g}, contabilizando-se apenas em \texttt{qtd\_frascos\_saldo\_desconhecido}.
\item[\texttt{condicao\_inicial\_cadastro}] string enum; O, imutável. Fato histórico do ingresso no controle do LCQUI: \texttt{FECHADO} (recipiente entrou fisicamente fechado) ou \texttt{JA\_ABERTO} (já estava aberto). Não muda depois e não se confunde com \texttt{saldo\_desconhecido} (propriedade metrológica atual). \texttt{JA\_ABERTO} implica estado ABERTO, tara NULL e saldo desconhecido; \texttt{FECHADO} implica saldo conhecido e tara apenas derivada enquanto há produto.
\end{description}
...
\item[\texttt{qtd\_frascos\_saldo\_desconhecido}] number; O. Contagem de frascos com saldo desconhecido, inteira não negativa; não editável pelo cliente quando materializada.
...
\item[\texttt{volume\_total\_disponivel\_aferido\_ml}] number; O. Métrica de volume total disponivel aferido ml, não negativa e calculada a partir dos eventos/saldos do domínio.
...
\item[\texttt{massa\_total\_disponivel\_aferida\_g}] number; O. Métrica de massa total disponivel aferida g, não negativa e calculada a partir dos eventos/saldos do domínio.
