...
\subsection{Projeção dos estados físicos do catálogo}
A fonte de estado físico é cada especificação. Um resumo com várias especificações não pode escolher arbitrariamente um único estado: manter \texttt{estados\_fisicos} como conjunto derivado e filtrar por pertencimento. A projeção é mantida pelo servidor e não altera a modelagem 3FN.
...
Resumo\_Reagente & Coleção raiz. Denorm: \texttt{letra\_inicial}, \texttt{estados\_fisicos} & Catálogo de reagentes; filtros de igualdade combinados exigem ambos os campos como propriedades do documento.\\
...
\item[\texttt{photo\_url\_proposta}] string; N. Valor de photo url proposta, validado pelo formulário e pelo servidor conforme regras do domínio.
...
\item[\texttt{qtd\_em\_que\_e\_considerado\_escasso}] number; O. Limiar inteiro positivo de quantidade de frascos, usado para alerta de escassez do resumo.
...
\item[\texttt{valor\_composicao}] number; N. Valor numérico da concentração quando conhecido; alternativa à faixa.
...
\item[\texttt{unidade}] string; N. Notação textual legada de concentração/faixa; ver Q03. Máximo 20 caracteres.
\end{description}
...
\item[\texttt{estado\_fisico}] string enum; O. Estado da especificação; SOLIDO usa g e LIQUIDO usa ml. Valores: SOLIDO,LIQUIDO.
\item[\texttt{unidade\_de\_medida}] string enum; O. Valor de unidade de medida, validado pelo formulário e pelo servidor conforme regras do domínio. Valores: ml,g.
...
\item[\texttt{id\_especificacao\_reagente}] string; N. Identificador da entidade especificacao reagente, cuja existência e escopo são verificados no servidor.
...
\item[\texttt{conteudo\_nominal}] number; O. Conteúdo inicial declarado em ml ou g conforme especificação.
...
\item[\texttt{estado\_fisico\_frasco}] string enum; O. Condição do recipiente/conteúdo; quarentena e vencimento são dimensões separadas. Valores: FECHADO,ABERTO,VAZIO,QUEBRADO,DESCARTADO.
...
\item[\texttt{uso\_vencido\_autorizado}] boolean; O. Decisão expressa de gestor para uso exclusivamente didático/demonstração, auditada.
...
\item[\texttt{unidade\_medida\_ajustada}] string enum; O. Valor de unidade medida ajustada, validado pelo formulário e pelo servidor conforme regras do domínio. Valores: ml,g.
\item[\texttt{timestamp}] Timestamp; O. Data/instante de timestamp, com ordem temporal validada e autoria do relógio do servidor para eventos.
\end{description}
...
\item[\texttt{status}] string enum; O. Estado controlado pelo domínio; transições são validadas no servidor. Valores: EM\_USO,ATRASADO,DEVOLVIDO,DEVOLVIDO\_COM\_ATRASO.
...
\item[\texttt{medida\_utilizada}] number; N. Consumo calculado na unidade do empréstimo.
...
\item[\texttt{finalidade\_uso}] string enum; O. Finalidade declarada na retirada; vencido autorizado só admite DIDATICO\_DEMONSTRACAO. Valores: PESQUISA, DIDATICO\_DEMONSTRACAO, OUTRO.
\end{description}
...
\paragraph{Roteiro\_Professor\_Compartilhado}\mbox{}\par
Caminho: \texttt{Roteiro\_Professor\_Compartilhado/\{id\}}.
\begin{description}
\item[\texttt{id\_roteiro\_experimento}] string; O. Identificador da entidade roteiro experimento, cuja existência e escopo são verificados no servidor.
\item[\texttt{id\_professor}] string; O. Identificador da entidade professor, cuja existência e escopo são verificados no servidor.
\end{description}
...
\item[\texttt{papel\_destinatario}] string enum; O. Contexto de exibição da notificação, sem conceder esse papel ao usuário.
\item[\texttt{tipo}] string enum; O. Valor de tipo, validado pelo formulário e pelo servidor conforme regras do domínio.
...
\item[Reagentes] Resumo possui \texttt{letra\_inicial} (string O, derivada do nome). Como várias especificações podem diferir em estado, substituir cópia escalar ambígua por \texttt{estados\_fisicos} (array único de SOLIDO/LIQUIDO, derivado); filtro usa array-contains. Especificação embute \texttt{composicao} (array de maps com os campos da composição; IDs de substâncias únicos). O resumo pai fornece seu ID; manter ID de resumo em Lote e Frasco para resolver o caminho completo da especificação.
