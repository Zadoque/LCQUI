...
\begin{itemize}
    \item \textbf{Frascos Extraviados:} Frascos com \texttt{situacao\_localizacao = EXTRAVIADO} \textbf{nunca} são contabilizados no saldo disponível, seja na quantidade de frascos ou na massa/volume total.
...
Para permitir \texttt{sum()}, a projeção Firestore do próprio frasco possui \texttt{saldo\_aferido\_g} e \texttt{saldo\_aferido\_ml}, derivados server-owned. O backend calcula e atualiza esses campos na mesma transação das alterações de peso, tara ou conhecimento: saldo desconhecido ou metrologicamente inconsistente produz NULL; vazio explicitamente confirmado produz zero; caso consistente usa peso menos tara e, para líquidos, divisão pela densidade válida. A origem da tara permanece explícita. Sólidos têm campo mL NULL. As somas de estoque utilizável filtram o predicado único de aptidão \texttt{frascoAptoParaUso} definido na Seção~\ref{sec:m8-estoque-escassez-notificacoes} (\texttt{situacao\_localizacao = LOCALIZADO}, estado FECHADO/ABERTO, disponível, sem quarentena, sem pendência metrológica e respeitando validade/autorização); contagens totais, emprestados e desconhecidos usam filtros próprios identificados. Nenhuma quantidade desconhecida é apresentada como zero conhecido. Massa (g) e volume (mL) nunca são somados entre si.
...
\paragraph{Notificações não são cache nem materialização.}
A caixa de notificações é um conjunto de fatos persistidos com estado de leitura
próprio, não uma projeção descartável. Cache, resumo ou listagem do sino nunca
substitui a autorização corrente do objeto de origem nem pode devolver conteúdo
que o usuário deixou de poder acessar. A validade de $30$~segundos de
\texttt{Sistema\_Cache\_Dashboard} e o alerta de escassez são contratos
distintos (Seção~\ref{sec:regras-notificacoes-m13}).
