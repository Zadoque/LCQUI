...
\campo{qtd\_frascos\_saldo\_desconhecido}{INTEGER}{DEFAULT 0, NOT NULL}
...
% N-07: campo abaixo é LEGADO. Redundante e ambíguo frente às colunas dimensionadas.
% Não usar em novas queries. Utilizar volume\_utilizado\_no\_dia\_ml (líquidos)
% e massa\_utilizada\_no\_dia\_g (sólidos). Descontinuar na próxima migração.
\campo{volume\_total\_usado\_nos\_frascos\_devolvidos\_durante\_o\_dia}{NUMERIC(12,2)}{DEFAULT 0, NOT NULL -- LEGADO N-07}
\campo{volume\_total\_disponivel\_aferido\_ml}{NUMERIC(12,2)}{DEFAULT 0, NOT NULL}
...
\campo{massa\_total\_disponivel\_aferida\_g}{NUMERIC(12,2)}{DEFAULT 0, NOT NULL}
...
\textbf{N-07 -- campo legado:} \texttt{volume\_total\_usado\_nos\_frascos\_devolvidos\_durante\_o\_dia} pré-existe à separação massa vs. volume e mistura unidades. Sua semântica é coberta por \texttt{volume\_utilizado\_no\_dia\_ml} e \texttt{massa\_utilizada\_no\_dia\_g}. Não preencher em novos agregadores; manter apenas para compatibilidade de relatórios históricos até a migração.
\end{explicacao}

\begin{explicacao}[Projeção Derivada, Unicidade Determinística e Snapshot Temporal]
...
\textbf{Snapshot temporal.} \textit{DATA\_ALVO} = \texttt{YYYY-MM-DD} no fuso \texttt{America/Sao\_Paulo}; a janela considerada é \texttt{00:00:00.000}–\texttt{23:59:59.999}. O snapshot é o \textbf{estado efetivo no corte 23:59:59.999}; eventos ocorridos após a virada da meia-noite \textbf{não} contaminam o fechamento do dia anterior. É \textbf{proibido} reconstruir o histórico a partir do estado atual do documento: um job executado após a meia-noite não pode ler \textit{Frasco\_Reagente} atual e assumir que aquele era o estado no corte.
...
\begin{explicacao}[Contratos de Materialização e Saldo]
Para garantir a exatidão das métricas agregadas e evitar divergências com a Seção 4:
\begin{itemize}
...
\textbf{Snapshot temporal.} \textit{DATA\_ALVO} = \texttt{YYYY-MM-DD} no fuso \texttt{America/Sao\_Paulo}; a janela considerada é \texttt{00:00:00.000}–\texttt{23:59:59.999}. O snapshot é o \textbf{estado efetivo no corte 23:59:59.999}; eventos após a virada da meia-noite \textbf{não} contaminam o fechamento anterior. É \textbf{proibido} reconstruir o histórico consultando o estado atual de \textit{Frasco\_Reagente} após a virada.
