...
export const aceitarConviteAluno = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (convite.id_turma) {
      ...
      // convite.exceder_capacidade não existe no schema, mas se houvesse flag no convite, seria verificado aqui.
      // Assumindo limite estrito:
      if (alunosSnap.size >= turma.capacidade) {
        throw new HttpsError("resource-exhausted", "Turma lotada.");
      }

      const vincRef = turmaRef.collection("Alunos").doc(request.auth!.uid);
      const vincRef2 = admin.firestore()
        .collection("Usuarios").doc(request.auth!.uid)
        .collection("Turmas").doc(convite.id_turma);
      
      const agora = admin.firestore.FieldValue.serverTimestamp();
      tx.set(vincRef, { id_aluno: request.auth!.uid, ingressou_em: agora });
      tx.set(vincRef2, { 
