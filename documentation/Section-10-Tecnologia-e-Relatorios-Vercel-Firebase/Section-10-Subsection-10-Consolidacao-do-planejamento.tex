...
export const aceitarConviteAluno = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (!alunoSnap.exists) {
      matriculaFinal = convite.numero_matricula || matriculaInformada;
      if (!matriculaFinal) {
        throw new HttpsError("invalid-argument", "Matricula e obrigatoria para a criacao do aluno e nao consta no convite.");
      }
      const matriculaNorm = matriculaFinal.trim().toUpperCase();
      lockRefMatricula = admin.firestore().collection("Chaves_Unicas").doc(`Aluno__${matriculaNorm}`);
      const lockSnap = await tx.get(lockRefMatricula);
      if (lockSnap.exists) throw new HttpsError("already-exists", "A matricula informada ja esta em uso por outro aluno.");
    }
