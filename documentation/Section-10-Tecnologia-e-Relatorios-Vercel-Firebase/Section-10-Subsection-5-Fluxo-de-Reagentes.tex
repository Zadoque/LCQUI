...
export const registrarDevolucao = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    const esgotado = diferencaTara > 0 && diferencaTara <= 5
      && dados.confirmarEsgotamento === true;
    if (diferencaTara > 0 && !esgotado) {
      throw new HttpsError("failed-precondition", "Requer recalibrarTaraFrascoEsgotado: diferença superior a 5 g ou esgotamento não confirmado.");
    }
    ...
    const atualizacaoFrasco: Record<string, unknown> = {
      disponibilidade: esgotado ? "INDISPONIVEL" : "DISPONIVEL",
