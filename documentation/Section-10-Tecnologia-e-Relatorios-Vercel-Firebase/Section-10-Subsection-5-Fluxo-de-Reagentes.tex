...
interface RetiradaFrasco {
  ...
  tcrFisicoAssinado?: boolean;
  idTermoUpload?: string;
...
export const registrarRetirada = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    tx.set(emprestimoRef, {
      ...
      tcr_versao: aceite?.versao ?? (dados.tcrFisicoAssinado ? "1.0" : null),
      tcr_aceito_em: aceite?.aceito_em ?? (dados.tcrFisicoAssinado ? admin.firestore.FieldValue.serverTimestamp() : null),
      tcr_aceito_por: aceite?.uid ?? (dados.tcrFisicoAssinado ? dados.idUsuarioRetirou : null),
      tcr_auditoria_id: aceite?.auditoria_id ?? (dados.idTermoUpload ?? null),
...
export const registrarDevolucao = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (esgotado) pesoConsumido = Math.max(0, emprestimo.peso_saida - frasco.peso_frasco_vazio);
...
export const registrarExtravioOuReencontro = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (dados.acao === "EXTRAVIAR") {
      ...
      if (dados.estadoConstatadoAoReencontrar === "ABERTO" && frasco.estado_fisico_frasco === "FECHADO") {
