...
export const registrarRetirada = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    tx.set(emprestimoRef, {
      ...
      tcr_versao: aceite?.versao ?? (dados.tcrFisicoAssinado ? 1 : null),
...
export const registrarDevolucao = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (diferencaTara > 0 && !esgotado) {
      throw new HttpsError("failed-precondition", "Requer confirmação de esgotamento e/ou motivo de recalibração para diferenças superiores a 5 g.");
    }
    ...
    const atualizacaoFrasco: Record<string, unknown> = {
      ...
      ...(esgotado && { estado_fisico_frasco: "VAZIO" }),
      data_ultima_pesagem: admin.firestore.FieldValue.serverTimestamp(),
      peso_atual: esgotado ? frasco.peso_frasco_vazio : dados.pesoRetorno,
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (dados.acao === "EXTRAVIAR") {
      ...
      tx.update(frascoRef, {
        ...
        disponibilidade: venceuAgora ? "INDISPONIVEL" : "DISPONIVEL",
        em_quarentena: true,
        vencido: venceuAgora,
        detalhe_status: "Reencontrado - aguardando análise" + (venceuAgora ? " (vencido)" : "") + ": " + dados.motivo,
      });
...
export const descartarFrasco = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (frasco.estado_fisico_frasco !== "VAZIO" && frasco.estado_fisico_frasco !== "QUEBRADO") {
      throw new HttpsError("failed-precondition", "Apenas frascos vazios ou quebrados podem ser descartados institucionalmente.");
    }
