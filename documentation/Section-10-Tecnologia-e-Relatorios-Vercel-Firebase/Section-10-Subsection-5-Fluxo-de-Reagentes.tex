...
export const registrarDevolucao = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    // PDF-025: Ganho acima da tolerância bloqueia por anomalia. Retorno abaixo da tara é bloqueado e encaminhado ao fluxo Q06 para confirmação de esgotamento ou recalibração da tara.
...
\begin{lstlisting}[language=TypeScript, title={Extravio e Reencontro}]
...
export const registrarExtravioOuReencontro = onCall(async (request) => {
  ...
  const frascoRef = admin.firestore().collection("Frasco_Reagente").doc(dados.idFrasco);
  // Pré-busca do empréstimo ativo (evita query dentro da transação)
  const emprestimoAberto = await admin.firestore().collection("Emprestimo_Reagente")
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (dados.acao === "EXTRAVIAR") {
      ...
      tx.update(frascoRef, {
        ...
        disponibilidade: "INDISPONIVEL", // Não contabiliza no saldo
      ...
      tx.update(frascoRef, {
        ...
        detalhe_status: "Reencontrado. " + dados.motivo,
      });

      // Empréstimo encerrado anteriormente NÃO é reaberto.
