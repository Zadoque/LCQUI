...
async function validarPermissao(
  ...
  if (requerAtivo) {
    ...
    const versaoToken = request.auth.token["versao_permissoes"] as number | undefined;
    const versaoDoc = data?.versao_permissoes as number | undefined;
    if (versaoDoc !== undefined && versaoToken !== versaoDoc) {
...
\begin{regra}[Erratum M9 --- autoridade atual]
Este trecho histórico de M0--M8 é substituído, para toda operação crítica,
pela Seção~7/M9. Autenticação identifica o \texttt{uid}; autorização é a
decisão \texttt{podeExecutar(uid, operacao, recurso, estadoAtual)}. O backend
privilegiado lê, \textbf{dentro da mesma transação do efeito}, Usuário ativo,
papéis persistidos, vínculo/ownership e recurso canônico. Claim é somente
projeção assinada e nunca prova suficiente de papel atual ou escopo.
\end{regra}

\begin{lstlisting}[language=TypeScript, title={Núcleo obrigatório M9}]
async function mutacaoCriticaTx(tx, uid, operacao, recurso, dados) {
  const decisao = await podeExecutarTx(tx, uid, operacao, recurso);
  if (!decisao.permitida) throw new HttpsError("permission-denied", decisao.codigo);
  // idempotência M7 e todas as pré-condições são lidas antes da escrita neste tx.
  return aplicarEfeitoDeDominioTx(tx, uid, dados, decisao);
}
\end{lstlisting}

Estados ausentes, desconhecidos, divergentes ou não determináveis negam acesso.
Callables privilegiadas exigem App Check antes de Auth; no dashboard a ordem é
App Check $\rightarrow$ Auth $\rightarrow$ RBAC/escopo $\rightarrow$ rate
limit $\rightarrow$ cache/agregação, inclusive em cache hit. Jobs/triggers usam
IAM mínimo e entrada server-owned, e revalidam destinatário/recurso antes de
cada persistência.
