...
export const responderRequisicaoAdicaoBem = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (aprovar) {
      // PDF-001: Validação de chave única e leituras obrigatórias antes das escritas
      ...
      if (req.id_resumo_bem_patrimonial) {
        ...
        if (!resumoSnap.exists) throw new HttpsError("not-found", "Resumo referenciado não existe.");
        resumoNome = resumoSnap.data()!.nome;
      }
      
      const localSnap = await tx.get(admin.firestore().collection("Local").doc(req.id_local));
      if (!localSnap.exists) throw new HttpsError("not-found", "Local referenciado não existe.");
    ...
    // DEVE ocorrer APÓS as leituras da transação (tx.get)
