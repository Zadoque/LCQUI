...
export const responderRequisicaoEdicaoBem = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (req.novo_id_local) {
      ...
      if (!localSnap.exists) throw new HttpsError("not-found", "Local referenciado na requisição não encontrado.");
    }
    ...
    // PASSO 3: Aprovação -- verificar versão e aplicar alterações
    // N-02: se o bem foi alterado após a abertura da requisição, a versão diverge.
    // Política normativa: rejeitar a requisição como conflito E liberar o lock;
    // NÃO abortar sem liberar o lock (evita lock irrecuperável).
