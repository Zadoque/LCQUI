...
export const responderRequisicaoAdicaoBem = onCall(async (request) => {
  ...
  return admin.firestore().runTransaction(async (tx) => {
    ...
    if (aprovar) {
      // PDF-001: Validação de chave única (leitura obrigatória antes das escritas)
      // Unificação da definição da chave de unicidade (Bem_Patrimonial__{numero}) com normalização
      const numeroNormalizado = req.numero_patrimonio_proposto.trim().toUpperCase();
      chaveUnicaRef = admin.firestore().collection("Chaves_Unicas")
        .doc(`Bem_Patrimonial__${numeroNormalizado}`);
      const chaveUnicaSnap = await tx.get(chaveUnicaRef);
      if (chaveUnicaSnap.exists) {
