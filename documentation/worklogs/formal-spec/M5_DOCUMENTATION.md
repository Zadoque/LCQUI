...
Nota de estado corrente (pré-M8): `M6 = DOCUMENTATION_VALIDATED`, `M7 = DOCUMENTATION_VALIDATED`, `M8 = NOT_STARTED`; backfill executável M5–M7 pendente. Este arquivo é registro histórico do fechamento de M5.
...
- M6 = NOT_STARTED (naquele checkpoint; no checkpoint pós-M6, M6 = DOCUMENTATION_VALIDATED e M7 = NOT_STARTED). Nenhuma HQ M5 aberta; findings M5-F01..F08 encerrados.

PRÓXIMA AÇÃO EXATA (no checkpoint de fechamento de M5):
INICIAR M6 DOCUMENTAL — Q06 / tara / metrologia quantitativa.
(Concluída posteriormente, assim como M7 documental. A ação corrente é o backfill executável pré-M8; ver `PRE_M8_FORMALIZATION_BACKFILL.md`.)

Naquele checkpoint, a formalização executável de M5 (CUE/Alloy/Rust/IR/receipt)
era dívida futura. Ela é objeto da etapa extraordinária pré-M8.
