...
\nolinkurl{Turma/\{turmaId\}/Posts/\{postId\}/Comentarios} & Idem ao Post pai.\\
...
\texttt{Roteiro\_Experimento} & Read: \texttt{resource.data.id\_professor\_upload == uid} OU existe documento em \texttt{Roteiro\_Professor\_Compartilhado} com \texttt{id\_professor\_destino == uid}. Write: negado.\\
...
Além do papel, toda operação verifica conta ativa, versão de permissões e escopo atual. Atualizar Custom Claims não invalida imediatamente um token já emitido. A aplicação deve renovar o token após sincronização e o backend/Rules deve rejeitar versão desatualizada ou conta inativa consultando o controle persistido. Preferência de papel no cliente só altera apresentação.
