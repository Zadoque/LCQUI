...
As transições para estado físico \texttt{EXTRAVIADO}, entrada ou saída de \texttt{em\_quarentena}, desativação ou exclusão lógica nunca devem ser expostas para escrita direta do cliente. O backend deve validar as condições de contorno, registrar motivo e autor da ação, e garantir efeitos colaterais (como encerramento de empréstimos), persistindo toda a operação no \texttt{Historico\_Frasco\_Reagente} de forma atômica.
\end{regra}
